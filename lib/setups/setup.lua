...
require("lib/managers/menu/NXInvitationsGUI")
...
function Setup:load_packages()
	...
	elseif not PackageManager:loaded("packages/nx_base") then
		PackageManager:load("packages/nx_base")
	end
	...
	if Global.load_level then
		...
		if not using_steam_controller then
			...
			if SystemInfo:platform() == Idstring("NX64") then
				load_level_data.controller_image = "guis/textures/switch/joycon"
				load_level_data.controller_image_alt = "guis/textures/switch/switch_pro"
				load_level_data.nx_controller_type = managers.controller.nx_controller_type

				if managers.controller.nx_controller_type == "joycon" then
					coords = tweak_data:get_joycon_coords()
				else
					coords = tweak_data:get_switch_pro_coords()
				end
