...
function LightLoadingScreenGuiScript:init(scene_gui, res, progress, base_layer, is_win32)
	...
	self._fullscreen_panel = self._ws:panel()
	self._gui_tweak_data = {}

	if is_NX64 then
		self._gui_tweak_data.upper_saferect_border = 0
		self._gui_tweak_data.border_pad = 0
	else
	...
	if is_NX64 then
		self._background = self._fullscreen_panel:bitmap({
			texture = "guis/textures/switch/loadingscreen_pattern",
			name = "background",
			layer = self._base_layer + 1
		})

		self._background:set_size(self._fullscreen_panel:w(), self._fullscreen_panel:h())

		self._stonecold_small_logo = self._saferect_panel:bitmap({
			texture = "guis/textures/switch/loadingscreen_logo_switch",
			name = "stonecold_small_logo",
			layer = self._base_layer + 2
		})

		self._stonecold_small_logo:set_size(1024, 256)
		self._stonecold_small_logo:set_center_x(self._safe_rect_pixels.width / 2)
		self._stonecold_small_logo:set_center_y(self._safe_rect_pixels.height / 2)
	else
		self._stonecold_small_logo = self._saferect_panel:bitmap({
...
function LightLoadingScreenGuiScript:setup(res, progress)
	...
	if is_NX64 then
		self._background:set_size(self._fullscreen_panel:w(), self._fullscreen_panel:h())
		self._background:set_right(self._background:parent():w())
		self._background:set_bottom(self._background:parent():h())
		self._stonecold_small_logo:set_size(1024, 256)
		self._stonecold_small_logo:set_center_x(self._saferect_panel:w() / 2)
		self._stonecold_small_logo:set_center_y(self._saferect_panel:h() / 2)
	end
