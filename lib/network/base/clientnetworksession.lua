...
function ClientNetworkSession:request_join_host(host_rpc, result_cb)
	...
	local id, peer = self:add_peer(host_name, nil, nil, nil, nil, 1, nil, host_user_id, xuid, "")
	...
	local xuid = (SystemInfo:platform() == Idstring("X360") or SystemInfo:platform() == Idstring("XB1") or SystemInfo:platform() == Idstring("NX64")) and managers.network.account:player_id() or ""
...
function ClientNetworkSession:on_NX64_connection_established(name, ip)
	if SystemInfo:platform() ~= Idstring("NX64") then
		return
	end

	self:chk_send_connection_established(name, ip, false)
end
