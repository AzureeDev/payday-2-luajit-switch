...
function BaseNetworkSession:peer_by_ip(ip)
	if SystemInfo:platform() == Idstring("NX64") then
		local new_peer_ip_address_split = string.split(ip, ":")
		local new_peer_ip = new_peer_ip_address_split[1]

		for peer_id, peer in pairs(self._peers_all) do
			local peer_ip = peer:ip()

			if peer_ip then
				local old_peer_ip_address_split = string.split(peer_ip, ":")
				local old_peer_ip = old_peer_ip_address_split[1]

				if old_peer_ip == new_peer_ip then
					return peer
				end
			end
		end
...
function BaseNetworkSession:_on_peer_removed(peer, peer_id, reason)
	...
	if Network:multiplayer() then
		if SystemInfo:platform() == Idstring("X360") or SystemInfo:platform() == Idstring("XB1") or SystemInfo:platform() == Idstring("PS4") or SystemInfo:platform() == Idstring("NX64") then
...
function BaseNetworkSession:nx64_disconnected()
	if Global.game_settings.single_player then
		return
	end
...
function BaseNetworkSession:nx64_disconnect(connected)
	if not connected and not NX64Online:is_online() and game_state_machine:current_state().on_disconnected then
		game_state_machine:current_state():on_disconnected()
	end
end
...
function BaseNetworkSession:chk_send_connection_established(name, user_id, peer)
	...
	elseif SystemInfo:platform() == Idstring("NX64") then
		...
		local rpc = Network:handshake(connection_info.external_ip, managers.network.DEFAULT_PORT, "TCP_IP")
