...
function MousePointerManager:_setup()
	...
	if SystemInfo:platform() == Idstring("NX64") then
		...
		self._mouse:child("pointer"):set_size(30, 36)
	end
...
function MousePointerManager:set_pointer_image(type)
	...
	if SystemInfo:platform() == Idstring("NX64") and type == "arrow" then
		type = "hand"
	end
...
function MousePointerManager:is_controller_mouse()
	return self._controller_updater and true or false
end
...
function MousePointerManager:_activate()
	...
	if not managers.menu:is_switch_controller() then
		if not self._controller_updater then
...
function MousePointerManager:hide()
	self._ws:hide()
end

function MousePointerManager:show()
	self._ws:show()
end
...
function MousePointerManager:force_move_mouse_pointer(x, y)
	...
	if not managers.menu:is_switch_controller() then
		self._ws:feed_mouse_position(self._mouse:world_position())
	elseif self._mouse_callbacks[#self._mouse_callbacks] and self._mouse_callbacks[#self._mouse_callbacks].mouse_move then
		self._mouse_callbacks[#self._mouse_callbacks].mouse_move(self._mouse, self._mouse:x(), self._mouse:y(), self._ws)
	end
end
