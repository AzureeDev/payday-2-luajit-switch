...
require("lib/managers/dialogs/NXKeyboardInputDialog")
require("lib/managers/dialogs/NXInvitationDialog")
...
function GenericSystemMenuManager:_show_class(data, generic_dialog_class, dialog_class, force)
	...
	force = self.force_override or force
...
NXSystemMenuManager = NXSystemMenuManager or class(GenericSystemMenuManager)
NXSystemMenuManager.KEYBOARD_INPUT_DIALOG = NXKeyboardInputDialog
NXSystemMenuManager.GENERIC_KEYBOARD_INPUT_DIALOG = NXKeyboardInputDialog
SystemMenuManager.PLATFORM_CLASS_MAP[Idstring("NX64"):key()] = NXSystemMenuManager

function NXSystemMenuManager:init()
	GenericSystemMenuManager.init(self)
end

function NXSystemMenuManager:block_exec()
	return GenericSystemMenuManager.is_active(self)
end

function NXSystemMenuManager:is_active()
	return GenericSystemMenuManager.is_active(self)
end

function NXSystemMenuManager:get_typed_text()
	return NXSystemMenuManager.KEYBOARD_INPUT_DIALOG.last_typed_string
end

function NXSystemMenuManager:check_active_state()
	local active = self:is_active(true)

	if not self._old_active_state ~= not active then
		if NXSystemMenuManager.KEYBOARD_INPUT_DIALOG.keyboard_just_closed then
			NXSystemMenuManager.KEYBOARD_INPUT_DIALOG.keyboard_just_closed = false

			self:event_active_changed(false)

			self._old_active_state = false
...
function NXSystemMenuManager:init_finalize()
	NXSystemMenuManager.super.init_finalize(self)

	if self.noti_obj == nil then
		self.noti_obj = NXInvitationBanner:new()
	end
end

function NXSystemMenuManager:resolution_changed()
	if self.noti_obj then
		self.noti_obj:resolution_changed()
	end

	NXSystemMenuManager.super.resolution_changed(self)
end

function NXSystemMenuManager:destroy()
	if self.noti_obj then
		self.noti_obj:destroy()

		self.noti_obj = nil
	end

	NXSystemMenuManager.super.destroy(self)
end

function NXSystemMenuManager:nudge_notifications()
	if self.noti_obj then
		self.noti_obj:kick_anim()
	end
end

function NXSystemMenuManager:conditionally_view_invitations(force)
	if self.noti_obj and (force or self.noti_obj:is_okay_to_open_invitations()) then
		self.noti_obj:opening_list_box()

		local dialog_data = {
			title = managers.localization:text("nx64_noti_invites_title"),
			button_list = {},
			no_buttons = true,
			id = "nx_invitation_list",
			force = true
		}
		local success = self:_show_class(dialog_data, NXInvitationsDialog, NXInvitationsDialog, dialog_data.force)

		self:_show_result(success, dialog_data)
