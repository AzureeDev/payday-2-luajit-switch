...
function ObjectInteractionManager:_update_rumble()
	if self._active_object_locked_data and alive(self._active_unit) and self._active_unit:interaction():active() and managers.rumble:is_playing(self._rumble_start) then
		local tweak_id = self._active_unit:interaction().tweak_data
		local tweak = tweak_id and tweak_data.interaction[tweak_id]

		if tweak then
			if not tweak.rumble_random_loop or not tweak.rumble_random_loop_length or not tweak.rumble_random_loop_delay then
				return
			end

			local current_time = TimerManager:main():time()

			if self._rand_rumble_delay <= current_time - self._rand_rumble_start then
				managers.rumble:stop(self._rumble_start)

				local tweak_id = self._active_unit:interaction().tweak_data
				local tweak = tweak_id and tweak_data.interaction[tweak_id]

				if tweak and tweak.rumble_start then
					self._rumble_start = managers.rumble:play(self._playing_rand_rumble and tweak.rumble_start or tweak.rumble_random_loop, nil, 1, nil, 0)
				end

				self._rand_rumble_start = current_time
				local min = self._playing_rand_rumble and tweak.rumble_random_loop_delay[1] or tweak.rumble_random_loop_length[1]
				local max = self._playing_rand_rumble and tweak.rumble_random_loop_delay[2] or tweak.rumble_random_loop_length[2]
				self._rand_rumble_delay = math.random(min, max) * 0.001
				self._playing_rand_rumble = not self._playing_rand_rumble
			end
		end
	end
end
	...
	if alive(self._active_unit) then
		...
		if timer then
			...
			if SystemInfo:platform() == Idstring("NX64") then
				...
				if tweak and tweak.rumble_start then
					self._rumble_start = managers.rumble:play(tweak.rumble_start, nil, 1, nil, 0)
					self._rand_rumble_delay = math.random(50, 150) * 0.01
					self._rand_rumble_start = TimerManager:main():time()
					self._playing_rand_rumble = false
				else
					self._rumble_start = managers.rumble:play("lock_picking", nil, 1, nil, 0)
				end
			end
		end
...
function ObjectInteractionManager:end_action_interact(player)
	...
	if alive(self._active_unit) then
		...
		if SystemInfo:platform() == Idstring("NX64") then
			...
			if tweak and tweak.rumble_done then
				self._rumble_done = managers.rumble:play(tweak.rumble_done, nil, 1, nil, 0)
			else
				self._rumble_done = managers.rumble:play("generic_grab", nil, 1, nil, 0)
			end
		end
	end
end
...
function ObjectInteractionManager:interupt_action_interact()
	...
	if SystemInfo:platform() == Idstring("NX64") and self._rumble_start then
		managers.rumble:stop(self._rumble_start)
	end
end
