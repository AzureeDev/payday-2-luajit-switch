...
function GenericDLCManager:give_dlc_and_verify_blackmarket()
	...
	if managers.blackmarket then
		managers.blackmarket:verify_installed_items()
		...
		if Global.dlc_save.packages[package_id] and self:is_dlc_unlocked(package_id) then
			for _, loot_drop in ipairs(data.content and data.content.loot_drops or {}) do
				if #loot_drop == 0 then
					...
					if entry and loot_drop.type_items == "weapon_mods" and entry.is_a_unlockable then
						has_item = managers.blackmarket:get_item_amount(global_value, loot_drop.type_items, loot_drop.item_entry, true) > 0
						passed = not has_item
					elseif entry and loot_drop.type_items ~= "weapon_mods" and entry.value == 0 then
						has_item = managers.blackmarket:get_item_amount(global_value, loot_drop.type_items, loot_drop.item_entry, true) > 0
...
function GenericDLCManager:has_bigscore()
	return self:is_dlcs_unlocked({
		"akm4_pack",
...
function GenericDLCManager:has_crimewave_edition()
	return true
end
...
NX64DLCManager = NX64DLCManager or class(GenericDLCManager)
DLCManager.PLATFORM_CLASS_MAP[Idstring("NX64"):key()] = NX64DLCManager

function NX64DLCManager:init()
	NX64DLCManager.super.init(self)
	...
	if not Global.dlc_manager then
		Global.dlc_manager = {
			all_dlc_data = {
				...
				akm4_pack = {
					is_default = true,
					product_id = "8ddca7fb-ee35-479a-8ad2-0d57162e1a7c",
					index = 25
				},
				character_pack_sokol = {
					is_default = true,
					product_id = "e07b2a06-3c8f-434d-8568-fd1789c7da65",
					index = 26
				},
				bbq = {
					is_default = true,
					product_id = "a696721f-e56f-467c-8eba-6e7cdafbb475",
					index = 27
				},
				west = {
					is_default = true,
					product_id = "2c4189f0-dc5b-4fb3-a3fa-7e451b4181bb",
					index = 28
				},
				arena = {
					is_default = true,
					product_id = "fc86b480-a6de-436e-b5de-8193b79a82b8",
					index = 29
				},
				kenaz = {
					is_default = true,
					product_id = "3af4c9c9-f071-427a-a56e-67a163f14534",
					index = 30
				},
				turtles = {
					is_default = true,
					product_id = "56739ebf-1727-4ea7-8eec-25ca09531bbf",
					index = 31
				},
				dragon = {
					is_default = true,
					product_id = "a51f2bde-ee19-415c-a1b0-e9528025b747",
					index = 32
				},
				berry = {
					is_default = true,
					product_id = "d30e334b-c0c3-45e9-ab3f-a1add0930c23",
					index = 33
				},
				steel = {
					is_default = true,
					product_id = "b27c6f5a-fb1c-40f8-b6d1-7b470bd2e765",
					index = 34
				},
				mad = {
					index = 35,
				...
				pal = {
					is_default = true,
					product_id = "e005fb93-84c6-4276-b029-e4321f6770aa",
					index = 37
				},
				peta = {
					is_default = true,
					product_id = "de50d563-bb5a-475e-ab12-884e4484a941",
					index = 38
				},
				opera = {
					is_default = true,
					product_id = "0582c53d-0390-4b1b-8581-113d2bad7225",
					index = 39
				},
				wild = {
					is_default = true,
					product_id = "d67d2237-b386-42fe-a434-9f942965f40b",
					index = 40
				},
				born = {
					is_default = true,
					product_id = "1f7060ab-35d3-451d-ae93-0f610a3ac2f2",
					index = 41
				},
				joy = {
					index = 43,
...
function NX64DLCManager:_verify_dlcs()
	local dlc_content_updated = false
	local old_verified = nil
	...
	for dlc_name, dlc_data in pairs(Global.dlc_manager.all_dlc_data) do
		...
		if dlc_data.is_default then
			...
			dlc_data.verified = NX64Online and NX64Online:check_dlc(dlc_data.product_id)
		end
...
function NX64DLCManager:chk_content_updated()
	print("[NX64DLCManager:chk_content_updated]")

	if not managers.blackmarket:currently_customizing_mask() and self:_verify_dlcs() then
		print("[NX64DLCManager:chk_content_updated] content updated")
...
function NX64DLCManager:on_signin_complete()
	self:chk_content_updated()
end
