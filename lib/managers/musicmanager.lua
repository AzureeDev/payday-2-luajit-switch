...
function MusicManager:jukebox_default_tracks()
	...
	if managers.dlc:is_dlc_unlocked("mad") then
		default_options.heist_pal = "track_44"
	end
...
function MusicManager:jukebox_music_tracks()
	...
	local lock_data = {
		...
		berry = managers.dlc and managers.dlc:is_dlc_unlocked("berry"),
		peta = managers.dlc and managers.dlc:is_dlc_unlocked("peta"),
		pal = managers.dlc and managers.dlc:is_dlc_unlocked("pal"),
		born = managers.dlc and managers.dlc:is_dlc_unlocked("born")
	}

	for _, data in ipairs(tweak_data.music.track_list) do
		table.insert(tracks, data.track)

		if data.lock and (SystemInfo:platform() ~= Idstring("NX64") or not self:track_unlocked(data.track)) then
...
function MusicManager:jukebox_menu_tracks()
	...
	local lock_data = {
		...
		berry = managers.dlc and managers.dlc:is_dlc_unlocked("berry"),
		born_wild = managers.dlc and (managers.dlc:is_dlc_unlocked("born") or managers.dlc:is_dlc_unlocked("wild")),
		peta = managers.dlc and managers.dlc:is_dlc_unlocked("peta"),
		pal = managers.dlc and managers.dlc:is_dlc_unlocked("pal")
	}
