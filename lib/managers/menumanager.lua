	...
	if is_start_menu then
		if SystemInfo:platform() ~= Idstring("NX64") then
			...
			local menu_main = {
				...
				renderer = "MenuRenderer",
				id = "start_menu",
				content_file = "gamedata/menus/start_menu_switch",
				callback_handler = MenuCallbackHandler:new()
			}
		...
		if SystemInfo:platform() ~= Idstring("NX64") then
			...
			local menu_pause = {
				...
				renderer = "MenuPauseRenderer",
				id = "pause_menu",
				content_file = "gamedata/menus/pause_menu_switch",
				callback_handler = MenuCallbackHandler:new()
			}
		...
		if SystemInfo:platform() ~= Idstring("NX64") then
			...
			local kit_menu = {
				...
				renderer = "MenuKitRenderer",
				id = "kit_menu",
				content_file = "gamedata/menus/kit_menu_switch",
				callback_handler = MenuCallbackHandler:new()
			}
...
function MenuManager:get_rumble_position()
	return 0, 0, 0
end
...
function MenuManager:_node_selected(menu_name, node)
	if self:is_nx64() then
		managers.system_menu:nudge_notifications()

		if menu_name == "menu_main" and node and (node:parameters().name == "main" or node:parameters().name == "play" or node:parameters().name == "crimenet_selection") and not managers.network.matchmake:in_game() then
			managers.network.matchmake:set_inet_mode(false, true)
		end
	end
...
function MenuManager:close_menu(menu_name)
	...
	if menu_name == "menu_pause" then
		if self:is_nx64() then
			managers.rumble:pause_rumbles(false, false)
		end
...
function MenuManager:is_switch_controller()
	return managers.controller:get_default_wrapper_type() == "nx64"
end
...
function MenuManager:toggle_menu_state()
	if self._is_start_menu then
		if self:is_nx64() then
			managers.system_menu:conditionally_view_invitations()
		end
	...
	if (not Application:editor() or Global.running_simulation) and not managers.system_menu:is_active() then
		...
		elseif (not self:active_menu() or #self:active_menu().logic._node_stack == 1 or not managers.menu:active_menu().logic:selected_node() or managers.menu:active_menu().logic:selected_node():parameters().allow_pause_menu) and managers.menu_component:input_focus() ~= 1 then
			...
			if self:is_nx64() then
				local pause = Global.game_settings.single_player
				local mute = not Global.game_settings.single_player

				managers.rumble:pause_rumbles(pause, mute)
			end
...
function MenuManager:activate()
	if #self._open_menus == 1 then
		managers.rumble:set_enabled(self:is_nx64() and managers.user:get_setting("rumble") or false)
...
function MenuManager:southpaw_changed(name, old_value, new_value)
	if self._controller.TYPE ~= "xbox360" and self._controller.TYPE ~= "ps3" and self._controller.TYPE ~= "xb1" and self._controller.TYPE ~= "ps4" and self._controller.TYPE ~= "nx64" then
		return
	end
...
function MenuManager:is_console()
	return self:is_ps3() or self:is_x360() or self:is_ps4() or self:is_xb1() or self:is_nx64()
end
...
function MenuManager:is_nx64()
	return SystemInfo:platform() == Idstring("NX64")
end
...
function MenuManager:_enter_online_menus_nx64()
	managers.platform:set_presence("Signed_in")
	managers.user:on_entered_online_menus()
...
function MenuManager:nx64_disconnect(connected)
	if not connected then
		managers.network:queue_stop_network()
		managers.platform:set_presence("Idle")
		managers.network.matchmake:leave_game()
		managers.network.friends:nx64_disconnected()
		self:exit_online_menues()
	end
...
function MenuManager:nx64_disconnected()
	if managers.network:session() then
		managers.network:queue_stop_network()
		managers.platform:set_presence("Idle")
		managers.network.matchmake:leave_game()
...
function MenuManager:leave_online_menu()
	...
	if self:is_x360() or self:is_xb1() or self:is_nx64() then
		managers.user:on_exit_online_menus()
	end
end
...
function MenuManager:on_press_rumble()
	if self:is_nx64() then
		managers.rumble:play("menu_click", nil, 0.8, nil, 0, true)
	end
end

function MenuManager:on_move_rumble()
	if self:is_nx64() then
		managers.rumble:play("menu_move", nil, 0.8, nil, 0, true)
	end
end

function MenuManager:on_slider_press_rumble(touch)
	if self:is_nx64() then
		if touch then
			if not self.slider_rumble or not managers.rumble:is_playing(self.slider_rumble) then
				self.slider_rumble = managers.rumble:play("menu_slider", nil, 0.8, nil, 0, true)
			end
		elseif not self.slider_touch_rumble or not managers.rumble:is_playing(self.slider_touch_rumble) then
			self.slider_touch_rumble = managers.rumble:play("menu_slider", nil, 0.8, nil, 0, true)
		end
	end
end

function MenuManager:update_slider_rumble(value, touch)
	if self:is_nx64() then
		self:on_slider_press_rumble(touch)

		if touch then
			if self.slider_rumble then
				managers.rumble:set_rumble_time(self.slider_rumble, value)
			end
		elseif self.slider_touch_rumble then
			managers.rumble:set_rumble_time(self.slider_touch_rumble, value)
		end
	end
end

function MenuManager:stop_slider_rumble(touch)
	if self:is_nx64() then
		if touch then
			if self.slider_rumble then
				managers.rumble:stop(self.slider_rumble)

				self.slider_rumble = nil
			end
		elseif self.slider_touch_rumble then
			managers.rumble:stop(self.slider_touch_rumble)

			self.slider_touch_rumble = nil
		end
	end
end
...
function MenuCallbackHandler:visible_callback_coco()
	return self:is_dlc_latest_locked("coco")
end
...
function MenuCallbackHandler:visible_callback_mad()
	return self:is_dlc_latest_locked("mad")
end
...
function MenuCallbackHandler:is_nx64()
	return SystemInfo:platform() == Idstring("NX64")
end
...
function MenuCallbackHandler:is_not_nx64()
	return not self:is_nx64()
end
...
function MenuCallbackHandler:is_not_nextgen()
	return not self:is_xb1() and not self:is_ps4() and not self:is_nx64()
end
...
function MenuCallbackHandler:is_inet_lobby()
	return self:is_multiplayer() and managers.network.matchmake:is_inet_mode()
end

function MenuCallbackHandler:is_local_lobby()
	return self:is_server() and self:is_multiplayer() and managers.network.matchmake:is_local_mode()
end

function MenuCallbackHandler:is_inet_mode()
	return managers.network.matchmake:is_inet_mode()
end
...
function MenuCallbackHandler:play_single_player()
	...
	Global.game_settings.local_game = false
...
function MenuCallbackHandler:play_online_game()
	...
	if managers.network.matchmake then
		if SystemInfo:platform() == Idstring("NX64") then
			managers.network.matchmake:set_inet_mode(true, false)
		end

		if managers.network.matchmake.load_user_filters then
...
if SystemInfo:platform() == Idstring("NX64") then
	function MenuCallbackHandler:play_local_network_game()
		Global.game_settings.single_player = false
		Global.game_settings.local_game = true

		if managers.network.matchmake then
			if SystemInfo:platform() == Idstring("NX64") then
				managers.network.matchmake:set_local_mode()
			end
...
function MenuCallbackHandler:nx_view_invitations()
	if self:is_nx64() then
		managers.system_menu:conditionally_view_invitations(true)
	end
end
...
function MenuCallbackHandler:invite_friend_nx64(item)
	if item:parameters().signin_status ~= "signed_in" then
		return
	end

	managers.network.matchmake:send_join_invite(item:parameters().friend, item:parameters().name)
end
...
function MenuCallbackHandler:invite_friends_nx64()
	NX64Online:invite_friends()
end
...
function MenuCallbackHandler:view_gamer_card(item)
	NX64Online:show_gamer_card_ui(item:parameters().text_id)
end
...
InviteFriendsNX64 = InviteFriendsNX64 or class()

function InviteFriendsNX64:modify_node(node, up)
	local new_node = up and node or deep_clone(node)
...
function InviteFriendsNX64:refresh_node(node, friends)
		...
		if i < 103 then
			...
			if not item then
				local params = {
					texture_x = "46",
					localize = "false",
					is_options_button = "true",
					texture_scale_h = "66",
					texture_y = "31",
					callback = "invite_friend_nx64",
					is_large_menu_button = "true",
					texture_scale_w = "420",
					name = name,
					friend = friend._id,
				...
				local divider = node:create_item(data_node, divider_params)

				node:add_item(divider)
...
function InviteFriendsNX64:update_node(node)
...
function KickPlayer:modify_node(node, up)
	...
	if managers.network:session() then
		for _, peer in pairs(managers.network:session():peers()) do
			...
			if SystemInfo:platform() == Idstring("NX64") then
				params.is_large_menu_button = "true"
				params.is_options_button = "true"
				params.texture_x = "46"
				params.texture_y = "31"
				params.texture_scale_w = "420"
				params.texture_scale_h = "66"
				params.is_kick_menu = "true"
				params.name = peer:xuid()
				params.text_id = peer:xuid() .. " (" .. (peer:rank() > 0 and managers.experience:rank_string(peer:rank()) .. "-" or "") .. (peer:level() or "") .. ")"
			end
...
function MenuCrimeNetContactInfoInitiator:modify_node(original_node, data)
	...
	local divider_font_size = SystemInfo:platform() == Idstring("NX64") and 22 or nil

	node:clean_items()

	for i, codex in ipairs(codex_data) do
		self:create_divider(node, codex.id, codex.name_localized, nil, tweak_data.screen_colors.text, divider_font_size)

		for i, info_data in ipairs(codex) do
			self:create_item(node, info_data)

			if SystemInfo:platform() == Idstring("NX64") then
				self:create_divider(node, info_data.id, nil, 3)
			end
		end
...
function MenuCrimeNetContactInfoInitiator:create_divider(node, id, text_id, size, color, font_size, additional_params)
	local params = {
		localize = "false",
		name = "divider_" .. id,
		no_text = not text_id,
		...
		size = size or 8,
		color = color,
		font_size = font_size and font_size or nil
	}

	if additional_params and SystemInfo:platform() == Idstring("NX64") then
	...
	for _, job_data in ipairs(tweak_data.narrative.tutorials) do
		...
		if SystemInfo:platform() == Idstring("NX64") then
			info_data = {
				...
				texture_y = "31",
				texture_scale_h = "66",
				is_short_mission_button = "true",
				halign = "top",
				texture_scale_w = "420",
				multi_item_row = 2,
				multi_item_number = item_number,
				texture_rect = {
					46,
					31,
					420,
				...
				text = managers.localization:to_upper_text(heist_tweak.switch_name_id),
				subtext = managers.localization:to_upper_text(heist_tweak.switch_job_plan_id)
			}
		else
...
function MenuCrimeNetContactShortInitiator:create_item(node, data)
	local params = data
	params.name = data.id
	params.text_id = data.text
	params.localize = "false"
	params.callback = "play_short_heist"
	params.job = data.id
	local data_node = {}
	local new_item = node:create_item(data_node, params)
...
function MenuJukeboxInitiator:modify_node(node, data)
	if not node:item("choose_jukebox_your_choice") then
		...
		if SystemInfo:platform() == Idstring("NX64") then
			...
			params.wants_switch_arrows = true
			params.is_large_menu_button = true
			params.is_options_button = true
			params.jukebox_gui = true
			params.texture_x = "46"
			params.texture_y = "31"
			params.texture_scale_w = "420"
			params.texture_scale_h = "66"
...
function MenuJukeboxHeistPlaylist:modify_node(node, data)
		...
		data = {
			{
				...
				s_w = "64",
				_meta = "option",
				icon = "guis/textures/switch/tick_box_unselected_hud",
				x = "32",
				s_icon = "guis/textures/switch/tick_box_unselected_hud"
			},
			{
				w = "64",
				y = "32",
				...
				s_w = "64",
				_meta = "option",
				icon = "guis/textures/switch/tick_box_selected_hud",
				x = "32",
				s_icon = "guis/textures/switch/tick_box_selected_hud"
			},
			texture_scale_w = "420",
	...
	for _, track_name in pairs(track_list) do
		...
		local params = {
			...
			name = track_name,
			text_id = "menu_jukebox_" .. track_name,
			help_id = locked and "menu_jukebox_locked_consoles_" .. locked,
...
function MenuPrePlanningInitiator:create_info_items(node, params, selected_item)
	...
	self:create_divider(node, "cat_info", managers.localization:text("menu_pp_sub_info"), nil, tweak_data.screen_colors.text, nil, {
		is_preplanning_divider = true
	})
...
function MenuPrePlanningInitiator:modifiy_node_view_only(node, item_name, selected_item)
	...
	for i = 1, #tweak_data.preplanning.location_groups do
		...
		if data then
			self:create_divider(node, "sub_" .. tostring(i), managers.localization:text(data.name_id), nil, tweak_data.screen_colors.text, nil, {
				is_preplanning_divider = true
			})
...
function MenuPrePlanningInitiator:modifiy_node_preplanning(node, item_name, selected_item)
	...
	for i, data in ipairs(subgroups) do
		if #data.subgroup > 0 then
			self:create_divider(node, "cat_" .. tostring(i), managers.localization:text(data.name_id), nil, tweak_data.screen_colors.text, nil, {
				is_preplanning_divider = true
			})
...
function MenuPrePlanningInitiator:modifiy_node_preplanning_category(node, item_name, selected_item)
	...
	self:create_divider(node, 1, managers.preplanning:get_category_name(current_category), nil, tweak_data.screen_colors.text, nil, {
		is_preplanning_divider = true
	})
...
function MenuPrePlanningInitiator:modifiy_node_preplanning_type(node, item_name, selected_item)
	...
	self:create_divider(node, 1, managers.preplanning:get_type_name(current_type), nil, tweak_data.screen_colors.text, nil, {
		is_preplanning_divider = true
	})
	...
	for index = 1, #tweak_data.preplanning.location_groups do
		...
		if elements then
			self:create_divider(node, "div_" .. tostring(index), managers.preplanning:get_location_name_by_index(index), nil, tweak_data.screen_colors.text, nil, {
				is_preplanning_divider = true
			})
...
function MenuPrePlanningInitiator:modifiy_node_preplanning_plan(node, item_name, selected_item)
	...
	self:create_divider(node, 1, managers.preplanning:get_category_name(current_plan), nil, tweak_data.screen_colors.text, nil, {
		is_preplanning_divider = true
	})
...
function MenuPrePlanningInitiator:modifiy_node_preplanning_custom(node, item_name, selected_item)
	...
	self:create_divider(node, 1, managers.localization:text("menu_pp_extra_info"), nil, tweak_data.screen_colors.text, nil, {
		is_preplanning_divider = true
	})
...
function MenuCrimeNetGageAssignmentInitiator:modify_node(original_node, data)
	...
	if SystemInfo:platform() == Idstring("NX64") then
		self:create_divider(node, 1, managers.localization:text("menu_gage_assignment_div_menu"), nil, tweak_data.screen_colors.text, 23)
		self:create_item(node, {
			id = "_introduction",
			name_localized = managers.localization:text("menu_gage_assignment_introduction_title")
		})
		self:create_divider(node, "introduction", nil, 3)
		self:create_item(node, {
			id = "_summary",
			name_localized = managers.localization:text("menu_gage_assignment_summary_title")
		})
		self:create_divider(node, "summary", nil, 3)
		self:create_item(node, {
			id = "_video",
			name_localized = managers.localization:text("menu_gage_assignment_video_title")
		})
		self:create_divider(node, "video", nil, 3)
		self:create_divider(node, 3, managers.localization:text("menu_gage_assignment_div_packages"), nil, tweak_data.screen_colors.text, 23)
		...
		for assignment, data in ipairs(node_data) do
			...
			self:create_divider(node, assignment, nil, 3)
		end
	else
...
function MenuCrimeNetGageAssignmentInitiator:create_divider(node, id, text_id, size, color, font_size)
	local params = {
		localize = "false",
		name = "divider_" .. id,
		no_text = not text_id,
...
function MenuCrimeNetGageAssignmentInitiator:create_item(node, contact)
	local text_id = contact.name_localized
	local sub_text = contact.sub_text
	local files = contact.files
	local video_id = contact.video
...
function MenuCrimeNetSpecialInitiator:setup_node(node)
	...
	if not node:item("divider_end") then
		...
		if SystemInfo:platform() == Idstring("NX64") then
			params.wants_switch_arrows = true
			params.is_contract_broker = true
			params.font_size = "24"
			params.add_unique_panel = true
			params.hide_highlight = true
		end
		...
		if SystemInfo:platform() == Idstring("NX64") then
			self:create_divider(node, "title", self.contract_divider_id or "menu_cn_premium_buy_title", nil, tweak_data.screen_colors.text, {
				is_contract_broker = true,
				add_unique_panel = true
			})
		else
			self:create_divider(node, "title", self.contract_divider_id or "menu_cn_premium_buy_title", nil, tweak_data.screen_colors.text)
		end
...
function MenuCrimeNetSpecialInitiator:create_divider(node, id, text_id, size, color, additional_params)
	local params = {
		name = "divider_" .. id,
		no_text = not text_id,
		text_id = text_id,
...
function MenuCrimeNetSpecialInitiator:create_job(node, contract)
	...
	if SystemInfo:platform() == Idstring("NX64") then
		self:create_divider(node, id, nil, 3)
	end
end
...
function MenuReticleSwitchInitiator:setup_node(node, data)
	...
	if not node:item("divider_end") then
		...
		if SystemInfo:platform() == Idstring("NX64") then
			params.wants_switch_arrows = true
			params.is_large_menu_button = true
			params.switch_reticle = true
			params.colored_background = true
		end
		...
		if SystemInfo:platform() == Idstring("NX64") then
			self:create_divider(node, "multi", nil, 3)
		end
		...
		if SystemInfo:platform() == Idstring("NX64") then
			params_color.wants_switch_arrows = true
			params_color.is_large_menu_button = true
			params_color.switch_reticle = true
			params_color.colored_background = true
		end

		local data_node_color = {
			type = "MenuItemMultiChoice"
		}
		...
		local new_item_color = node:create_item(data_node_color, params_color)

		node:add_item(new_item_color)
		new_item_color:set_value(color_index)

		if SystemInfo:platform() == Idstring("NX64") then
			self:create_divider(node, "end", nil, 150)
		else
	...
	if not node:item("confirm") then
		local params_confirm = {
		...
		if SystemInfo:platform() == Idstring("NX64") then
			params_confirm.multi_item_row = 2
			params_confirm.multi_item_number = 0
			params_confirm.is_large_menu_button = true
			params_confirm.switch_reticle = true
			params_confirm.align = "center"
			params_confirm.texture_x = 45
			params_confirm.texture_y = 31
			params_confirm.texture_scale_w = 421
			params_confirm.texture_scale_h = 66
		end

		local data_node_confirm = {}
		local new_item_confirm = node:create_item(data_node_confirm, params_confirm)

		node:add_item(new_item_confirm)
		...
		if SystemInfo:platform() == Idstring("NX64") then
			params_back.multi_item_row = 2
			params_back.multi_item_number = 1
			params_back.is_large_menu_button = true
			params_back.switch_reticle = true
			params_back.align = "center"
			params_back.texture_x = 45
			params_back.texture_y = 31
			params_back.texture_scale_w = 421
			params_back.texture_scale_h = 66
			params_back.enabled = true
			params_back.queue_action = true
		end

		local data_node_back = {}
		local new_item_back = node:create_item(data_node_back, params_back)

		node:add_item(new_item_back)
	end
...
function MenuCrimeNetCasinoInitiator:_create_items(node, options)
	...
	if SystemInfo:platform() == Idstring("NX64") then
		preferred_params.font_size = 30
		preferred_params.wants_switch_arrows = true
		preferred_params.is_large_menu_button = true
		preferred_params.offshore_payday = true
	end
	...
	if SystemInfo:platform() == Idstring("NX64") then
		self:create_divider(node, "casino_divider_preferred", nil, 30)
	end

	local infamous_data = nil
	...
	if SystemInfo:platform() == Idstring("NX64") then
		infamous_params.align = "left"
		infamous_params.font = tweak_data.menu.pd2_large_font
		infamous_params.font_size = 30
		infamous_params.icon_by_text = false
		infamous_params.align_icon = "right"
		infamous_params.colored_background = true
		infamous_params.offshore_payday = true
	end
	...
	self:create_divider(node, "casino_divider_securecards", nil)

	local card1_data = nil
	...
	if SystemInfo:platform() == Idstring("NX64") then
		card1_params.align = "left"
		card1_params.font = tweak_data.menu.pd2_large_font
		card1_params.font_size = 30
		card1_params.icon_by_text = false
		card1_params.align_icon = "right"
		card1_params.colored_background = true
		card1_params.offshore_payday = true
	end
	...
	if SystemInfo:platform() == Idstring("NX64") then
		self:create_divider(node, "casino_divider_securecards1")
	end

	local card2_data = nil
	...
	if SystemInfo:platform() == Idstring("NX64") then
		card2_params.align = "left"
		card2_params.font = tweak_data.menu.pd2_large_font
		card2_params.font_size = 30
		card2_params.icon_by_text = false
		card2_params.align_icon = "right"
		card2_params.colored_background = true
		card2_params.offshore_payday = true
	end
	...
	if SystemInfo:platform() == Idstring("NX64") then
		self:create_divider(node, "casino_divider_securecards2")
	end

	local card3_data = nil
	...
	if SystemInfo:platform() == Idstring("NX64") then
		card3_params.align = "left"
		card3_params.font = tweak_data.menu.pd2_large_font
		card3_params.font_size = 30
		card3_params.icon_by_text = false
		card3_params.align_icon = "right"
		card3_params.colored_background = true
		card3_params.offshore_payday = true
	end
