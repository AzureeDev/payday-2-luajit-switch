...
function HUDHint:_animate_show(hint_panel, done_cb, seconds, text)
	...
	local offset = 32
	local _, _, hint_w, h = hint_text:text_rect()

	hint_text:set_w(hint_w + offset)
	clip_panel:set_w(hint_w + offset)
	clip_panel:set_center_x(clip_panel:parent():w() / 2)
	clip_panel:set_w(0)

	local target_w = hint_w + offset
	local w = 0
