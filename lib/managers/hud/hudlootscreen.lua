...
function HUDLootScreen:make_cards(peer, max_pc, left_card, right_card)
	...
	if SystemInfo:platform() == Idstring("NX64") then
		peer_name_string = is_local_peer and tostring(peer and peer:xuid() or NX64Online:name() or managers.blackmarket:get_preferred_character_real_name()) or peer and peer:xuid() or ""
	end
