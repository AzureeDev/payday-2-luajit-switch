if SystemInfo:platform() == Idstring("NX64") then
	function MenuManager:show_inet_connection_dialog_nx64()
		local dialog_data = {
			title = managers.localization:text("dialog_connecting_inet_title_NX64"),
	...
	function MenuManager:show_room_contains_blocked_player(params)
		local dialog_data = {
			title = managers.localization:text("dialog_error_title"),
			text = managers.localization:text("dialog_err_contains_blocked_player_NX64")
		}
		local ok_button = {
	...
	function MenuManager:show_unknown_error(info)
		local dialog_data = {
			title = managers.localization:text("dialog_error_title"),
			text = "Unknown Nintendo Pia Error: " .. info.error_code .. " : " .. info.error_description .. " (" .. info.task_name .. ")" .. "\n\n Please raise a bug..."
		}
		local ok_button = {
			text = managers.localization:text("dialog_ok")
		}
...
function MenuManager:show_new_message_dialog(params)
	...
	if SystemInfo:platform() == Idstring("NX64") and params.id then
		dialog_data.id = params.id
	end
	...
	if SystemInfo:platform() == Idstring("NX64") and #dialog_data.text > 250 then
		dialog_data.w = 900
	else
		dialog_data.w = 620
	end
