...
function FbiFilesInitiator:modify_node(node, up)
	...
	if managers.network:session() then
		...
		if SystemInfo:platform() == Idstring("NX64") then
			params.text_id = peer:xuid() .. " (" .. (managers.experience:current_rank() > 0 and managers.experience:rank_string(managers.experience:current_rank()) .. "-" or "") .. (managers.experience:current_level() or "") .. ")"
		end
