...
function NavigationManager:_reconstruct_geographic_segments()
	...
	for i_room, room in ipairs(self._rooms) do
		...
		local room_borders_x_neg = room_borders.x_neg - seg_offset.x
		local room_borders_x_pos = math.floor((room_borders.x_pos - seg_offset.x) / seg_size)
		local room_borders_y_neg = room_borders.y_neg - seg_offset.y
		local room_borders_y_pos = math.floor((room_borders.y_pos - seg_offset.y) / seg_size)

		if room_borders_x_neg > 0 and room_borders_x_neg % seg_size == 0 then
			room_borders_x_neg = room_borders_x_neg - seg_size
		end

		if room_borders_y_neg > 0 and room_borders_y_neg % seg_size == 0 then
			room_borders_y_neg = room_borders_y_neg - seg_size
		end

		room_borders_x_neg = math.floor(room_borders_x_neg / seg_size)
		room_borders_y_neg = math.floor(room_borders_y_neg / seg_size)
		local y_sweep = room_borders_y_neg

		while y_sweep <= room_borders_y_pos and y_sweep < nr_seg_y do
			local x_sweep = room_borders_x_neg

			while x_sweep <= room_borders_x_pos and x_sweep < nr_seg_x do
				local seg_idx = 1 + y_sweep * nr_seg_x + x_sweep
				segments[seg_idx] = segments[seg_idx] or {}
				segments[seg_idx].rooms = segments[seg_idx].rooms or {}
				segments[seg_idx].rooms[i_room] = true
				x_sweep = x_sweep + 1
			end
