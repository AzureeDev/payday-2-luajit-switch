...
function GenericUserManager:setup_setting_map()
	...
	self:setup_setting(10, "hold_to_run", not is_ps3 and not is_x360 and not is_ps4 and not is_xb1 and not is_nx64 and true)
...
NX64UserManager = NX64UserManager or class(GenericUserManager)
NX64UserManager.NOT_SIGNED_IN_STATE = "not_signed_in"
NX64UserManager.STORE_SETTINGS_ON_PROFILE = false
NX64UserManager.CAN_SELECT_USER = false
NX64UserManager.CAN_SELECT_STORAGE = false
NX64UserManager.CUSTOM_PROFILE_VARIABLE_COUNT = 3
NX64UserManager.CUSTOM_PROFILE_VARIABLE_CHAR_COUNT = 999
NX64UserManager.CAN_CHANGE_STORAGE_ONLY_ONCE = false
UserManager.PLATFORM_CLASS_MAP[Idstring("NX64"):key()] = NX64UserManager

function NX64UserManager:init()
	self._platform_setting_conversion_func_map = {
		gamer_control_sensitivity = callback(self, self, "convert_gamer_control_sensitivity")
	}
...
function NX64UserManager:update(t, dt)
	NX64UserManager.super.update(self, t, dt)
end

function NX64UserManager:_check_privilege_callback(is_success)
	if not self._privilege_check_enabled then
		return
	end
	...
	if not is_success and (self._in_online_menu or not Global.game_settings.single_player and not rawget(_G, "setup").IS_START_MENU) and not rawget(_G, "setup"):has_queued_exec() then
		...
		if self._privilege_check_fail_count > 1 then
			print("[NX64UserManager] Lost privileges.")
...
function NX64UserManager:disconnect_callback(reason)
	print("  NX64UserManager:disconnect_callback", reason)
	...
	if self._disconnected then
		print("[NX64UserManager:disconnect_callback] Already disconnected. No action taken.")
...
function NX64UserManager:connect_callback()
end

function NX64UserManager:on_entered_online_menus()
	self._disconnected = nil
	self._in_online_menu = true
end

function NX64UserManager:on_exit_online_menus()
	self._in_online_menu = false

	managers.platform:set_presence("Signed_out")
end

function NX64UserManager:is_online_menu()
	return self._in_online_menu
end

function NX64UserManager:convert_gamer_control_sensitivity(value)
	if value == "low" then
		return 0.5
	elseif value == "medium" then
		return 1
...
function NX64UserManager:active_user_change_state(old_user_data, user_data)
	Global.user_manager.platform_setting_map = nil
...
function NX64UserManager:load_platform_setting_map(callback_func)
	cat_print("user_manager", "[UserManager] Loading platform setting map.")
end

function NX64UserManager:_load_platform_setting_map_callback(callback_func, platform_setting_map)
	cat_print("user_manager", "[UserManager] Done loading platform setting map. Success: " .. tostring(not not platform_setting_map))
...
function NX64UserManager:save_platform_setting(setting_name, setting_value, callback_func)
	cat_print("user_manager", "[UserManager] Saving platform setting \"" .. tostring(setting_name) .. "\": " .. tostring(setting_value))
end

function NX64UserManager:_save_platform_setting_callback(callback_func, success)
	cat_print("user_manager", "[UserManager] Done saving platform setting \"" .. tostring("Dont get setting name in callback") .. "\". Success: " .. tostring(success))
...
function NX64UserManager:save_setting_map(callback_func)
	if self._setting_map_save_counter > 0 then
		Appliction:error("[UserManager] Tried to set setting map again before it was done with previous set.")
...
function NX64UserManager:_save_setting_map_callback(callback_func, success)
	self._setting_map_save_success = self._setting_map_save_success and success
	self._setting_map_save_counter = self._setting_map_save_counter - 1
...
function NX64UserManager:signin_changed_callback(selected_xuid)
	print("[NX64UserManager:signin_changed_callback] selected_xuid", selected_xuid)
	...
	for user_index, user_data in pairs(Global.user_manager.user_map) do
		local was_signed_in = user_data.signin_state ~= self.NOT_SIGNED_IN_STATE
		local is_signed_in = managers.network.account:signin_state() ~= "not_signed_in"
		user_data.has_signed_out = was_signed_in and not is_signed_in
	end

	if selected_user_index and self._active_check_user_callback_func then
		print("[NX64UserManager:signin_changed_callback] executing _active_check_user_callback_func")
		managers.system_menu:close("show_select_user_question_dialog")
		self._active_check_user_callback_func(true)
...
function NX64UserManager:profile_setting_changed_callback(...)
end

function NX64UserManager:update_all_users()
	local old_user_indexes = {}
...
function NX64UserManager:update_user(xuid, ignore_username_change)
	if type(xuid) == "string" then
		xuid = Xuid.from_string(xuid)
	end

	local signin_state = managers.network.account:signin_state()
	local is_signed_in = signin_state ~= self.NOT_SIGNED_IN_STATE
	local storage_id, username = nil

	print("[NX64UserManager:update_user] xuid", xuid, "signin_state", signin_state, "is_signed_in", is_signed_in)

	if is_signed_in then
		username = NX64Online:name()
		storage_id = Application:current_storage_device_id(xuid)
...
function NX64UserManager:storage_devices_changed_callback()
	self:update_all_users()
end

function NX64UserManager:get_xuid(user_index)
	local platform_id = self:get_platform_id(user_index)

	return platform_id
end

function NX64UserManager:invite_accepted_by_inactive_user()
end

function NX64UserManager:set_index(user_index)
	local old_user_index = Global.user_manager.user_index

	print("[NX64UserManager:set_index]", user_index, "old_user_index", old_user_index)
	Application:stack_dump()

	local user_index_str = user_index and tostring(user_index) or nil

	if old_user_index ~= user_index_str and user_index then
		self:update_user(user_index, false)
	end

	NX64UserManager.super.set_index(self, user_index_str)
end
