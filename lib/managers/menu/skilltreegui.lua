...
function SkillTreeGui:_setup(add_skilltree, add_specialization)
	...
	if #specialization_data > 0 then
		...
		self.desc_box_panel = desc_box_panel
		...
		if SystemInfo:platform() == Idstring("NX64") then
			self._btn_panel = self._specialization_panel:panel({
				...
				x = desc_box_panel:x(),
				w = desc_box_panel:w(),
				h = (tweak_data.menu.pd2_large_font_size + button_padding) * 3 + button_padding
			})
		else
			self._btn_panel = self._specialization_panel:panel({
				name = "btn_panel",
		...
		if SystemInfo:platform() == Idstring("NX64") then
			self._spec_scroll_bar_panel:set_visible(false)
		end
		...
		if SystemInfo:platform() == Idstring("NX64") then
			points_text:set_top(spec_box_panel:bottom() + 10)
		end
		...
		self.spec_description_panel = spec_description_panel
...
function SkillTreeGui:set_selected_item(item, no_sound)
	if self._is_skilltree_page_active then
		...
		return self:_set_selected_spec_item(item, no_sound)
	end
end
...
function SkillTreeGui:update_spec_descriptions(item)
	...
	if SystemInfo:platform() == Idstring("NX64") then
		self._spec_description_title:set_y(10)
	end
	...
	if SystemInfo:platform() == Idstring("NX64") then
		local x, y, w, h = self._spec_description_progress:text_rect()

		self.spec_description_panel:set_y(0)
		self.spec_description_panel:stop()
		self.spec_description_panel:set_h(self._spec_description_progress:y() + h)

		if self.desc_box_panel:h() < self.spec_description_panel:h() then
			local owning_panel = self.desc_box_panel

			self.spec_description_panel:animate(callback(nil, _G, "auto_scroll_text"), owning_panel)
		end
	end
end
...
function SkillTreeGui:touch_scroll(dy)
	local grid_panel = self._specialization_panel:child("spec_box_panel")
	local grid_scroll_panel = self._specialization_page_panel
	local limit = -1 * (grid_scroll_panel:h() + 30 - grid_panel:h())

	for tree, tree_item in ipairs(self._spec_tree_items) do
		local bottom_y = tree_item._tree_panel:bottom()
		local top_y = tree_item._tree_panel:top()
		local h = tree_item._tree_panel:h()
		local panel_reduce_points = tree_item._parent_panel:child("point_btns_panel" .. tostring(tree_item._tree)):child("reduce_points")
		local panel_add_points = tree_item._parent_panel:child("point_btns_panel" .. tostring(tree_item._tree)):child("increase_points")

		if panel_reduce_points:world_y() - 5 < grid_panel:world_bottom() and grid_panel:world_y() < panel_reduce_points:world_y() then
			panel_reduce_points:set_visible(true)
			panel_add_points:set_visible(true)
		else
			panel_reduce_points:set_visible(false)
			panel_add_points:set_visible(false)
		end

		if bottom_y + h > grid_scroll_panel:y() * -1 or top_y - h < grid_scroll_panel:y() * -1 + h then
			tree_item:set_visible(true)
		else
			tree_item:set_visible(false)
		end
	end
...
function SkillTreeGui:mouse_moved(o, x, y)
	if SystemInfo:platform() == Idstring("NX64") and self.pan_y and self.panning then
...
function SkillTreeGui:mouse_released(button, x, y)
	...
	if SystemInfo:platform() == Idstring("NX64") then
		for _, button in pairs(self._btns) do
			local data = button:data()

			if button:inside(x, y) then
				if data.name == "menu_st_add_spec_points" then
					self.switch_btn_add_points = false
				elseif data.name == "menu_st_remove_spec_points" then
					self.switch_btn_remove_points = false
				end
			end
		end
	end
...
function highlight_animation_skilltree_gui(panel, time_to_animate)
...
function SkillTreeGui:mouse_pressed(button, x, y)
	...
	if SystemInfo:platform() == Idstring("NX64") and self._specialization_panel:h() < self._specialization_page_panel:h() and self._specialization_page_panel:inside(x, y) then
	...
	if button == Idstring("0") then
			...
			if self._button_highlighted and self._btns[self._button_highlighted] and self._btns[self._button_highlighted]:inside(x, y) then
				...
				if data.callback and (not self._button_press_delay or self._button_press_delay < TimerManager:main():time()) then
					if SystemInfo:platform() == Idstring("NX64") then
						...
						self.anim_panel:child("anim_box"):animate(callback(nil, _G, "highlight_animation_skilltree_gui"), anim_time)
					end
		...
		if self._specialization_panel:child("controller_page_tab_panel"):child("prev_page"):inside(x, y) then
			self:previous_page(true)
		elseif self._specialization_panel:child("controller_page_tab_panel"):child("next_page"):inside(x, y) then
			self:next_page(true)
		end
	end

	if (button == Idstring("0") or button == Idstring("press")) and SystemInfo:platform() == Idstring("NX64") then
		for _, button in pairs(self._btns) do
			local data = button:data()

			if button:inside(x, y) then
				local anim_time = 0.15
				local panel = button._panel
				...
				if data.name == "menu_st_add_spec_points" and button.active then
					self.switch_btn_add_points = true

					managers.menu:on_press_rumble()
				elseif data.name == "menu_st_remove_spec_points" and button.active then
					self.switch_btn_remove_points = true
...
function SkillTreeGui:move_spec_item(x, y)
	...
	if self._selected_spec_tier then
		...
		return self:set_selected_item(tree_item:item(next_tier), true)
	else
...
function SkillTreeGui:update(t, dt)
	...
	if not managers.menu:is_pc_controller() or managers.menu:is_steam_controller() then
		...
		if self._selected_spec_tier == managers.skilltree:get_specialization_value(self._active_spec_tree, "tiers", "current_tier") + 1 then
			controller_spec_adding_points = not self._is_skilltree_page_active and managers.menu_component:get_controller_input_bool("upgrade_alternative1") or self.switch_btn_add_points
			controller_spec_removing_points = not controller_spec_adding_points and not self._is_skilltree_page_active and managers.menu_component:get_controller_input_bool("upgrade_alternative4") or self.switch_btn_remove_points
		end
...
function SkillTreeGui:show_btns(...)
	...
	if SystemInfo:platform() ~= Idstring("NX64") then
		...
		for i, btn in pairs(self._btns) do
			...
			self._btns[i].active = false

			self._btns[i]._panel:child("select_rect"):set_color(tweak_data.menu.disabled_color)
			self._btns[i]._panel:child("text"):set_alpha(tweak_data.menu.disabled_alpha)
			self._btns[i]._panel:child("select_rect"):set_alpha(tweak_data.menu.disabled_alpha + 0.05)
			table.insert(btns, self._btns[i])
		end

		for i, btn in ipairs(data) do
			if self._btns[btn] then
				self._btns[btn].active = true

				self._btns[btn]._panel:child("select_rect"):set_color(tweak_data.menu.unselected_color)
				self._btns[btn]._panel:child("select_rect"):set_alpha(tweak_data.menu.unselected_alpha)
				self._btns[btn]._panel:child("text"):set_alpha(1)
			end
		end
	end
...
function SpecializationTabItem:refresh()
	...
	if SystemInfo:platform() == Idstring("NX64") then
		local background = self._spec_tab:child("spec_tab_select_rect")

		if self._active then
			background:set_image("guis/textures/switch/large_selected_hud")
		else
			background:set_image("guis/textures/switch/large_unselected_hud")
		end

		background:set_texture_rect(46, 31, 420, 66)
		self._spec_tab:child("spec_tab_name"):set_color(self._active and Color.white or self._selected and tweak_data.screen_colors.button_stage_2 or tweak_data.screen_colors.button_stage_3)
	...
	local function anim_refresh(o)
		...
		while not is_done do
			...
			for _, item in ipairs(self._items) do
				...
				if SystemInfo:platform() == Idstring("NX64") and not item:update_size(dt, self._selected) then
					is_done = false
				end
			end
		end
...
function SpecializationTierItem:update_corners(visible)
	local cornerSize = visible and self.corner_deco.corner_visible_size or self.corner_deco.corner_hidden_size

	self.corner_deco.corners[1]:set_texture_rect(0, 0, cornerSize, cornerSize)
	self.corner_deco.corners[2]:set_texture_rect(cornerSize, 0, -cornerSize, cornerSize)
	self.corner_deco.corners[3]:set_texture_rect(0, cornerSize, cornerSize, -cornerSize)
	self.corner_deco.corners[4]:set_texture_rect(cornerSize, cornerSize, -cornerSize, -cornerSize)
	self.corner_deco.corners[1]:set_size(cornerSize, cornerSize)
	self.corner_deco.corners[2]:set_size(cornerSize, cornerSize)
	self.corner_deco.corners[3]:set_size(cornerSize, cornerSize)
	self.corner_deco.corners[4]:set_size(cornerSize, cornerSize)
	self.corner_deco.corners[1]:set_left(0)
	self.corner_deco.corners[1]:set_top(0)
	self.corner_deco.corners[2]:set_right(self.corner_deco.owning_panel:w())
	self.corner_deco.corners[2]:set_top(0)
	self.corner_deco.corners[3]:set_left(0)
	self.corner_deco.corners[3]:set_bottom(self.corner_deco.owning_panel:h())
	self.corner_deco.corners[4]:set_right(self.corner_deco.owning_panel:w())
	self.corner_deco.corners[4]:set_bottom(self.corner_deco.owning_panel:h())
end
...
function SpecializationTierItem:init(tier_data, tree_panel, tree, tier, x, y, w, h)
	...
	if SystemInfo:platform() == Idstring("NX64") then
		...
		self.corner_deco = {
			corners = {},
			owning_panel = tier_panel
		}
		self.corner_deco.corners[1] = tier_panel:bitmap({
		...
		self.corner_deco.corners[2] = tier_panel:bitmap({
			texture = "guis/textures/pd2/hud_corner",
			name = "TR",
			layer = 2,
			halign = "right",
		...
		self.corner_deco.corners[3] = tier_panel:bitmap({
			texture = "guis/textures/pd2/hud_corner",
			name = "BL",
			layer = 2,
			halign = "left",
		...
		self.corner_deco.corners[4] = tier_panel:bitmap({
			texture = "guis/textures/pd2/hud_corner",
			name = "BR",
			layer = 2,
			halign = "right",
		...
		self.corner_deco.corner_visible_size = self.corner_deco.corners[1]:w()
		self.corner_deco.corner_hidden_size = 2

		self:update_corners(false)
	end
...
function SpecializationTierItem:refresh()
	...
	if SystemInfo:platform() == Idstring("NX64") then
		self:update_corners(self._selected)
		self.bg_rect:set_color(self._selected and tweak_data.menu.selected_color or tweak_data.menu.unselected_color)
		self.bg_rect:set_alpha(self._selected and tweak_data.menu.selected_alpha or tweak_data.menu.unselected_alpha)
	end
...
function SpecializationGuiButtonItem:init(main_panel, data, x)
	...
	if SystemInfo:platform() == Idstring("NX64") then
		self._panel:set_size(self.main_panel_width, large_font_size)
		self._panel:rect({
			name = "select_rect",
			layer = 48,
			halign = "scale",
		...
		make_fine_text(self._btn_text)
		self._panel:set_size(self.main_panel_width - x * 2, medium_font_size)
		self._panel:rect({
			blend_mode = "add",
			name = "select_rect",
			halign = "scale",
...
function SpecializationGuiButtonItem:set_order(prio)
	if SystemInfo:platform() == Idstring("NX64") then
		...
		self._panel:set_size(self.main_panel_width - x_padding * 2, large_font_size)
		self._panel:set_y(math.max(0, prio - 1) * (self._panel:h() + 6) + 6)
		self._panel:set_x(self._panel:parent():x() + x_padding)
