...
function StatsTabItem:init(panel, tab_panel, text, i)
	...
	if SystemInfo:platform() == Idstring("NX64") then
		self._select_rect = tab_panel:bitmap({
			texture = "guis/textures/switch/job_overview_selected_hud",
			layer = 0,
			visible = false,
		...
		self._deselect_rect = tab_panel:bitmap({
			texture = "guis/textures/switch/job_overview_unselected_hud",
			layer = 0,
			visible = true,
	...
	if self._deselect_rect then
		self._deselect_rect:set_shape(self._tab_text:shape())
	end
...
function StageEndScreenGui:init(saferect_ws, fullrect_ws, statistics_data)
	...
	if SystemInfo:platform() == Idstring("NX64") then
		self._continue_button:set_rightbottom(self._panel:w() - 30, self._panel:h() - 10)

		self._continue_button_box = self._panel:bitmap({
		...
		self:update_continue_button_dims()
	else
...
function StageEndScreenGui:update_continue_button_dims()
	if SystemInfo:platform() == Idstring("NX64") then
		self._continue_button:set_rightbottom(self._panel:w() - 30, self._panel:h() - 10)
		self._continue_button_box:set_size(self._continue_button:w() * 1.2, self._continue_button:h() * 1.3)
		self._continue_button_box:set_center_y(self._continue_button:center_y())
		self._continue_button_box:set_center_x(self._continue_button:center_x())
	end
end
...
function StageEndScreenGui:set_continue_button_text(text, not_clickable)
	...
	self._continue_button:set_color(self._button_not_clickable and tweak_data.screen_colors.item_stage_1 or tweak_data.screen_colors.button_stage_4)
	self:update_continue_button_dims()
end
...
function StageEndScreenGui:mouse_moved(x, y)
	...
	elseif self._continue_button_highlighted then
		...
		self._continue_button:set_color(tweak_data.screen_colors.button_stage_4)
		managers.menu_component:post_event("highlight")
	end
