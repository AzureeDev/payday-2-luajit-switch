	...
	for index, item in pairs(self._tree_items) do
		if tweak_data.infamy.tree[index] then
			...
			if SystemInfo:platform() == Idstring("NX64") then
				color = item.owned and tweak_data.screen_colors.item_stage_1 or item.unlocked and tweak_data.screen_colors.item_stage_2 or tweak_data.menu.unselected_color
			end
			...
			item.panel:text({
				...
				x = 0,
				layer = 1,
				y = item_height - tweak_data.menu.pd2_small_font_size - border_size - padding,
				w = item_width,
				font = tweak_data.menu.pd2_small_font,
				font_size = tweak_data.menu.pd2_small_font_size
			})
			...
			if SystemInfo:platform() == Idstring("NX64") then
				local bg_panel = item.panel:panel({
					name = "bg_panel",
					h = item.panel:h() - padding,
					w = item.panel:w() - padding
				})

				bg_panel:move(padding * 0.5, padding * 0.5)

				local bg_rect = bg_panel:rect({
					name = "bg_rect",
					color = tweak_data.menu.unselected_color,
					alpha = tweak_data.menu.unselected_alpha
				})
				local corner_deco = {}
				local panel = bg_panel
				corner_deco.corners = {
					panel:bitmap({
				...
				item.corner_deco = corner_deco

				bg_panel:set_visible(item.unlocked or item.owned and true or false)
			end
...
function InfamyTreeGui:_select_item(index)
	...
	if self._selected_item ~= index then
		if self._selected_item then
			...
			local unavailable_color = tweak_data.screen_colors.item_stage_3

			if SystemInfo:platform() == Idstring("NX64") then
				unavailable_color = tweak_data.menu.unselected_color
			end

			selected_item.panel:child("image"):set_color(self._tree_items[self._selected_item].owned and tweak_data.screen_colors.item_stage_1 or self._tree_items[self._selected_item].unlocked and tweak_data.screen_colors.item_stage_2 or unavailable_color)
			selected_item.panel:child("text"):set_text("")
			selected_item.panel:child("text"):set_alpha(0)

			if SystemInfo:platform() == Idstring("NX64") then
				local panel = selected_item.panel:child("bg_panel")

				panel:child("bg_rect"):set_color(tweak_data.menu.unselected_color)
				panel:child("bg_rect"):set_alpha(tweak_data.menu.unselected_alpha)

				local corner_deco = selected_item.corner_deco
				local cornerSize = corner_deco.corner_hidden_size
		...
		if SystemInfo:platform() == Idstring("NX64") then
			local panel = item.panel:child("bg_panel")

			panel:child("bg_rect"):set_color(tweak_data.menu.selected_color)
			panel:child("bg_rect"):set_alpha(tweak_data.menu.selected_alpha)

			local corner_deco = item.corner_deco
			local cornerSize = corner_deco.corner_visible_size
...
function InfamyTreeGui:reload()
	...
	for index, item in pairs(self._tree_items) do
		...
		item.panel:child("image"):set_color(item.owned and tweak_data.screen_colors.item_stage_1 or item.unlocked and tweak_data.screen_colors.item_stage_2 or unavailable_color)

		item.select_size = item.owned and self._owned_selected_size or item.unlocked and self._unlocked_selected_size or self._locked_selected_size
		item.unselect_size = item.owned and self._owned_unselected_size or item.unlocked and self._unlocked_unselected_size or self._locked_unselected_size

		if SystemInfo:platform() == Idstring("NX64") then
			item.panel:child("bg_panel"):set_visible(item.unlocked or item.owned and true or false)
		end
	...
	if button == Idstring("0") then
		...
		if self._selected_item then
			...
			if infamy_item and infamy_item.panel:inside(x, y) then
				if self.just_selected_item then
...
function InfamyTreeGui:move(x, y)
	...
	local is_new = self._selected_item ~= new_selected
