...
function animation_scroll_text_horizontal(text, limit)
	text:set_x(0)
...
function MenuNodeGui:_create_legends_nx(node)
	local safe_rect_pixels = self:_scaled_size()
	local res = RenderSettings.resolution
	local visible = true
	...
	self.legend_y_offset = node:parameters().legend_y_offset and node:parameters().legend_y_offset or 0
	self.legend_x_offset = node:parameters().legend_x_offset and node:parameters().legend_x_offset or 0
	local scale_factor = self.legends_scale or 1
	self._legends_panel = self.ws:panel():panel({
		name = "legend_panel",
		visible = visible,
		x = safe_rect_pixels.x + self.legend_x_offset,
		y = safe_rect_pixels.y + self.legend_y_offset,
		w = safe_rect_pixels.width,
		h = safe_rect_pixels.height
	})
	local w = 0
	...
	for i, legend in ipairs(node:legends()) do
		...
		local legend_panel = self._legends_panel:panel({
			name = legend.string_id or "no_button",
			visible = visible
		})
		local panel_requires_button = legend.string_id == "menu_legend_back" or legend.string_id == "menu_casino_choice_bet"
		local spacing = i > 1 and "  |  " or ""
		local t_text = utf8.to_upper(managers.localization:text(legend.string_id, {
			BTN_UPDATE = managers.localization:btn_macro("menu_update"),
			BTN_BACK = managers.localization:btn_macro("back")
		}))
		local text = nil

		if panel_requires_button then
			text = legend_panel:text({
				text = t_text,
				font = tweak_data.menu.pd2_large_font,
				font_size = (panel_requires_button and self.legends_font_size or self.font_size) * scale_factor,
				color = self.color,
				layer = self.layers.items
			})
		else
		...
		if panel_requires_button then
			legend_panel:set_size(width * 1.5, height * 1.5)
		else
			legend_panel:set_size(width, height)
		end

		text:set_center_x(legend_panel:w() / 2)
		text:set_center_y(legend_panel:h() / 2)
		legend_panel:set_x(w)
		legend_panel:set_bottom(self._legends_panel:h())

		if panel_requires_button then
			local button_img = legend_panel:bitmap({
			...
			button_img:set_size(legend_panel:w() * tweak_data.menu.switch_legend_button_scale_w * scale_factor, legend_panel:h() * tweak_data.menu.switch_legend_button_scale_h * scale_factor)
			button_img:set_center_y(legend_panel:h() / 2)
			button_img:set_center_x(legend_panel:w() / 2)
		end

		if h < legend_panel:h() then
			h = legend_panel:h()
		end

		w = w + width

		table.insert(created, legend_panel)
	end

	self._legends_panel:set_w(w)
	self._legends_panel:set_h(h)

	for i, legend_panel in ipairs(created) do
		legend_panel:set_center_y(self._legends_panel:center_y())
	end
...
function MenuNodeGui:_setup_item_panel_parent(safe_rect, shape)
	...
	if SystemInfo:platform() == Idstring("NX64") then
		if self.name == "jukebox" then
			y = 30
		elseif not shape or not shape.force_apply then
			y = y * 1.5
		end
	end
	...
	if SystemInfo:platform() == Idstring("NX64") and not shape.force_apply then
		self._item_panel_parent:set_h(safe_rect.height - CoreMenuRenderer.Renderer.border_height * 2)
	end
	...
	if SystemInfo:platform() ~= Idstring("NX64") then
		...
		self._legends_panel:set_bottom(self.ws:panel():bottom() + self.legend_y_offset)
	end
	...
	if SystemInfo:platform() == Idstring("NX64") then
		self._item_panel_parent:set_h(self._item_panel_parent:h() - self.legends_font_size)
	end

	if SystemInfo:platform() == Idstring("NX64") and self._item_panel_parent:h() < self.item_panel:h() then
		if not self.scroll_bar_bg then
			self.scroll_bar_bg = self._item_panel_parent:panel({
				y = 0,
				name = "scroll_bar_bg",
				visible = false,
				w = 14,
			...
			self._scroll_bar_bg_box_class = self.scroll_bar_bg:rect({
				name = "scroll_bar_box",
				valign = "grow",
				halign = "grow",
				alpha = 0.2,
				layer = 500,
				color = Color(0, 0.69, 1)
			})
		end

		if not self.scroll_bar then
			self.scroll_bar = self._item_panel_parent:panel({
				y = 0,
				name = "scroll_bar",
				visible = false,
				w = 14,
				x = 0,
				h = self._item_panel_parent:h() / self.item_panel:h() * self._item_panel_parent:h()
			})
			self._scroll_bar_box_class = self.scroll_bar:rect({
				name = "scroll_bar_box",
				valign = "grow",
				halign = "grow",
				alpha = 1,
		...
		self.scroll_bar_bg:set_right(self._item_panel_parent:w())
		self.scroll_bar:set_right(self._item_panel_parent:w())
	end
end
...
function MenuNodeGui:_setup_item_panel(safe_rect, res)
	...
	if self._item_panel_parent:h() < self.item_panel:h() then
		...
		if SystemInfo:platform() == Idstring("NX64") then
			self.scroll_bar_bg:set_h(self._item_panel_parent:h())
			self.scroll_bar:set_h(self._item_panel_parent:h() / self.item_panel:h() * self._item_panel_parent:h())
			self.scroll_bar_bg:set_visible(true)
			self.scroll_bar:set_visible(true)
			...
			if not self.dont_move_item_panel_for_scroll_bar then
				-- Nothing
			end

			for _, item in pairs(self.row_items) do
				local new_menu_w = tweak_data.menu.main_menu_width - tweak_data.menu.scroll_bar_width - tweak_data.menu.main_menu_padding
				item.has_scroll_bar = true

				if item.is_large_menu_button or item.is_options_button and not item.is_crimenet_contacts then
					if item.items_on_row and item.items_on_row > 1 then
						local new_menu_multi_width = (new_menu_w - (item.items_on_row - 1) * tweak_data.menu.main_menu_padding) / 2
						local adjustment = item.gui_panel:w() - new_menu_multi_width

						item.gui_panel:set_w(new_menu_multi_width)
						item.gui_panel:set_x(item.gui_panel:x() - adjustment * item.item_number)
					else
						item.gui_panel:set_w(new_menu_w)
					end
				end

				if item.item.reload then
					item.item:reload(item, self)
				end
			end
		end
	end
...
function MenuNodeGui.add_corner_deco(row_item, o_panel, c_layer, bounding_obj)
	local corners = {}
	row_item.corner_deco = corners
	corners[1] = o_panel:bitmap({
	...
	corners.corner_size_large = corners[1]:w()
	corners.corner_size_small = 2
	corners.bounds = bounding_obj
	corners.highlighted = false

	function corners:set_corner_size(cornerSize)
		local panObject = self.bounds

		for i = 1, 4 do
			local corner = self[i]

			corner:set_w(cornerSize)
			corner:set_h(cornerSize)
		end

		self[1]:set_texture_rect(0, 0, cornerSize, cornerSize)
		self[2]:set_texture_rect(cornerSize, 0, -cornerSize, cornerSize)
		self[3]:set_texture_rect(0, cornerSize, cornerSize, -cornerSize)
		self[4]:set_texture_rect(cornerSize, cornerSize, -cornerSize, -cornerSize)
		self[1]:set_world_left(panObject:world_left())
		self[1]:set_world_top(panObject:world_top())
		self[2]:set_world_right(panObject:world_right())
		self[2]:set_world_top(panObject:world_top())
		self[3]:set_world_left(panObject:world_left())
		self[3]:set_world_bottom(panObject:world_bottom())
		self[4]:set_world_right(panObject:world_right())
		self[4]:set_world_bottom(panObject:world_bottom())
	end

	function corners:set_highlight(newState)
		if self.highlighted ~= newState then
			self.highlighted = newState

			self:set_corner_size(newState and self.corner_size_large or self.corner_size_small)
		end
	end

	function corners:resize(bounding_obj)
		self.bounds = bounding_obj or self.bounds
		local isHighlighted = self.highlighted
		self.highlighted = 2

		self:set_highlight(isHighlighted)
	end

	function corners:kill()
		local parent = self[1]:parent()

		for i = 1, 4 do
			parent:remove(self[i])
...
function MenuNodeGui.init_pan(row_item, panObject, item_panel)
	local state = row_item.pan_state or {}
	row_item.pan_state = state
	state.row_item = row_item
	state.item_panel = item_panel
	state.bmObject = panObject
	...
	local bglayer = panObject:layer()
	local shadow = item_panel:rect({
		y = 0,
		x = 0,
		visible = false,
		layer = bglayer - 1
	...
	shadow:set_color(Color.black)
	shadow:set_alpha(0.2)

	local bluepane = item_panel:bitmap({
	...
	state.bluepaneObject = bluepane

	bluepane:set_alpha(1)
	bluepane:set_blend_mode("add")

	local fh = bluepane:texture_height()
	local fw = bluepane:texture_width()

	bluepane:set_texture_rect(0, fh, fw, -fh)

	local flicker2 = item_panel:bitmap({
	...
	state.flicker2Object = flicker2

	flicker2:set_alpha(0.4)
	flicker2:set_blend_mode("add")
	flicker2:set_color(Color(255, 52, 161, 226) / 255)
	flicker2:set_wrap_mode("clamp")

	local flicker3 = item_panel:bitmap({
		texture = "guis/textures/pd2/skilltree_2/subtree_fill",
		name = "flicker3",
		y = 0,
		visible = false,
	...
	state.flicker3Object = flicker3

	flicker3:set_alpha(1)
	flicker3:set_blend_mode("add")
	flicker3:set_texture_rect(0, 0, fw, fh)
	flicker3:set_wrap_mode("wrap")

	local whiteout = item_panel:rect({
		y = 0,
		visible = false,
		x = 0,
		color = Color.white,
	...
	state.whiteoutObject = whiteout

	whiteout:set_blend_mode("add")

	function state:resize()
		local row_item = self.row_item
		local panObject = self.bmObject
		local minu = row_item.pan_minu or 0
		local minv = row_item.pan_minv or 0
		local maxu = row_item.pan_maxu or panObject:texture_width() - 1
		local maxv = row_item.pan_maxv or panObject:texture_height() - 1
		local objW = panObject:w()
		local objH = panObject:h()
		local uvW = maxu - minu + 1
		local uvH = maxv - minv + 1
		local adjust = row_item.pan_unselected_adjust or 0.5
		local scale = nil

		if objW * uvH > objH * uvW then
			scale = objW / uvW
			self.pan_u = 0.5
			self.pan_v = adjust
		else
		...
		self.base_scale = 0.5 / (scale + 0.001)
		self.pan_minu = minu
		self.pan_minv = minv
		self.default_panU = self.pan_u
		self.default_panV = self.pan_v
		self.pan_uvw = uvW
		self.pan_uvh = uvH
		self.objW = objW
		self.objH = objH
		local combinedScale = self.base_scale / self.zoom_max
		local halfW = self.objW * combinedScale
		local halfH = self.objH * combinedScale
		self.distScaleX = self.pan_uvw - 2 * halfW
		self.distScaleY = self.pan_uvh - 2 * halfH

		self:set_zoom(self.zoom)
		self:apply()

		local pow = panObject:w()
		local poh = panObject:h()
		local pocx = panObject:center_x()
		local pocy = panObject:center_y()
		local shadow = self.shadow

		if shadow ~= nil then
			shadow:set_w(pow + 16)
			shadow:set_h(poh + 16)
			shadow:set_center_x(pocx)
			shadow:set_center_y(pocy)
		end

		local bluepane = self.bluepaneObject

		if bluepane ~= nil then
			bluepane:set_w(pow)
			bluepane:set_h(poh)
			bluepane:set_center_x(pocx)
			bluepane:set_center_y(pocy)
		end

		local flicker2 = self.flicker2Object

		if flicker2 ~= nil then
			flicker2:set_w(pow)
			flicker2:set_h(poh)
			flicker2:set_center_x(pocx)
			flicker2:set_center_y(pocy)
		end

		local flicker3 = self.flicker3Object

		if flicker3 ~= nil then
			flicker3:set_w(pow)
			flicker3:set_h(poh)
			flicker3:set_center_x(pocx)
			flicker3:set_center_y(pocy)
		end

		local whiteout = self.whiteoutObject

		if whiteout ~= nil then
			whiteout:set_w(pow)
			whiteout:set_h(poh)
			whiteout:set_center_x(pocx)
			whiteout:set_center_y(pocy)
		end

		local oldHighlight = self.highlighted
		self.highlighted = 99

		self:set_highlight(oldHighlight)
	end

	function state:kill()
		local panObject = self.bmObject

		if panObject ~= nil then
			panObject:stop()
		...
		if self.shadow ~= nil then
			self.item_panel:remove(self.shadow)
		...
		if bluepane ~= nil then
			bluepane:stop()
			self.item_panel:remove(bluepane)

			self.bluepaneObject = nil
		end
		...
		if flicker2 ~= nil then
			flicker2:stop()
			self.item_panel:remove(flicker2)

			self.flicker2Object = nil
		end
		...
		if flicker3 ~= nil then
			flicker3:stop()
			self.item_panel:remove(flicker3)

			self.flicker3Object = nil
		end

		if self.whiteoutObject ~= nil then
			self.whiteoutObject:stop()
			self.item_panel:remove(self.whiteoutObject)

			self.whiteoutObject = nil
		end
	...
	function state:set_zoom(zoom)
		local combinedScale = self.base_scale / zoom
		local halfW = self.objW * combinedScale
		local halfH = self.objH * combinedScale
		self.curange = self.pan_uvw - 2 * halfW
		self.cvrange = self.pan_uvh - 2 * halfH
		self.cubase = self.pan_minu + halfW
		self.cvbase = self.pan_minv + halfH
		self.halfW = halfW
		self.halfH = halfH
		self.zoom = zoom
	end
	...
	function state:apply()
		local cu = self.cubase + self.pan_u * self.curange
		local cv = self.cvbase + self.pan_v * self.cvrange
		local halfW = self.halfW
		local halfH = self.halfH

		self.bmObject:set_texture_rect(cu - halfW, cv - halfH, halfW * 2, halfH * 2)
	end

	function state:set_highlight(newState)
		if self.highlighted ~= newState then
			self.highlighted = newState
			local panObject = self.bmObject
			...
			if newState then
				if not self.animating then
					...
					panObject:animate(self.pan_animation1, self)
				end

				if not self.flickering and self.bluepaneObject ~= nil then
					self.flickering = true

					self.bluepaneObject:animate(self.flicker_animation)
				end

				if self.whiteoutObject ~= nil then
					self.whiteoutObject:stop()
					self.whiteoutObject:animate(self.whiteout_animation)
				end
			else
				self.pan_u = self.default_panU
				self.pan_v = self.default_panV
			...
			if shadow ~= nil then
				shadow:set_visible(newState)
			end

			if self.bluepaneObject ~= nil then
				self.bluepaneObject:set_visible(newState)
			end
		end
	end

	function state.pan_animation1(o, self)
		local phase = -0.7855
		local timeU = 50
		local omegaU = 6.284 / timeU
		local dt = 0
		...
		while true do
			...
			if self.highlighted then
				newZoom = oldZoom + (self.zoom_max - 1) / 0.05 * dt

				if self.zoom_max < newZoom then
			...
			phase = phase + omegaU * dt

			if phase >= 6.284 then
				phase = phase - 6.284
			end

			self.pan_u = 0.5 + 0.5 * math.sin(math.deg(phase))
			self.pan_v = 0.5 + 0.5 * math.sin(math.deg(phase * 2))
	...
	function state.flicker_animation()
		local self = state
		local flarr = {
			{
				setter = function (self, newState)
					local bluepane = self.bluepaneObject
					local fh = bluepane:texture_height()
					local fw = bluepane:texture_width()

					if newState then
						bluepane:set_texture_rect(0, fh / 3, fw, -fh / 3)
					else
						bluepane:set_texture_rect(0, fh, fw, -fh)
					end
				end
			...
			{
				setter = function (self, newState)
					self.flicker2Object:set_visible(newState)

					if newState then
						self.flicker2Object:set_texture_rect(8, -4 + math.random() * 12, 3, 16)
					end
				end
			},
			{
				tick = function (self, ticked)
					if ticked then
						local obj = self.flicker3Object
						local fh = obj:texture_height()
						local fw = obj:texture_width()
						local offset = math.random()
						local tl = Vector3(0, (offset - 2.73) * fh, 0)
						local tr = Vector3(fw, (offset - 2.45) * fh, 0)
						local bl = Vector3(0, (offset + 1.64) * fh, 0)
						local br = Vector3(fw, (offset + 1.91) * fh, 0)

						obj:set_texture_coordinates(tl, tr, bl, br)
					end
				end,
				setter = function (self, newState)
					self.flicker3Object:set_visible(newState)
				end
			}
		}
		local numFuncs = 3
		local numActive = 0
		local pulseLengthMap = {
			0.03333333333333333,
			0.03333333333333333,
			0.03333333333333333,
			0.06666666666666667,
		...
		local pauseTime = math.random() * 0.25

		while self.highlighted do
			if pauseTime > 0 then
				pauseTime = pauseTime - coroutine.yield()
			else
				pauseTime = 0.5 + math.random() * 5
				local maxActive = math.random(4)
				local preload = 2
				local accum = 0
				...
				repeat
					...
					while accum >= 0.03333333333333333 do
						accum = accum - 0.03333333333333333
						preload = preload + 1
						tick = true
					end

					if preload == 0 and maxActive > 0 and (numActive == 0 or math.random(1, 3) == 1) then
						preload = 1
					end

					while preload > 0 and maxActive > 0 and numActive < 2 and numActive < numFuncs do
						preload = preload - 1
						maxActive = maxActive - 1
						numActive = numActive + 1
						local newThing = math.random(numActive, numFuncs)
						local temp = flarr[newThing]
						flarr[newThing] = flarr[numActive]
						flarr[numActive] = temp

						temp.setter(self, true)

						temp.time = pulseLengthMap[math.random(8)]

						if temp.tick ~= nil then
							temp.tick(self, true)
						end
					end
					...
					while numActive >= i do
						...
						if tt > 0 then
							...
							gg.setter(self, false)

							flarr[i] = flarr[numActive]
							flarr[numActive] = gg
							numActive = numActive - 1
						end
					end
				until numActive > 0 and not self.highlighted
			end
		end

		if numActive > 0 then
			for i = 1, numActive do
				flarr[i].setter(self, false)
			end
		end
	...
	function state.whiteout_animation()
		local self = state
		local whiteout = self.whiteoutObject
		local alpha = 1

		whiteout:set_visible(true)

		while alpha > 0 do
			whiteout:set_alpha(alpha * 0.3)

			local dt = coroutine.yield()
			alpha = alpha - dt * 7.5
		end

		whiteout:set_visible(false)
	end

	state:resize()
	panObject:set_alpha(0.5)
end
...
function MenuNodeGui:_create_menu_item(row_item)
	...
	elseif not row_item.item:setup_gui(self, row_item) then
		...
		if SystemInfo:platform() == Idstring("NX64") and row_item.is_options_button then
			...
			row_item.gui_text = self:_text_item_part(row_item, self.item_panel, self:_right_align() + (row_item.is_expanded and 20 or 0))

			if row_item.is_gage_button then
				row_item.gui_text:set_font_size(30)
			end

			if row_item.font_size then
				row_item.gui_text:set_font_size(row_item.font_size)
			end
		elseif SystemInfo:platform() == Idstring("NX64") and row_item.is_preplanning_button then
			row_item.gui_panel = self:_text_item_part(row_item, self.item_panel, self:_left_align() + 10)
		else
		...
		if row_item.item:parameters().icon then
			if row_item.item:parameters().texture_rect then
				row_item.icon = self.item_panel:bitmap({
					texture = row_item.item:parameters().icon,
					texture_rect = row_item.item:parameters().texture_rect,
	...
	if SystemInfo:platform() == Idstring("NX64") then
		row_item.menu_unselected = self.item_panel:bitmap({
			y = 0,
			x = 0,
			visible = row_item.is_large_menu_button or row_item.is_short_mission_button or row_item.is_preplanning_button or row_item.show_menu_selected or row_item.switch_reticle or false,
			texture = row_item.unselected_texture or "guis/textures/switch/large_unselected_hud",
			texture_rect = {
				row_item.texture_x,
				row_item.texture_y,
				row_item.texture_scale_w,
				row_item.texture_scale_h
			},
			layer = self.layers.items - 2
		})
		...
		row_item.menu_selected = self.item_panel:bitmap({
			y = 0,
			visible = false,
			x = 0,
			texture = row_item.selected_texture or "guis/textures/switch/large_selected_hud",
			texture_rect = {
				row_item.texture_x,
				row_item.texture_y,
				row_item.texture_scale_w,
		...
		row_item.menu_selected:set_color(row_item.item:parameters().is_expanded and Color(0.5, 0.5, 0.5) or Color.white)

		if row_item.pan_texture ~= nil and (row_item.is_large_menu_button or row_item.is_short_mission_button) then
			row_item.pan_background = self.item_panel:bitmap({
				y = 0,
				visible = true,
				x = 0,
				texture = row_item.pan_texture or "guis/textures/switch/large_unselected_hud",
				layer = self.layers.items - 1
			})

			MenuNodeGui.init_pan(row_item, row_item.pan_background, self.item_panel)
		end

		if row_item.show_corner_deco then
			if row_item.menu_unselected then
				MenuNodeGui.add_corner_deco(row_item, self.item_panel, self.layers.items + 1, row_item.menu_unselected)
			else
				MenuNodeGui.add_corner_deco(row_item, self.item_panel, self.layers.items + 1, row_item.gui_panel)
			end
		end
	else
		row_item.menu_unselected = self.item_panel:bitmap({
...
function MenuNodeGui:_create_info_panel(row_item)
	...
	if SystemInfo:platform() == Idstring("NX64") then
		local y_offset = shape.y or safe_rect.y + CoreMenuRenderer.Renderer.border_height

		row_item.gui_info_panel:set_y(y_offset)
	end
...
function MenuNodeGui:_align_friend(row_item)
	...
	if SystemInfo:platform() == Idstring("NX64") then
		row_item.friend_name:set_font_size(tweak_data.menu.options_font_size)
	end
	...
	if SystemInfo:platform() == Idstring("NX64") then
		row_item.friend_name:set_right(row_item.friend_name:parent():w() - row_item.friend_name:font_size() / 2)
	else
	...
	if SystemInfo:platform() == Idstring("NX64") then
		local padding = tweak_data.menu.main_menu_padding
		local padding_on_row = (row_item.items_on_row and row_item.items_on_row - 1 or 0) * padding

		if row_item.is_large_menu_button then
			row_item.gui_panel:set_height(tweak_data.menu.main_menu_button_height)
			row_item.gui_panel:set_w((tweak_data.menu.main_menu_width - padding_on_row) / (row_item.items_on_row or 1))

			if row_item.is_options_button then
				row_item.gui_panel:set_height(tweak_data.menu.options_button_height)
				row_item.friend_name:set_height(tweak_data.menu.options_button_height)
				row_item.friend_name:set_w((tweak_data.menu.main_menu_width - padding_on_row) / (row_item.items_on_row or 1))
			end

			local adjustPanel = row_item.items_on_row and (row_item.items_on_row - row_item.item_number - 1) * padding or 0

			if row_item.items_on_row then
				adjustPanel = adjustPanel + row_item.gui_panel:w() * (row_item.items_on_row - row_item.item_number - 1)
			end

			row_item.gui_panel:set_right(self._item_panel_parent:w() - adjustPanel)
		else
			row_item.gui_panel:set_height(h)
	...
	if SystemInfo:platform() == Idstring("NX64") then
		row_item.signin_status:set_font_size(tweak_data.menu.options_font_size)
		row_item.signin_status:set_height(tweak_data.menu.options_button_height)
		row_item.signin_status:set_center_y(tweak_data.menu.options_button_height / 2)
		row_item.signin_status:set_x(tweak_data.menu.options_font_size / 2)
...
function MenuNodeGui:_text_item_part(row_item, panel, align_x, text_align)
	local padding = row_item.row_padding or 0
...
function MenuNodeGui:_highlight_row_item(row_item, mouse_over)
	if row_item then
		...
		if SystemInfo:platform() == Idstring("NX64") and (row_item.is_large_menu_button or row_item.is_short_mission_button or row_item.is_preplanning_button or row_item.show_menu_selected) then
			if row_item.corner_deco ~= nil then
				row_item.corner_deco:set_highlight(true)
			end

			if row_item.pan_state ~= nil then
				row_item.pan_state:set_highlight(true)
			else
				if row_item.menu_selected then
					row_item.menu_selected:set_visible(true)
				end
...
function MenuNodeGui:_align_marker(row_item)
	if not row_item.is_large_menu_button and not row_item.is_short_mission_button and not row_item.is_preplanning_button or row_item.offshore_payday then
		if row_item.item.hide_highlight or row_item.hide_highlight then
		...
		if row_item.offshore_payday then
			self._marker_data.marker:set_width(400)
			self._marker_data.marker:set_world_left(row_item.gui_panel:world_left())
		...
		if row_item.item:parameters().back then
			...
			if not row_item.is_large_menu_button and not row_item.is_short_mission_button then
				self._marker_data.marker:set_visible(true)
			end
		...
		if not row_item.is_large_menu_button and not row_item.is_short_mission_button and not row_item.offshore_payday then
...
function MenuNodeGui:_fade_row_item(row_item)
	if row_item then
		...
		if SystemInfo:platform() == Idstring("NX64") and (row_item.is_large_menu_button or row_item.is_short_mission_button or row_item.is_preplanning_button or row_item.show_menu_selected) then
			if row_item.corner_deco ~= nil then
				row_item.corner_deco:set_highlight(false)
			end

			if row_item.pan_state ~= nil then
				row_item.pan_state:set_highlight(false)
			else
				if row_item.menu_selected then
				...
				if row_item.menu_unselected then
					row_item.menu_unselected:set_visible(true)
				end
			end
		end
...
function MenuNodeGui:_align_normal(row_item)
	...
	if SystemInfo:platform() == Idstring("NX64") and row_item.is_options_button or row_item.is_crimenet_contacts then
		row_item.gui_text:set_font_size(tweak_data.menu.options_font_size)
	elseif row_item.is_contract_broker then
		row_item.gui_panel:set_font_size(tweak_data.menu.options_font_size)
	else
	...
	if SystemInfo:platform() == Idstring("NX64") and row_item.is_options_button then
		x, y, w, h = row_item.gui_text:text_rect()

		row_item.gui_text:set_size(w, h)
	else
		x, y, w, h = row_item.gui_panel:text_rect()
	end

	h = h + (row_item.row_padding or 0) * 2
	...
	if SystemInfo:platform() == Idstring("NX64") then
		...
		if row_item.is_large_menu_button then
			...
			if row_item.is_crimenet_contacts then
				row_item.gui_panel:set_height(tweak_data.menu.crimenet_contract_buttons_height)
				row_item.gui_text:set_height(tweak_data.menu.options_button_height)
				row_item.gui_panel:set_w((safe_rect.width / 3 - padding_on_row) / (row_item.items_on_row or 1))
				row_item.gui_text:set_w((safe_rect.width / 3 - padding_on_row) / (row_item.items_on_row or 1))
			elseif row_item.is_options_button then
				row_item.gui_panel:set_height(tweak_data.menu.options_button_height)
				row_item.gui_text:set_w((tweak_data.menu.main_menu_width - padding_on_row) / (row_item.items_on_row or 1))
			elseif row_item.is_contract_broker then
				row_item.gui_panel:set_height(tweak_data.menu.options_button_height)
			elseif row_item.is_payday_selection then
				row_item.gui_panel:set_height(tweak_data.menu.main_crimenet_menu_button_height)
			elseif row_item.is_play_menu_button then
				row_item.gui_panel:set_height(tweak_data.menu.main_play_menu_button_height)
			elseif row_item.switch_reticle then
				row_item.gui_panel:set_height(tweak_data.menu.switch_reticle_button_height)
			end
			...
			if row_item.is_options_button and not row_item.crimenet_contract_gui and not row_item.is_contract_broker then
				row_item.gui_text:set_x(row_item.gui_panel:x() + row_item.font_size / 2)
			end
		elseif row_item.is_short_mission_button then
			row_item.gui_panel:set_height(row_item.font_size * 3)
			row_item.gui_panel:set_w(170)

			local adjustPanel = 0
			local padding = 6

			if row_item.items_on_row then
				adjustPanel = row_item.gui_panel:w() * (row_item.item_number + 1) + padding * (row_item.item_number + 1)
			end

			row_item.gui_panel:set_right(x + adjustPanel)
		else
			row_item.gui_panel:set_height(h)
			row_item.gui_panel:set_w(safe_rect.width - row_item.gui_panel:left())
			row_item.gui_panel:set_left(self:_right_align() + (row_item.item:parameters().expand_value or 0))
	...
	if SystemInfo:platform() == Idstring("NX64") then
		if row_item.gui_text then
			row_item.gui_text:set_world_center_y(row_item.gui_panel:world_center_y())
		end

		if row_item.is_kick_menu then
			row_item.gui_text:set_right(row_item.gui_panel:x() + row_item.gui_panel:w() - row_item.gui_text:font_size() / 2)
		end
	end
...
function MenuNodeGui:resolution_changed()
	...
	if SystemInfo:platform() == Idstring("NX64") then
		self._legends_panel:set_shape(safe_rect.x + self.legend_x_offset, safe_rect.y + self.legend_y_offset, safe_rect.width, safe_rect.height + 5)
		self:_layout_legends_nx()
	else
...
function MenuNodeGui:_layout_legends_nx()
	local safe_rect = self:_scaled_size()
	local res = RenderSettings.resolution
	local w = 0
	local h = 0
	local num_legends = self._legends_panel:num_children()

	for i = 0, num_legends - 1 do
		local panel = self._legends_panel:child(num_legends - 1 - i)
	...
	self._legends_panel:set_w(w)
	self._legends_panel:set_h(h + 5)
	self._legends_panel:set_right(self.ws:panel():w() + self.legend_x_offset)
	self._legends_panel:set_bottom(self.ws:panel():bottom() + self.legend_y_offset)
	...
	for i = 0, num_legends - 1 do
		...
		panel:set_center_y(self._legends_panel:h() / 2)
		panel:set_right(self._legends_panel:w() - w)
...
function MenuNodeGui:_add_version_string()
	...
	if Application:debug_enabled() or SystemInfo:platform() == Idstring("WIN32") or SystemInfo:platform() == Idstring("NX64") then
		local version = Application:version()

		if SystemInfo:platform() == Idstring("NX64") then
			version = SystemInfo:build_version()
		end
