...
function CrimeNetContractGui:init(ws, fullscreen_ws, node)
	...
	if SystemInfo:platform() == Idstring("NX64") then
		...
		self._contact_text_header:set_valign("center")
		self._contact_text_header:set_size(width, h + 6)
		self._contact_text_header:move(0, 1)
	end
	...
	local contact_text, contact_panel = nil
	contact_text = self._contract_panel:text({
	...
	if SystemInfo:platform() == Idstring("NX64") then
		...
		contact_text:set_x(text_w + 20)
		contact_text:set_y(10)

		contact_panel = self._contract_panel:panel({
			w = contact_w,
			h = contact_h
		})

		contact_panel:set_top(contact_text:bottom())
		contact_panel:set_x(contact_text:x())
	else
		contact_panel = self._contract_panel:panel({
			y = 10,
			w = contact_w,
	...
	if self._briefing_event then
		...
		if SystemInfo:platform() == Idstring("NX64") then
			...
			self.button_text_panel = self._briefing_len_panel:panel()
			self.button_text = self._briefing_len_panel:text({
				visible = true,
				name = "button_text",
				blend_mode = "normal",
				text = " ",
			...
			self.button_text_panel:set_w(self.button_text:w())
			self.button_text_panel:set_h(self.button_text:h())
			self.button_text_panel:set_center_y(self.button_text:center_y())
			self.button_text_panel:set_center_x(self.button_text:center_x())

			button_text = self.button_text
			local _, _, _, h = self.button_text:text_rect()
		else
			self._briefing_len_panel:rect({
				blend_mode = "add",
				name = "duration",
			...
			button_text = self._briefing_len_panel:text({
				blend_mode = "add",
				name = "button_text",
				text = " ",
				y = 10,
		...
		if SystemInfo:platform() == Idstring("NX64") then
			self._briefing_len_panel:set_h(h + 20)
			self._briefing_len_panel:set_position(contact_panel:left(), contact_panel:bottom())
		else
	...
	if self._customizable then
		...
		if not is_win_32 and not is_nextgen then
			...
			if SystemInfo:platform() == Idstring("NX64") then
				contact_text:set_bottom(contact_panel:top())
			else
		...
		if SystemInfo:platform() == Idstring("NX64") then
			premium_text:set_top(contact_panel:bottom() + 3)
		else
...
function CrimeNetContractGui:sound_event_callback(event_type, duration)
	if event_type == "end_of_event" then
		...
		if SystemInfo:platform() == Idstring("NX64") then
			self._briefing_len_panel:child("button_text"):set_right(self._briefing_len_panel:w() - 10)
			self._briefing_len_panel:child("button_text"):set_center_y(self._briefing_len_panel:h() / 2)
			self._briefing_len_panel:child("text"):set_center_y(self._briefing_len_panel:h() / 2)
			self.button_text_panel:set_w(self.button_text:w())
			self.button_text_panel:set_h(self.button_text:h())
			self.button_text_panel:set_center_y(self.button_text:center_y())
			self.button_text_panel:set_center_x(self.button_text:center_x())
...
function CrimeNetContractGui:mouse_pressed(o, button, x, y)
	if alive(self._briefing_len_panel) and self._briefing_len_panel:visible() and self._step > 2 and (SystemInfo:platform() == Idstring("NX64") and self._briefing_len_panel:inside(x, y) or self._briefing_len_panel:child("button_text"):inside(x, y)) then
		self:toggle_post_event()
	end
...
function CrimeNetContractGui:special_btn_pressed(button)
	if button == Idstring("voice_message") or self._smart_matchmaking and button == Idstring("menu_toggle_voice_message") or SystemInfo:platform() == Idstring("NX64") and button == Idstring("menu_toggle_voice_message") then
		if alive(self._briefing_len_panel) and self._briefing_len_panel:visible() and self._step > 2 then
			self:toggle_post_event()
		end
