...
function PrePlanningMapGui:update(t, dt)
	...
	if not self._is_pc_controller and (not managers.system_menu or not managers.system_menu:is_active() or not not managers.system_menu:is_closing()) then
		...
		if SystemInfo:platform() == Idstring("NX64") then
			if self.nx_zoom_out and self.nx_zoom_out.active then
				self:zoom_out(self.nx_zoom_out.x_loc, self.nx_zoom_out.y_loc)
			elseif self.nx_zoom_in and self.nx_zoom_in.active then
				self:zoom_in(self.nx_zoom_in.x_loc, self.nx_zoom_in.y_loc)
			end

			if self.nx_pan then
				local speed = 20

				self:_move_map_position(self.nx_pan.x * speed, -self.nx_pan.y * speed)
			end
		end
	end
...
function PrePlanningMapGui:mouse_pressed(button, x, y)
	...
	if button == Idstring("0") and self._panel:inside(x, y) and SystemInfo:platform() ~= Idstring("NX64") then
		self._released_map = nil
		self._grabbed_map = {
			x = x,
			y = y,
	...
	if SystemInfo:platform() == Idstring("NX64") then
		...
		if button == Idstring("pan_left") then
			self.nx_pan.x = -1
		elseif button == Idstring("pan_right") then
			self.nx_pan.x = 1
		end
