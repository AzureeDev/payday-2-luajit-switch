...
function MenuNodeCrimenetGui:_setup_item_panel(safe_rect, res)
	...
	local width = 900
	local height = 580
	local is_nextgen = SystemInfo:platform() == Idstring("PS4") or SystemInfo:platform() == Idstring("XB1") or SystemInfo:platform() == Idstring("NX64")
...
function MenuNodeCrimenetGui:_clamp_scroll_y(new_panel_y)
	return new_panel_y
end

function MenuNodeCrimenetGui:_align_normal(row_item)
	if SystemInfo:platform() ~= Idstring("NX64") then
		MenuNodeCrimenetGui.super._align_normal(self, row_item)
	else
		...
		if row_item.is_large_menu_button then
			row_item.gui_panel:set_height(row_item.font_size * 2.5)

			if row_item.is_options_button then
				if not row_item.crimenet_contract_gui then
					row_item.gui_panel:set_height(row_item.gui_text:font_size() * 2)
					row_item.gui_text:set_height(row_item.gui_text:font_size() * 2)
					row_item.gui_panel:set_w(safe_rect.width / 3 / (row_item.items_on_row or 1))
					row_item.gui_text:set_w(safe_rect.width / 3 / (row_item.items_on_row or 1))
				else
					local width = 900
					local height = 580
					...
					row_item.gui_text:set_height(tweak_data.menu.crimenet_contract_buttons_height)
					...
					row_item.gui_text:set_x(row_item.gui_text:x() + row_item.gui_text:font_size())
				end
			else
				row_item.gui_panel:set_w(safe_rect.width / 3 / (row_item.items_on_row or 1))
			end
		...
		if row_item.icon then
			...
			row_item.icon:set_top(row_item.gui_panel:top())
...
function MenuNodeCrimenetFiltersGui:_setup_item_panel(safe_rect, res)
	...
	if alive(self.box_panel) then
		if SystemInfo:platform() == Idstring("NX64") then
			self.item_panel:parent():parent():remove(self.box_panel)
		else
	...
	if SystemInfo:platform() == Idstring("NX64") then
		self.box_panel = self.item_panel:parent():parent():panel()
	else
	...
	if SystemInfo:platform() == Idstring("NX64") then
		for _, row_item in pairs(self.row_items) do
			row_item.gui_panel:set_w(self.box_panel:w() * 0.95)
			row_item.gui_panel:set_x(0)
			...
			if row_item.gui_icon then
				row_item.gui_icon:set_right(self.box_panel:w() - row_item.gui_icon:w())
			end
		end
		...
		if self.scroll_bar_bg then
			if self._item_panel_parent:h() < self.item_panel:h() then
				...
				self.scroll_bar_bg:set_y(0)
				self.scroll_bar_bg:set_right(self.box_panel:right() + 10)
				self.scroll_bar:set_y(0)
				self.scroll_bar:set_right(self.box_panel:right() + 10)
				self.box_panel:grow(self.scroll_bar_bg:w() + 5, 0)
			else
				self.scroll_bar_bg:set_visible(false)
				self.scroll_bar:set_visible(false)
			end
		end

		self.item_panel:set_y(0)
		self._item_panel_parent:set_y(self.box_panel:y() + 10)
	end
...
function MenuNodeCrimenetFiltersGui:reload_item(item)
	...
	if row_item and not row_item.add_unique_panel and alive(row_item.gui_panel) then
...
function MenuNodeCrimenetSpecialGui:_setup_item_panel(safe_rect, res)
	...
	if SystemInfo:platform() == Idstring("NX64") then
		self._contact_filter = self._item_panel_parent:parent():child("contact_filter")
		self._title_divider = self._item_panel_parent:parent():child("divider_title")

		self._item_panel_parent:set_h(350)
		self._item_panel_parent:set_y(130)
		self.item_panel:set_top(0)
		self._item_panel_parent:parent():remove(self.box_panel)

		self.box_panel = nil
		self.box_panel = self._item_panel_parent:parent():panel({
			h = 375,
			y = 100,
			layer = 51,
			x = self.item_panel:x() - 10,
			w = self.item_panel:w() + 20
		})
		local final_ipp_h = self.box_panel:h() - self._contact_filter:h() - self._title_divider:h() - 25
		local final_ipp_y = self.box_panel:y() + self._title_divider:h() + self._contact_filter:h() + 10

		if self.scroll_bar_bg and final_ipp_h < self.item_panel:h() - self._contact_filter:h() - self._title_divider:h() - 10 then
			self.scroll_bar_bg:set_h(final_ipp_h)
			self.scroll_bar:set_h(final_ipp_h / self.item_panel:h() * final_ipp_h)
			self.scroll_bar_bg:set_visible(true)
			self.scroll_bar:set_visible(true)
			self.scroll_bar_bg:set_y(0)
			self.scroll_bar_bg:set_right(self.box_panel:right() - 7)
			self.scroll_bar:set_y(0)
			self.scroll_bar:set_right(self.scroll_bar_bg:right())
			self.item_panel:grow(-(self.scroll_bar_bg:w() + 5), 0)

			for _, row_item in pairs(self.row_items) do
				if row_item.type ~= "divider" and not row_item.add_unique_panel then
					row_item.gui_panel:set_w(row_item.gui_panel:w() - 10)
					row_item.menu_unselected:set_world_x(self.item_panel:x())
					row_item.menu_selected:set_world_x(self.item_panel:x())
					row_item.gui_panel:set_world_x(self.item_panel:x())
				end
			end
		else
			self.scroll_bar_bg:set_visible(false)
		...
		self._align_data.panel:set_left(self.box_panel:left())
		self._item_panel_parent:set_h(final_ipp_h)
		self._item_panel_parent:set_top(final_ipp_y)
		self.item_panel:set_world_top(self._item_panel_parent:world_top())
		self.item_panel:set_left(self.item_panel:left() + 2)
		self.item_panel:set_h(self:_item_panel_height())

		if alive(self.item_panel:parent():parent():child("special_title_text")) then
			self.item_panel:parent():parent():remove(self.item_panel:parent():parent():child("special_title_text"))
		end

		title_text = self.item_panel:parent():parent():text({
		...
		title_text = self.item_panel:parent():text({
			name = "special_title_text",
			blend_mode = "add",
			layer = 51,
			text = managers.localization:to_upper_text(self.title_id or "menu_cn_contract_broker_title"),
	...
	if self._title_divider then
		self._title_divider:set_world_left(self.item_panel:world_left() + 5)
	end
...
function MenuNodeCrimenetSpecialGui:touch_scroll(dy)
	if SystemInfo:platform() == Idstring("NX64") then
		MenuNodeCrimenetSpecialGui.super.touch_scroll(self, dy, -80)
	else
		MenuNodeCrimenetSpecialGui.super.touch_scroll(self, dy)
	end
end

function MenuNodeCrimenetSpecialGui:move_scrollbar_thumb(new_y)
	if self.scroll_bar and self.scroll_bar_bg and SystemInfo:platform() == Idstring("NX64") then
		local limit = -1 * (self.item_panel:h() - self._item_panel_parent:h()) + 75
		...
		self.item_panel:set_y((new_y - self.scroll_bar_bg:y()) * limit / gap_size - 75)
		self.scroll_bar:set_y(new_y)
	end
end

function MenuNodeCrimenetSpecialGui:_clamp_scroll_y(new_panel_y)
	if SystemInfo:platform() == Idstring("NX64") then
		if self.item_panel and self._item_panel_parent then
			...
			if top_offset < new_panel_y then
				new_panel_y = top_offset
			end
		...
		MenuNodeCrimenetSpecialGui.super._clamp_scroll_y(self, new_panel_y)
	end
end

function MenuNodeCrimenetSpecialGui:refresh_scrollbar()
	if SystemInfo:platform() == Idstring("NX64") and self._contact_filter then
		if self.scroll_bar and self.scroll_bar_bg and self.item_panel then
			local limit = -(self.item_panel:h() - 75 - self._item_panel_parent:h())
			local scroll_bar_top = self.scroll_bar_bg:y()
			local item_panel_y = self.item_panel:y() + 75
			local bg_h = self.scroll_bar_bg:h()
			local sb_h = self.scroll_bar:h()
			local scroll_height = bg_h - sb_h

			self.scroll_bar:set_y(scroll_bar_top + item_panel_y / limit * scroll_height)
		end
	else
		MenuNodeCrimenetSpecialGui.super.refresh_scrollbar(self)
	end
end

function MenuNodeCrimenetSpecialGui:reactivate()
	local active_menu = managers.menu:active_menu()
...
function MenuNodeCrimenetSpecialGui:reload_item(item)
	if SystemInfo:platform() == Idstring("NX64") then
		MenuNodeCrimenetFiltersGui.super.reload_item(self, item)
	else
		MenuNodeCrimenetSpecialGui.super.reload_item(self, item)
	end
end
...
function MenuNodeCrimenetCasinoGui:_setup_item_panel(safe_rect, res)
	...
	if SystemInfo:platform() == Idstring("NX64") then
		self.item_panel:set_bottom(self.item_panel:parent():h() - tweak_data.menu.pd2_large_font_size - 35)
		self.item_panel:set_right(start_x + width + 125)
	else
...
function MenuNodeCrimenetCasinoGui:_setup_layout()
	...
	local betting_width = width

	if SystemInfo:platform() == Idstring("NX64") then
		betting_width = betting_width + 190
	end
	...
	if SystemInfo:platform() == Idstring("NX64") then
		text_options:set_y(self.item_panel:y() + 25)
	else
	...
	local betting_title_font_size = small_font_size
	local betting_title_font = small_font
	local betting_align = "right"

	if SystemInfo:platform() == Idstring("NX64") then
		betting_title_font_size = 30
		betting_title_font = medium_font
		betting_align = "left"
	end
		...
		if item.type ~= "divider" then
			if not betting_titles[i].skip then
				self._betting_titles[betting_titles[i].id] = text_options:text({
					...
					y = y,
					align = betting_align,
					font_size = betting_title_font_size,
					font = betting_title_font,
					color = tweak_data.screen_colors.text
				})
			end
	...
	if SystemInfo:platform() == Idstring("NX64") then
		self._betting_cards_panel:set_size(self._betting_panel:w() - content_offset, (self.item_panel:y() - content_offset * 2) * 0.6 * 1.5)
		self._betting_cards_panel:set_y(content_offset - 15)
		self._betting_cards_panel:rect({
			alpha = 0.3,
			color = Color(0, 0, 0)
		})
	else
	...
	if SystemInfo:platform() == Idstring("NX64") then
		self._stats_panel:set_x(self._betting_panel:right() + 15)
	else
		self._stats_panel:set_x(self._betting_panel:right() + space_x)
	end

	if SystemInfo:platform() == Idstring("NX64") then
		self._stats_panel:rect({
			alpha = 0.3,
			color = Color(0, 0, 0)
		})
	end
	...
	breakdown_font_size = small_font_size
	local breakdown_y = content_offset

	if SystemInfo:platform() == Idstring("NX64") then
		breakdown_font_size = 30
		breakdown_y = content_offset - 10
	end
	...
	if SystemInfo:platform() == Idstring("NX64") then
		self._breakdown_costs:set_x(self._breakdown_panel:w() * 0.6)
	end

	local offshore_amount_font_size = small_font_size
	local offshore_amount_font = small_font

	if SystemInfo:platform() == Idstring("NX64") then
		offshore_amount_font_size = 30
		offshore_amount_font = medium_font
	end

	self._offshore_text = self._main_panel:text({
		blend_mode = "add",
		font_size = offshore_amount_font_size,
		font = offshore_amount_font,
		color = tweak_data.screen_colors.text
	})
...
if SystemInfo:platform() == Idstring("NX64") then
	MenuNodeCrimenetContactInfoGui.MENU_WIDTH = 280
	MenuNodeCrimenetContactInfoGui.HEIGHT = 450
else
...
function MenuNodeCrimenetContactInfoGui:_setup_item_panel_parent(safe_rect, shape)
	...
	if SystemInfo:platform() == Idstring("NX64") then
		self._item_panel_parent:set_h(shape.h + 20)
	end
end
...
function MenuNodeCrimenetContactInfoGui:set_empty()
	...
	local contact_desc_text = nil

	if SystemInfo:platform() == Idstring("NX64") then
		contact_desc_text = self._panel:child("contact_desc_text"):child("contact_desc_text_text")
	else
		contact_desc_text = self._panel:child("contact_desc_text")
	end

	if contact_desc_text then
...
function MenuNodeCrimenetContactInfoGui:_set_file()
	...
	if SystemInfo:platform() == Idstring("NX64") then
		local owning_panel = self._panel:child("contact_desc_text")

		if owning_panel:child("contact_desc_text_text") then
			local contact_desc_text = owning_panel:child("contact_desc_text_text")

			contact_desc_text:stop()
			contact_desc_text:set_y(0)
			contact_desc_text:set_text(desc_id)

			local x, y, w, h = contact_desc_text:text_rect()

			contact_desc_text:set_h(h)

			if owning_panel:h() < contact_desc_text:h() then
				contact_desc_text:animate(callback(nil, _G, "auto_scroll_text"), owning_panel)
			end
		else
			local contact_desc_text = owning_panel:text({
				text = "",
				name = "contact_desc_text_text",
				wrap = true,
				word_wrap = true,
				layer = 52,
...
function MenuNodeCrimenetContactInfoGui:mouse_moved(o, x, y)
	if SystemInfo:platform() == Idstring("NX64") then
		...
		elseif self.scrolling_bar then
			...
			return true
		elseif self.scroll_bar_bg and self.scroll_bar_bg:inside(x, y) or self.scroll_bar and self.scroll_bar:inside(x, y) then
...
function MenuNodeCrimenetContactInfoGui:mouse_pressed(button, x, y)
	if SystemInfo:platform() == Idstring("NX64") then
		...
		if button == Idstring("press") or button == Idstring("0") then
			self.swiped = false
			self.swipe_speed = 0
		end
		...
		if self._item_panel_parent:h() < self.item_panel:h() then
			if self.scroll_bar_bg and self.scroll_bar_bg:inside(adjust_x, y) or self.scroll_bar and self.scroll_bar:inside(adjust_x, y) then
...
function MenuNodeCrimenetContactInfoGui:_setup_menu()
	...
	for _, row_item in pairs(self.row_items) do
		if alive(row_item.gui_panel) then
			...
			if SystemInfo:platform() == Idstring("NX64") then
				row_item.gui_panel:set_align("left")
				row_item.gui_panel:set_left(0)

				if row_item.gui_text then
					if row_item.is_gage_button then
						row_item.gui_text:set_left(35)
						row_item.icon:set_w(2)
						row_item.icon:set_left(1)
					elseif SystemInfo:platform() ~= Idstring("NX64") and row_item.font_size ~= nil then
						row_item.gui_text:set_font_size(16)
					end
				end
				...
				if row_item.corner_deco ~= nil then
					row_item.corner_deco:set_corner_size(row_item.corner_deco.corner_size_small)
				end
			end
		end
	end
	...
	if SystemInfo:platform() == Idstring("NX64") then
		self.scroll_bar:set_left(260)
		self.scroll_bar_bg:set_left(260)
		self.item_panel:grow(-self.scroll_bar_bg:width() * 2, 0)
		self.item_panel:set_y(0)
	end
end
...
function MenuNodeCrimenetContactInfoGui:_setup_layout()
	...
	if SystemInfo:platform() == Idstring("NX64") then
		contact_desc_text = self._panel:panel({
...
if SystemInfo:platform() == Idstring("NX64") then
	MenuNodeCrimenetContactShortGui.WIDTH = 1000
	MenuNodeCrimenetContactShortGui.HEIGHT = 357
	MenuNodeCrimenetContactShortGui.MENU_WIDTH = 400
...
function MenuNodeCrimenetContactShortGui:_setup_layout()
	...
	if SystemInfo:platform() == Idstring("NX64") then
		...
		desc_panel:move(self.PADDING * 2, 300)
	else
	...
	if SystemInfo:platform() == Idstring("NX64") then
		self.desc_panel:set_bottom(reward_panel:bottom())
	end

	local reward_text = nil

	if SystemInfo:platform() == Idstring("NX64") then
		reward_text = reward_panel:text({
			name = "rewards_text",
			layer = 51,
	...
	local cash_drop_text = nil

	if SystemInfo:platform() == Idstring("NX64") then
		cash_drop_text = reward_panel:text({
			name = "cash_drop_text",
			layer = 51,
	...
	local loot_drop_text = nil

	if SystemInfo:platform() == Idstring("NX64") then
		loot_drop_text = reward_panel:text({
			name = "loot_drop_text",
			layer = 51,
...
if SystemInfo:platform() == Idstring("NX64") then
	MenuNodeCrimenetGageAssignmentGui.MENU_WIDTH = 320
else
...
function MenuNodeCrimenetGageAssignmentGui:_setup_layout()
	...
	if SystemInfo:platform() == Idstring("NX64") then
		self._item_panel_parent:set_top(70)
		self.item_panel:set_top(0)

		self.height_padding = -20
	end
end
...
function MenuNodeOpenContainerGui:update_info(button)
	...
	if button and button.params.weapon_id then
