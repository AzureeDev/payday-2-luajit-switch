...
function MenuNodeButtonLayoutGui:_setup()
	...
	if SystemInfo:platform() == Idstring("NX64") then
		if managers.controller.nx_controller_type == "joycon" then
			self._coords = tweak_data:get_joycon_coords()
		else
			self._coords = tweak_data:get_switch_pro_coords()
		end
	end
	...
	elseif managers.controller.nx_controller_type == "joycon" then
		self._controller = self.ws:panel():bitmap({
			texture = "guis/textures/switch/joycon",
			w = 512,
			h = 512,
			layer = self.layers.items
		})
		...
		self._controller = self.ws:panel():bitmap({
			texture = "guis/textures/switch/switch_pro",
			w = 512,
			h = 512,
			layer = self.layers.items
		})
