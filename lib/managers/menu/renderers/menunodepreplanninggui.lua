...
function MenuNodePrePlanningGui:init(node, layer, parameters)
	...
	if SystemInfo:platform() ~= Idstring("NX64") then
		...
		parameters.font_size = tweak_data.menu.pd2_medium_font_size
	end
...
function MenuNodePrePlanningGui:_setup_item_panel(safe_rect, res)
	...
	if SystemInfo:platform() == Idstring("NX64") then
		self.nx_marker = self.item_panel:bitmap({
			texture = "guis/textures/switch/menu_selected",
			visible = true,
			texture_rect = {
				197,
				42,
...
function MenuNodePrePlanningGui:set_nx_params(node)
	if SystemInfo:platform() == Idstring("NX64") then
		for _, item in pairs(node:items()) do
			if item._type ~= "divider" then
				item:set_parameter("is_preplanning_button", true)
				item:set_parameter("row_item_hightlight_color", tweak_data.screen_colors.button_stage_3)
				item:set_parameter("row_item_color", tweak_data.screen_colors.button_stage_3)
				item:set_parameter("selected_texture", "guis/textures/switch/menu_selected")
				item:set_parameter("unselected_texture", "guis/textures/switch/menu_unselected")
				item:set_parameter("texture_x", 198)
				item:set_parameter("texture_y", 43)
				item:set_parameter("texture_scale_w", 625)
				item:set_parameter("texture_scale_h", 172)
				item:set_parameter("row_item_color", tweak_data.screen_colors.button_stage_3)
				item:set_parameter("selection_color", Color.white)
			end
		end
	end
...
function MenuNodePrePlanningGui:_update_tooltip(item)
	...
	if row_item and alive(self._tooltip) then
		...
		if tooltip then
			...
			self._tooltip:show()
			self:_shrink_tooltip_text_to_fit()
		else
			self._tooltip:hide()
		end
	end
...
function MenuNodePrePlanningGui:_shrink_tooltip_text_to_fit()
	if SystemInfo:platform() == Idstring("NX64") then
		local description = self._tooltip:child("description")
		local error_text = self._tooltip:child("error")
		local title = self._tooltip:child("title")

		description:set_font_size(self.font_size)
		error_text:set_font_size(self.font_size)
		title:set_font_size(self.font_size)

		local _, _, w, _ = title:text_rect()

		while self._tooltip:w() < title:left() + w do
			title:set_font_size(title:font_size() - 1)
			title:set_kern(title:kern())

			_, _, w, _ = title:text_rect()

			if title:font_size() <= 0 then
				title:set_font_size(self.font_size)
		...
		self:_update_tooltip_height()

		while self._legends_panel:world_top() <= self._tooltip:world_bottom() do
			description:set_font_size(description:font_size() - 1)
			error_text:set_font_size(error_text:font_size() - 1)

			if description:font_size() <= 0 or error_text:font_size() <= 0 then
				title:set_font_size(self.font_size)
				description:set_font_size(self.font_size)
...
function MenuNodePrePlanningGui:_update_tooltip_height()
	local description = self._tooltip:child("description")
	local error_text = self._tooltip:child("error")
	local title = self._tooltip:child("title")
	local icon = self._tooltip:child("icon")
...
function MenuNodePrePlanningGui:reload_item(item)
	...
	if row_item and alive(row_item.gui_panel) then
		...
		if SystemInfo:platform() == Idstring("NX64") and row_item.is_preplanning_button then
	...
	for i, row_item in pairs(self.row_items) do
		...
		if item then
			...
			if votes and row_item.gui_panel then
				...
				local num_votes = 0
				local pixel_shift = 2
				local check_size_multiplier = 1

				if SystemInfo:platform() == Idstring("NX64") then
					pixel_shift = 26
					check_size_multiplier = 0.6
				end
				...
				for peer_id, voted in pairs(votes.players) do
					if voted then
						new_icon = self._icon_panel:bitmap({
							...
							w = row_item.gui_panel:h() * check_size_multiplier,
							h = row_item.gui_panel:h() * check_size_multiplier,
							color = tweak_data.chat_colors[peer_id],
							layer = self.layers.items
						})

						new_icon:set_world_right(prev_icon and prev_icon:world_left() - 2 or row_item.gui_panel:world_right() - pixel_shift)
						new_icon:set_world_center_y(row_item.gui_panel:world_center_y())
