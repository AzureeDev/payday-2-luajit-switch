...
if SystemInfo:platform() == Idstring("NX64") then
	TextBoxGui.PRESETS.system_menu = {
		...
		title_font_size = 48,
		h = 400,
		center_button_text = true,
		w = 640,
		forced_h = 150,
		box_type = "system_menu",
		button_gap = 10
	}
	TextBoxGui.PRESETS.system_menu2 = {
		font_size = 28,
		button_gap = 13,
		h = 700,
		nx_forced_button_h = 78,
		w = 652,
		forced_h = 190,
		box_type = "system_menu2",
		title_font_size = 50,
		button_font_size = 30,
		button_text_lights_up = true,
		center_button_text = true
	}
else
	TextBoxGui.PRESETS.system_menu = {
...
function TextBoxGui:_create_text_box(ws, title, text, content_data, config)
	...
	self.box_type = preset and preset.box_type
	...
	if config.button_gap ~= nil then
		if preset and preset.button_gap == nil then
			preset.button_gap = config.button_gap
		elseif preset == nil then
			preset = {
				button_gap = config.button_gap
			}
		end
	end
	...
	local w = content_data and content_data.forced_w or preset and preset.w or config and config.w or scaled_size.width / 2.25
	...
	self.nx_forced_button_h = content_data and content_data.nx_forced_button_h or config and config.nx_forced_button_h or preset and preset.nx_forced_button_h
	local forced_h = content_data and content_data.forced_h or preset and preset.forced_h or config and config.forced_h or false
	...
	if config.make_text_a_panel then
		text = scroll_panel:panel(textParams)
		self._text_panel = text
	else
		textParams.font_size = 24
		text = scroll_panel:text(textParams)
	end

	if use_text_formating and not config.make_text_a_panel then
	...
	self._button_minw_nx = content_data and content_data.nx_button_minw or 0
	local buttons_panel = self:_setup_buttons_panel(info_area, button_list, focus_button, only_buttons, preset)
	self._button_minw_nx = nil
	...
	if SystemInfo:platform() == Idstring("NX64") then
		self._scroll_bar_box_class = scroll_bar:rect({
			halign = "grow",
		...
		scroll_bar:set_w(14)
		self._scroll_bar_box_class:set_color(Color(0, 0.69, 1))

		local scroll_bar_bg = main:panel({
			name = "scroll_bar_bg",
			halign = "right",
			w = 14,
			layer = 3,
		...
		self._scroll_bar_bg_box_class = scroll_bar_bg:rect({
			halign = "grow",
			name = "scroll_bar_bg_box",
			valign = "grow",
			layer = 3
		...
		self._scroll_bar_bg_box_class:set_color(Color(0.5, 0.5, 0.5))
		scroll_bar_bg:set_alpha(0.5)
		scroll_bar_bg:set_bottom(scroll_down_indicator_arrow:bottom())
		scroll_bar_bg:set_center_x(scroll_down_indicator_arrow:center_x())
	else
	...
	if SystemInfo:platform() == Idstring("NX64") and self.button_list then
		for i, button in ipairs(self.button_list) do
			button.corner_deco:refresh()
		end
	end
...
function TextBoxGui:inside(x, y)
...
function TextBoxGui.add_corner_deco(item, o_panel, c_layer, bounding_obj)
	local corners = {}
	item.corner_deco = corners
...
function TextBoxGui:_setup_buttons_panel(info_area, button_list, focus_button, only_buttons, preset)
	local num_buttons = button_list and #button_list or 0
	local has_buttons = num_buttons > 0
	local is_NX = SystemInfo:platform() == Idstring("NX64")
	local button_gap = preset and preset.button_gap or 0
	local buttons_panel = info_area:panel({
		name = "buttons_panel",
		h = 0,
		x = 10,
		layer = 1,
		w = has_buttons and 200 + num_buttons * button_gap or 0
	})
	local basic_button_w = nil

	if is_NX then
		if self.box_type and self.box_type == "system_menu" then
			buttons_panel:set_w(info_area:w() * 0.65)

			basic_button_w = buttons_panel:w() / (num_buttons > 2 and num_buttons or 2) - num_buttons * button_gap

			if basic_button_w < self._button_minw_nx and num_buttons > 0 then
				basic_button_w = self._button_minw_nx

				buttons_panel:set_w(basic_button_w * num_buttons + button_gap * (num_buttons - 1))
			end

			buttons_panel:set_right(info_area:w() - button_gap)
			buttons_panel:set_h(self.nx_forced_button_h or info_area:h() / 3)
		else
			basic_button_w = info_area:w() * 0.25

			if basic_button_w < self._button_minw_nx and num_buttons > 0 then
				basic_button_w = self._button_minw_nx
			end

			buttons_panel:set_w((num_buttons > 0 and (num_buttons - 1) * button_gap or 0) + basic_button_w * num_buttons)
			buttons_panel:set_h(self.nx_forced_button_h or info_area:h() / 8)
			buttons_panel:set_right(info_area:w() - 10)
		end
	else
		buttons_panel:set_right(info_area:w())
		buttons_panel:set_h(info_area:h())

		basic_button_w = buttons_panel:w() / (num_buttons > 0 and num_buttons or 1)
	end

	self._text_box_buttons_panel = buttons_panel
	local center_button_text = preset and preset.center_button_text or false
	...
	if has_buttons then
		...
		if preset and preset.button_text_lights_up then
			self._normal_bt_color = button_text_config.color
		end

		if preset and preset.button_font_size then
			button_text_config.font_size = preset.button_font_size
		end
		...
		if button_list then
			for i, button in ipairs(button_list) do
				...
				if is_NX then
					text:set_size(basic_button_w, buttons_panel:h())
					text:set_wrap(true)
					button_panel:set_h(buttons_panel:h())
					button_panel:set_w(basic_button_w)
					text:set_center(button_panel:w() * 0.5, button_panel:h() * 0.5)
					text:set_align("center")
					text:set_halign("center")

					local fbw = button_panel:w()
					local fbh = button_panel:h()

					button_panel:set_right(buttons_panel:w() - (#button_list - i) * (button_panel:w() + button_gap))
					button_panel:set_bottom(buttons_panel:h())

					local unselected = button_panel:bitmap({
					...
					local selected = button_panel:bitmap({
					...
					TextBoxGui.add_corner_deco(button, info_area, 10, button_panel)
					unselected:set_h(fbh)
					unselected:set_w(fbw)
					unselected:set_center_y(fbh * 0.5)
					unselected:set_center_x(fbw * 0.5)
					selected:set_h(fbh)
					selected:set_w(fbw)
					selected:set_center_y(fbh * 0.5)
					selected:set_center_x(fbw * 0.5)
...
function TextBoxGui:_set_button_selected(index, is_selected)
	...
	local button = self.button_list[index]
	local is_NX = SystemInfo:platform() == Idstring("NX64")

	if button_panel and button then
		local button_text = button_panel:child("button_text")

		if is_NX then
			local unselected = button_panel:child("unselected")
			local selected = button_panel:child("selected")

			if selected and unselected then
				selected:set_visible(is_selected)
				unselected:set_visible(not is_selected)
			end

			if button.corner_deco then
				button.corner_deco:set_highlight(is_selected)
			end

			if self._normal_bt_color then
				button_text:set_color(is_selected and Color.white or self._normal_bt_color)
			end

			if is_selected and selected then
				button_text:set_color(Color.white)
			else
...
function TextBoxGui:change_focus_button(change)
	local button_count = self._text_box_buttons_panel:num_children()
	local is_NX = SystemInfo:platform() == Idstring("NX64")
	...
	if is_NX and self._text_box_focus_button ~= focus_button then
...
function TextBoxGui:_set_scroll_indicator()
	...
	local bar_h, scroll_bar_bg = nil

	if SystemInfo:platform() == Idstring("NX64") then
		scroll_bar_bg = self._text_box:child("scroll_bar_bg")
		bar_h = scroll_bar_bg:h()
	else
		bar_h = self._text_box:child("scroll_down_indicator_arrow"):top() - self._text_box:child("scroll_up_indicator_arrow"):bottom()
	end

	local is_visible = nil

	if scroll_panel:h() < scroll_text:h() then
		is_visible = true

		scroll_bar:set_h(bar_h * scroll_panel:h() / scroll_text:h())
	else
		is_visible = self.force_scroll_bar_visible or false

		scroll_bar:set_h(bar_h)
	end

	scroll_bar:set_visible(is_visible)

	local arrows_visible = is_visible

	if SystemInfo:platform() == Idstring("NX64") then
		scroll_bar_bg:set_visible(is_visible)

		arrows_visible = false
	end

	legend_scroll:set_visible(not self._no_scroll_legend and is_visible)
	self._text_box:child("scroll_up_indicator_shade"):set_visible(arrows_visible)
	self._text_box:child("scroll_up_indicator_arrow"):set_visible(arrows_visible)
	self._text_box:child("scroll_down_indicator_shade"):set_visible(arrows_visible)
	self._text_box:child("scroll_down_indicator_arrow"):set_visible(arrows_visible)
end
...
function TextBoxGui:_check_scroll_indicator_states()
	...
	if SystemInfo:platform() == Idstring("NX64") then
		local scroll_bar_bg = self._text_box:child("scroll_bar_bg")
		local window_h = scroll_panel:h()
		local pane_h = scroll_text:h()
		local base_y = scroll_bar_bg:y()

		if pane_h <= window_h then
			scroll_bar:set_y(base_y)
		else
			local scroll_frac = -scroll_text:y() / (pane_h - window_h)

			if scroll_frac < 0 then
				scroll_frac = 0
			end

			if scroll_frac > 1 then
				scroll_frac = 1
			end

			scroll_bar:set_y(base_y + scroll_bar_bg:h() * (1 - window_h / pane_h) * scroll_frac)
		end
	else
...
function TextBoxGui:check_grab_scroll_bar(x, y)
	self.swiping_down_speed = 0
	self.swiping_up_speed = 0
	...
	if SystemInfo:platform() == Idstring("NX64") then
		if self._text_box:child("scroll_bar_bg") and self._text_box:child("scroll_bar_bg"):inside(x, y) then
			self._grabbed_scroll_bar = true
			self._current_y = y

			return true
		end
	elseif self._text_box:child("scroll_bar") and self._text_box:child("scroll_bar"):inside(x, y) then
		self._grabbed_scroll_bar = true
		self._current_y = y
...
function TextBoxGui:release_scroll_bar()
	...
	if self._grabbed_scroll_panel then
		self._grabbed_scroll_panel = false
...
function TextBoxGui:mouse_moved(x, y)
	...
	if SystemInfo:platform() == Idstring("NX64") then
		if self.swiping_down_speed and self.swiping_down_speed > 0 then
			self:scroll_up(self.swiping_down_speed * 0.75)

			self.swiping_down_speed = self.swiping_down_speed - 1
		elseif self.swiping_up_speed and self.swiping_up_speed > 0 then
			self:scroll_down(self.swiping_up_speed * 0.75)

			self.swiping_up_speed = self.swiping_up_speed - 1
		end
	...
	elseif self._grabbed_scroll_panel then
		if self._current_y < y then
			self:scroll_up((y - self._current_y) * 0.5)
		elseif y < self._current_y then
			self:scroll_down((self._current_y - y) * 0.5)
		end

		self._current_y = y

		return true, "grab"
	elseif self._grabbed_scroll_bar then
		self:scroll_with_bar(y, self._current_y)
...
function TextBoxGui:swipe_down(x, y)
	self.swiping_down_speed = 0
	self.swiping_up_speed = 0
	...
	if info_area:inside(x, y) and scroll_panel:inside(x, y) then
		self.swiping_down_speed = 30

		return true
	end
end

function TextBoxGui:swipe_up(x, y)
	self.swiping_down_speed = 0
	self.swiping_up_speed = 0
	...
	if info_area:inside(x, y) and scroll_panel:inside(x, y) then
		self.swiping_up_speed = 30

		return true
	end
end

function TextBoxGui:pan(x, y, amount_panned)
	if not self._visible then
		return false
	end
	...
	if info_area:inside(x, y) and scroll_panel:inside(x, y) then
		if amount_panned < 0 then
			self:scroll_up(-amount_panned)
		else
			self:scroll_down(amount_panned)
		end
	end
