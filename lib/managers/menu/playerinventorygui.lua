...
function PlayerInventoryGui:init(ws, fullscreen_ws, node)
	...
	if SystemInfo:platform() == Idstring("NX64") then
		title_string = "menu_player_inventory_new"
	end
	...
	if SystemInfo:platform() == Idstring("NX64") then
		eqpt_skills_panel:move(-5, -10)
		weapons_panel:move(-5, -10)
		character_panel:move(-5, -10)
		primary_box:move(-5, -10)
		secondary_box:move(-5, -10)
		melee_box:move(-5, -10)
		throwable_box:move(-5, -10)
		armor_box:move(-5, -10)
		deployable_box:move(-5, -10)
		mask_box:move(-5, -10)
		character_box:move(-5, -10)
		skill_box:move(-5, -10)
		specialization_box:move(-5, -10)
	end

	if SystemInfo:platform() == Idstring("NX64") then
		self._panel:child("character"):rect({
			name = "bg_rect",
			color = tweak_data.menu.unselected_color,
			alpha = tweak_data.menu.selected_alpha
		})
		self._panel:child("mask"):rect({
			name = "bg_rect",
			color = tweak_data.menu.unselected_color,
			alpha = tweak_data.menu.selected_alpha
		})
		self._panel:child("primary"):rect({
			name = "bg_rect",
			color = tweak_data.menu.unselected_color,
			alpha = tweak_data.menu.selected_alpha
		})
		self._panel:child("secondary"):rect({
			name = "bg_rect",
			color = tweak_data.menu.unselected_color,
			alpha = tweak_data.menu.selected_alpha
		})
		self._panel:child("melee"):rect({
			name = "bg_rect",
			color = tweak_data.menu.unselected_color,
			alpha = tweak_data.menu.selected_alpha
		})
		self._panel:child("throwable"):rect({
			name = "bg_rect",
			color = tweak_data.menu.unselected_color,
			alpha = tweak_data.menu.selected_alpha
		})
		self._panel:child("deployable"):rect({
			name = "bg_rect",
			color = tweak_data.menu.unselected_color,
			alpha = tweak_data.menu.selected_alpha
		})
		self._panel:child("armor"):rect({
			name = "bg_rect",
			color = tweak_data.menu.unselected_color,
			alpha = tweak_data.menu.selected_alpha
		})
		self._panel:child("skilltree"):rect({
			name = "bg_rect",
			color = tweak_data.menu.unselected_color,
			alpha = tweak_data.menu.selected_alpha
		})
		self._panel:child("specialization"):rect({
			name = "bg_rect",
			color = tweak_data.menu.unselected_color,
			alpha = tweak_data.menu.selected_alpha
		})
	...
	local alias_text = nil

	if SystemInfo:platform() == Idstring("NX64") then
		alias_text = self._panel:text({
			blend_mode = "add",
			x = 2,
			y = TOP_ADJUSTMENT,
			text = tostring(NX64Online:name() or managers.blackmarket:get_preferred_character_real_name()),
			font = tweak_data.menu.pd2_small_font,
			font_size = tweak_data.menu.pd2_small_font_size,
			color = tweak_data.screen_colors.text
		})
	...
	self._multi_profile_item:scale_arrows(0.5, 0.5)
...
function highlight_animation_inventory_gui(panel, time_to_animate, inv_gui)
	...
	inv_gui.button_anim_complete = true

	panel:set_visible(false)
end

function PlayerInventoryGui:update(t, dt)
...
function PlayerInventoryGui:setup_player_stats(panel)
	...
	local x = 0
	local y = 20
	local text_panel, text_columns = nil
...
function PlayerInventoryGui:create_box(params, index)
	...
	if SystemInfo:platform() == Idstring("NX64") and params.has_corners then
		corner_deco.corners = {
			panel:bitmap({
		...
		corner_deco.corner_visible_size = corner_deco.corners[1]:w()
		corner_deco.corner_hidden_size = 2
		local cornerSize = corner_deco.corner_hidden_size

		corner_deco.corners[1]:set_texture_rect(0, 0, cornerSize, cornerSize)
		corner_deco.corners[2]:set_texture_rect(cornerSize, 0, -cornerSize, cornerSize)
		corner_deco.corners[3]:set_texture_rect(0, cornerSize, cornerSize, -cornerSize)
		corner_deco.corners[4]:set_texture_rect(cornerSize, cornerSize, -cornerSize, -cornerSize)
		corner_deco.corners[1]:set_size(cornerSize, cornerSize)
		corner_deco.corners[2]:set_size(cornerSize, cornerSize)
		corner_deco.corners[3]:set_size(cornerSize, cornerSize)
		corner_deco.corners[4]:set_size(cornerSize, cornerSize)
		corner_deco.corners[1]:set_left(0)
		corner_deco.corners[1]:set_top(0)
		corner_deco.corners[2]:set_right(panel:w())
		corner_deco.corners[2]:set_top(0)
		corner_deco.corners[3]:set_left(0)
		corner_deco.corners[3]:set_bottom(panel:h())
		corner_deco.corners[4]:set_right(panel:w())
		corner_deco.corners[4]:set_bottom(panel:h())
	end

	local box = {
		selected = false,
		name = name,
		corner_deco = corner_deco,
		layer = layer,
		panel = panel,
		clbks = clbks,
	...
	for i, object_name in ipairs(list) do
		if box[object_name] then
			...
			if SystemInfo:platform() ~= Idstring("NX64") then
				...
				local corner_deco = box.corner_deco

				if panel:child("bg_rect") then
					panel:child("bg_rect"):set_color(selected and tweak_data.menu.selected_color or tweak_data.menu.unselected_color)
					panel:child("bg_rect"):set_alpha(selected and tweak_data.menu.selected_alpha or tweak_data.menu.unselected_alpha)

					if corner_deco and corner_deco.corners then
						local cornerSize = selected and corner_deco.corner_visible_size or corner_deco.corner_hidden_size
	...
	if box and box.clbks and box.panel:tree_visible() and box.panel:inside(x, y) then
		if left_clicked and box.clbks.left then
			...
			if SystemInfo:platform() == Idstring("NX64") then
				...
				if box.panel:parent():child("anim_panel") then
					anim_panel = box.panel:parent():child("anim_panel")

					anim_panel:set_width(box.panel:w())
					anim_panel:set_height(box.panel:h())
					anim_panel:set_x(box.panel:x())
					anim_panel:set_y(box.panel:y())
				else
					anim_panel = box.panel:parent():panel({
						name = "anim_panel",
						x = box.panel:x(),
						y = box.panel:y(),
						w = box.panel:w(),
				...
				anim_panel:child("anim_box"):animate(callback(nil, _G, "highlight_animation_inventory_gui"), anim_time, self)
