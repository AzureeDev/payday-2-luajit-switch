...
function BlackMarketGuiTabItem:init(main_panel, data, node, size_data, hide_select_rect, scroll_tab_table, parent)
	...
	if SystemInfo:platform() == Idstring("NX64") and slots[2] > 1 then
		local total_slots = slots[1] * slots[2]
		slots[1] = 4
		slots[2] = 3.5
	end
	...
	if SystemInfo:platform() == Idstring("NX64") then
		...
		padding_h = 6
		left_padding = 6
		self._size_data.padding_h = 6
		self._size_data.padding_w = 6
		local padding_on_row = padding_w * slots[1] + left_padding
		local padding_on_col = padding_w * slots[2] + 6
		square_w = (grid_panel_w - padding_on_row) / slots[1]
		square_h = (grid_panel_h - padding_on_col) / slots[2]
	else
	...
	if slots[2] == 1 then
		if SystemInfo:platform() == Idstring("NX64") then
			square_h = grid_panel_h - padding_w * 2
		else
	...
	if SystemInfo:platform() == Idstring("NX64") then
		self._tab_panel:set_size(tw + 30, th + 10)
	else
	...
	if SystemInfo:platform() == Idstring("NX64") then
		...
		self._tab_panel:bitmap({
			texture = "guis/textures/switch/large_selected_hud",
	...
	if slots[2] ~= 1 then
		self._grid_scroll_panel:set_h(grid_panel_h / slots[2] * math.ceil(#self._data / slots[1]))
	else
	...
	if SystemInfo:platform() ~= Idstring("NX64") and y_scrolling then
	...
	if SystemInfo:platform() == Idstring("NX64") then
		scroll_down_indicator_arrow:set_visible(false)
		scroll_bar_box_panel:rect({
			name = "scroll_bar_box",
			valign = "grow",
			halign = "grow",
			alpha = 1,
	...
	if SystemInfo:platform() == Idstring("NX64") then
		scroll_bar_box_panel:set_w(tweak_data.menu.scroll_bar_width)
		self._scroll_bar_panel:set_w(tweak_data.menu.scroll_bar_width)
	end
...
function BlackMarketGuiTabItem:update(t, dt)
...
function BlackMarketGuiTabItem:set_tab_text(new_text)
	...
	if SystemInfo:platform() == Idstring("NX64") then
		self._tab_panel:child("tab_unselected_rect"):set_size(self._tab_panel:w() * 1.4, self._tab_panel:h() * 1.7)
		self._tab_panel:child("tab_select_rect"):set_size(self._tab_panel:w() * 1.4, self._tab_panel:h() * 1.7)
		self._tab_panel:child("tab_select_rect"):set_center_x(text:center_x())
		self._tab_panel:child("tab_select_rect"):set_center_y(text:center_y())
		self._tab_panel:child("tab_unselected_rect"):set_center_x(text:center_x())
		self._tab_panel:child("tab_unselected_rect"):set_center_y(text:center_y())
	else
...
function BlackMarketGuiTabItem:refresh()
	...
	if self._selected then
		...
		if SystemInfo:platform() == Idstring("NX64") then
			self._tab_panel:child("tab_unselected_rect"):hide()
			self._tab_panel:child("tab_text"):set_color(Color.white)
		end
	elseif self._highlighted then
		...
		if SystemInfo:platform() == Idstring("NX64") then
			self._tab_panel:child("tab_unselected_rect"):show()
		end
	else
...
function BlackMarketGuiTabItem:mouse_pressed(button, x, y)
	...
	if alive(self._scroll_bar_panel) and self._grid_panel:h() < self._grid_scroll_panel:h() then
		if SystemInfo:platform() == Idstring("NX64") then
			...
			if self._scroll_bar_panel:center_x() - x > 0 then
				adjust_x = x + self._scroll_bar_panel:w() * 0.5
			else
				adjust_x = x - self._scroll_bar_panel:w() * 0.5
			end

			if grid_panel:h() < grid_scroll_panel:h() then
				if self._scroll_bar_panel and self._scroll_bar_panel:inside(adjust_x, y) or self.scroll_bar and self.scroll_bar:inside(adjust_x, y) then
					if Idstring("0") then
						local target_scroll_pct = (y - self._scroll_bar_panel:y()) / self._scroll_bar_panel:h()
						local current_scroll_pct = self.scroll_bar:center_y() / self._scroll_bar_panel:h()
						local scroll_amount = (target_scroll_pct - current_scroll_pct) * grid_scroll_panel:h() - grid_panel:h() * 0.5
				...
				elseif grid_scroll_panel:inside(x, y) then
	...
	if SystemInfo:platform() == Idstring("NX64") and button == Idstring("0") and alive(self._tab_pages_panel) then
	...
	if self._slots[self._slot_highlighted] and self._slots[self._slot_highlighted]:inside(x, y) and self._slots[self._slot_selected] then
...
function BlackMarketGuiTabItem:refresh_scrollbar()
	if SystemInfo:platform() == Idstring("NX64") then
		local grid_panel = self._grid_panel
		local grid_scroll_panel = self._grid_scroll_panel

		if self.scroll_bar and self._scroll_bar_panel then
			local limit = -(grid_scroll_panel:h() - grid_panel:h())
			local new_y = grid_scroll_panel:y() / limit * (self._scroll_bar_panel:h() - self.scroll_bar:h())
...
function BlackMarketGuiTabItem:touch_scroll(dy)
	local grid_panel = self._grid_panel
	local grid_scroll_panel = self._grid_scroll_panel
	local limit = -1 * (grid_scroll_panel:h() - grid_panel:h())

	if grid_scroll_panel:y() + dy > 0 then
		dy = -grid_scroll_panel:y()
	end

	if limit > grid_scroll_panel:y() + dy then
		dy = limit - grid_scroll_panel:y()
	end

	grid_scroll_panel:move(0, dy)
	self:refresh_scrollbar()

	if self.scroll_bar and self._scroll_bar_panel and SystemInfo:platform() ~= Idstring("NX64") then
		local scroll_bar_move_y = -(dy / grid_scroll_panel:h()) * self._scroll_bar_panel:h()
...
function BlackMarketGuiTabItem:mouse_moved(x, y)
	if SystemInfo:platform() == Idstring("NX64") then
		...
		elseif self.scrolling_bar then
			local amount_panned = self.pan_y - y
			local bar_to_panel_ratio = grid_scroll_panel:h() / self._scroll_bar_panel:h()
			amount_panned = amount_panned * bar_to_panel_ratio

			self:touch_scroll(amount_panned)

			self.pan_y = y
		elseif self._scroll_bar_panel and self._scroll_bar_panel:inside(x, y) or self.scroll_bar and self.scroll_bar:inside(x, y) then
			local target_scroll_pct = (y - self._scroll_bar_panel:y()) / self._scroll_bar_panel:h()
			local current_scroll_pct = self.scroll_bar:center_y() / self._scroll_bar_panel:h()
			local scroll_amount = (target_scroll_pct - current_scroll_pct) * grid_scroll_panel:h() - grid_panel:h() * 0.5
	...
	if SystemInfo:platform() ~= Idstring("NX64") and alive(self._tab_pages_panel) then
		self._tab_pages_highlighted = self._tab_pages_highlighted or {}
		local num_per_page = self.my_slots_dimensions and self.my_slots_dimensions[1] or 6
		local used = false
		local pointer = "arrow"
...
function BlackMarketGuiSlotItem:set_corners_visible(visible)
	self.corners[1]:set_visible(visible)
	self.corners[2]:set_visible(visible)
	self.corners[3]:set_visible(visible)
	self.corners[4]:set_visible(visible)
end

function BlackMarketGuiSlotItem:set_corners_selected(selected)
	local cornerSize = nil

	if selected then
		cornerSize = self.corner_visible_size
	else
		cornerSize = self.corner_hidden_size
	end

	self.corners[1]:set_texture_rect(0, 0, cornerSize, cornerSize)
	self.corners[2]:set_texture_rect(cornerSize, 0, -cornerSize, cornerSize)
	self.corners[3]:set_texture_rect(0, cornerSize, cornerSize, -cornerSize)
	self.corners[4]:set_texture_rect(cornerSize, cornerSize, -cornerSize, -cornerSize)
	self.corners[1]:set_size(cornerSize, cornerSize)
	self.corners[2]:set_size(cornerSize, cornerSize)
	self.corners[3]:set_size(cornerSize, cornerSize)
	self.corners[4]:set_size(cornerSize, cornerSize)
	self.corners[1]:set_left(0)
	self.corners[1]:set_top(0)
	self.corners[2]:set_right(self._panel:w())
	self.corners[2]:set_top(0)
	self.corners[3]:set_left(0)
	self.corners[3]:set_bottom(self._panel:h())
	self.corners[4]:set_right(self._panel:w())
	self.corners[4]:set_bottom(self._panel:h())
end
...
function BlackMarketGuiSlotItem:init(main_panel, data, x, y, w, h)
	...
	if SystemInfo:platform() == Idstring("NX64") then
		self.rect_bg = self._panel:rect({
			color = tweak_data.menu.unselected_color,
			alpha = tweak_data.menu.unselected_alpha
		})
		self.corners = {
			self._panel:bitmap({
		...
		self.corner_visible_size = self.corners[1]:w()
		self.corner_hidden_size = 2

		self:set_corners_visible(true)
		self:set_corners_selected(false)
	end
	...
	if SystemInfo:platform() == Idstring("NX64") and not data.button_text and not data.bitmap_texture and not data.mid_text then
		self.rect_bg:set_alpha(0)
		self:set_corners_visible(false)
	end
...
function BlackMarketGuiSlotItem:set_highlight(highlight, instant)
	if self._bitmap and not self._loading_texture or self._text_in_mid then
		if highlight then
			...
			if SystemInfo:platform() == Idstring("NX64") then
				self.rect_bg:set_color(tweak_data.menu.selected_color)
				self.rect_bg:set_alpha(tweak_data.menu.selected_alpha)

				if self.corners then
					self:set_corners_selected(true)
				end
			end
			...
			if SystemInfo:platform() == Idstring("NX64") then
				...
				self.rect_bg:set_color(tweak_data.menu.unselected_color)
				self.rect_bg:set_alpha(tweak_data.menu.unselected_alpha)
			end
...
function BlackMarketGuiButtonItem:init(main_panel, data, x, num_buttons)
	...
	if SystemInfo:platform() == Idstring("NX64") then
		if not data.disabled then
			self._btn_text = self._panel:text({
				...
				x = 10,
				valign = "center",
				font_size = medium_font_size + up_font_size,
				font = medium_font,
				color = Color.white
			})
		else
			self._btn_text = self._panel:text({
				...
				font = medium_font,
				color = Color.white,
				alpha = tweak_data.menu.disabled_alpha
			})
		end
	else
		self._btn_text = self._panel:text({
	...
	if not managers.menu:is_pc_controller() and not managers.menu:is_switch_controller() then
		self._btn_text:set_color(tweak_data.screen_colors.text)
	end

	self.main_panel_width = main_panel:w() - x * 2

	if SystemInfo:platform() == Idstring("NX64") then
		self.main_panel_width = main_panel:w()

		self._panel:set_size(main_panel:w() - 8, medium_font_size * 1.5)
		self._btn_text:set_size(self._panel:size())
		...
		if not data.disabled then
			...
			self._panel:rect({
				...
				halign = "scale",
				valign = "scale",
				color = tweak_data.menu.disabled_color,
				alpha = tweak_data.menu.disabled_alpha
			})
		end
	else
		self._panel:set_size(self.main_panel_width, medium_font_size)
		self._panel:rect({
			blend_mode = "add",
			name = "select_rect",
			halign = "scale",
...
function BlackMarketGuiButtonItem:refresh()
	...
	self._panel:child("select_rect"):set_visible(self._highlighted or managers.menu:is_switch_controller())
end
...
function BlackMarketGuiButtonItem:set_order(prio)
	if SystemInfo:platform() == Idstring("NX64") then
		...
		self._panel:set_size(self.main_panel_width - y_padding * 4, medium_font_size * 1.5)
		self._btn_text:set_w(self._panel:w())
		self._panel:set_left(y_padding * 2)
		self._panel:set_y((prio - 1) * (self._panel:h() + y_padding) + y_padding * 2)
		self._btn_text:set_font_size(medium_font_size)
		self._btn_text:set_center_y(self._panel:h() / 2)
		self._btn_text:set_x(self._btn_text:font_size() / 2)
	else
...
function BlackMarketGuiButtonItem:set_text_params(params)
	...
	if SystemInfo:platform() ~= Idstring("NX64") then
		self._btn_text:set_center_x(self._panel:w() / 2)
	end
end
...
function auto_scroll_text_blackmarket(text, owner, stat_panel, mini_icons)
	local scroll_speed = 30
	local limit = -1 * (text:h() - owner:h())
	...
	while true do
		...
		if pause > 0 then
			...
			elseif limit > text:y() + dy then
				...
				if stat_panel then
					stat_panel:move(0, dy)
				end

				if mini_icons then
					for _, desc_mini_icon in ipairs(mini_icons) do
						if desc_mini_icon then
							desc_mini_icon[1]:move(0, dy)
						end
					end
				end
			end
...
function BlackMarketGui:_setup(is_start_page, component_data)
	...
	if self._tabs[self._selected] then
		...
		if self._data.add_market_panel then
			...
			if (managers.menu:is_pc_controller() or managers.menu:is_switch_controller()) and num_market_bundles > 0 then
				info_box_top = info_box_top + h
				info_box_h = info_box_h - h
		...
		for btn, data in pairs(BTNS) do
			if SystemInfo:platform() == Idstring("NX64") then
				if data.btn == "BTN_A" then
					data.prio = 1
				elseif data.btn == "BTN_X" then
					data.prio = 2
				elseif data.btn == "BTN_Y" then
					data.prio = 4
				elseif data.btn == "BTN_STICK_R" then
					data.prio = 3
				end
			end
		...
		if SystemInfo:platform() == Idstring("NX64") then
			...
			for btn, btn_data in pairs(BTNS) do
				local new_btn = BlackMarketGuiButtonItem:new(self._buttons, btn_data, btn_x, #BTNS)
...
function BlackMarketGui:_update_borders()
	...
	local resize_button_panel = SystemInfo:platform() ~= Idstring("NX64") or self._selected_slot._data.category ~= "primaries" and self._selected_slot._data.category ~= "secondaries" and self._selected_slot._data.category ~= "armors" and self._selected_slot._data.category ~= "melee_weapons" or not not self._data.buying_weapon

	if SystemInfo:platform() ~= Idstring("NX64") or self._selected_slot._data.category ~= "primaries" and self._selected_slot._data.category ~= "secondaries" and self._selected_slot._data.category ~= "armors" and self._selected_slot._data.category ~= "melee_weapons" or not not self._data.buying_weapon then
		self._btn_panel:set_visible(self._button_count > 0 and true or false)
	end

	if SystemInfo:platform() == Idstring("NX64") then
		self._btn_panel:set_h(4 * (medium_font_size * 1.5 + 4) + 2)
		self._buttons:set_h(self._btn_panel:h())
	else
	...
	if SystemInfo:platform() == Idstring("NX64") and (self._selected_slot._data.category == "primaries" or self._selected_slot._data.category == "secondaries" or self._selected_slot._data.category == "armors" or self._selected_slot._data.category == "melee_weapons") and not self._data.buying_weapon then
		weapon_info_height = info_box_panel:h() - (self._btn_panel:h() + 8) - (self._detection_panel:visible() and self._detection_panel:h() + 8 or 0)
	end
	...
	if SystemInfo:platform() == Idstring("NX64") then
		self._info_texts_panel:set_h(weapon_info_height)
	else
	...
	if SystemInfo:platform() == Idstring("NX64") then
		if self._weapon_info_panel:h() < self._info_texts_panel:h() then
			local owning_panel = self._weapon_info_panel

			self._info_texts_panel:animate(callback(nil, _G, "auto_scroll_text_blackmarket"), owning_panel, self._stats_panel, self._desc_mini_icons)
		else
			self._info_texts_panel:set_y(0)
			self._info_texts_panel:stop()
		end
	end
end
...
function BlackMarketGui:update_info_text()
	...
	if SystemInfo:platform() == Idstring("NX64") then
		self._info_texts_panel:stop()
		self._info_texts_panel:set_y(0)
		self._stats_panel:set_y(self._stats_panel_y)
		...
		if self._desc_mini_icons then
			...
			for _, desc_mini_icon in ipairs(self._desc_mini_icons) do
				if desc_mini_icon then
					h = h + desc_mini_icon[1]:h()
				end
			end

			self._info_texts_panel:set_h(y + self._info_texts[#self._info_texts]:h() + h)
		else
			self._info_texts_panel:set_h(y + self._info_texts[#self._info_texts]:h())
		end
...
function BlackMarketGui:mouse_moved(o, x, y)
	...
	if not SystemInfo:platform() == Idstring("NX64") and update_select then
...
function highlight_animation_black_market(panel, time_to_animate, item)
	...
	item.anim_completed = true
end

function side_button_pressed_callback(item)
	if item._btns[item._button_highlighted] then
		local data = item._btns[item._button_highlighted]._data

		managers.menu_component:post_event("menu_enter")
		data.callback(item._slot_data, item._data.topic_params)
	end
end
...
function BlackMarketGui:mouse_pressed(button, x, y)
	...
	local scroll_button_pressed = false

	if SystemInfo:platform() == Idstring("NX64") then
		scroll_button_pressed = button == Idstring("pan_up") or button == Idstring("pan_down") or button == Idstring("swipe_up") or button == Idstring("swipe_down")
	else
		scroll_button_pressed = button == Idstring("mouse wheel up") or button == Idstring("mouse wheel down")
	end

	local inside_tab_area = self._tab_area_panel:inside(x, y) or self._data.scroll_tab_anywhere

	if SystemInfo:platform() == Idstring("NX64") then
		if self._tabs[self._selected] and scroll_button_pressed then
			self._tabs[self._selected]:mouse_pressed(button, x, y)
		end
	elseif inside_tab_area then
		if button == Idstring("mouse wheel down") then
			self:next_page(true)
	...
	if SystemInfo:platform() == Idstring("NX64") then
		...
		for i, tab in ipairs(self._tabs) do
			if inside_tab_scroll and tab:inside(x, y) then
				self._highlighted = i
	...
	elseif self._tabs[self._selected] then
		local selected_slot = self._tabs[self._selected]:mouse_pressed(button, x, y)
		local current_selected_name = self._selected_slot._name
		local already_selected = self._selected_slot:inside(x, y)

		if selected_slot then
			self:on_slot_selected(selected_slot)

			if SystemInfo:platform() == Idstring("NX64") then
				if already_selected and self._tabs[self._selected]:inside(x, y) then
					self:press_first_btn(button)

					return
				end
			elseif self._selected_slot._name == current_selected_name and self._tabs[self._selected]:inside(x, y) then
				self:press_first_btn(button)
		...
		if selected_slot and SystemInfo:platform() ~= Idstring("NX64") then
			managers.menu:on_press_rumble()
	...
	if self._btns[self._button_highlighted] and self._btns[self._button_highlighted]:inside(x, y) then
		...
		if data.callback and (not self._button_press_delay or self._button_press_delay < TimerManager:main():time()) then
			if SystemInfo:platform() ~= Idstring("NX64") then
				...
				local anim_time = 0.1
				local panel = self._btns[self._button_highlighted]._panel

				if alive(self.anim_panel) then
					self.anim_panel:set_width(panel:w())
					self.anim_panel:set_height(panel:h())
					self.anim_panel:set_x(panel:x())
					self.anim_panel:set_y(panel:y())
				else
					self.anim_panel = panel:parent():panel({
						name = "anim_panel",
						x = panel:x(),
						y = panel:y(),
						w = panel:w(),
				...
				self.anim_panel:child("anim_box"):animate(callback(nil, _G, "highlight_animation_black_market"), anim_time, self)

				self._button_press_delay = TimerManager:main():time() + anim_time * 1.5
			end
		end
	end
...
function BlackMarketGui:update(t, dt)
	if SystemInfo:platform() == Idstring("NX64") and self.anim_completed then
		side_button_pressed_callback(self)

		self.anim_completed = false
	end

	if self._tabs[self._selected] then
		self._tabs[self._selected]:update(t, dt)
	end
...
function BlackMarketGui:show_btns(slot)
	...
	if SystemInfo:platform() == Idstring("NX64") then
		...
		for i, btn in ipairs(data.buttons or data) do
			buttons_included[self._btns[btn]._data.btn] = true
		end

		if not buttons_included.BTN_A then
			table.insert(data.buttons or data, "dis_a")
		end

		if not buttons_included.BTN_X then
			table.insert(data.buttons or data, "dis_x")
		end

		if not buttons_included.BTN_Y then
			table.insert(data.buttons or data, "dis_y")
		end

		if not buttons_included.BTN_STICK_R then
			table.insert(data.buttons or data, "dis_stick_r")
		end
	end
	...
	if (managers.menu:is_pc_controller() or managers.menu:is_switch_controller()) and #btns > 0 then
...
function BlackMarketGui:populate_weapon_category(category, data)
	...
	for i, crafted in pairs(crafted_category) do
		...
		if currently_holding then
			...
			local can_mod = true
			can_mod = Application:installer():get_progress() >= 1
...
function BlackMarketGui:populate_characters(data)
	...
	for i = 1, CriminalsManager.get_num_characters() do
		...
		if managers.dlc:is_installing() and not character_table.installed then
			new_data.lock_texture = "guis/textures/pd2/skilltree/padlock"
			new_data.dlc_locked = "bm_menu_install_locked_char"
...
function BlackMarketGui:populate_grenades(data)
	...
	for i, grenades_data in ipairs(sort_data) do
		...
		if managers.dlc:is_installing() and not tweak_data.blackmarket.projectiles[new_data.name].installed then
			new_data.lock_texture = "guis/textures/pd2/skilltree/padlock"
			new_data.install_lock = "bm_menu_install_locked"
			new_data.unlocked = false
			active = false
		end
	...
	for i, crafted in pairs(crafted_category) do
		...
		if currently_holding then
			...
			if new_data.unlocked then
				...
				can_mod_mask = Application:installer():get_progress() >= 1
...
function BlackMarketGui:populate_masks_new(data)
	...
	for i, index in pairs(data.on_create_data) do
		...
		if crafted then
			...
			if managers.dlc:is_installing() and not tweak_data.blackmarket.masks[new_data.name].installed then
				new_data.lock_texture = "guis/textures/pd2/skilltree/padlock"
				new_data.dlc_locked = "bm_menu_install_locked"
				active = false
			end
...
function BlackMarketGui:populate_weapon_category_new(data)
	...
	for i, index in pairs(data.on_create_data) do
		...
		if crafted then
			...
			if managers.dlc:is_installing() and not tweak_data.weapon[new_data.name].installed then
				new_data.lock_texture = "guis/textures/pd2/skilltree/padlock"
				new_data.install_lock = "bm_menu_install_locked"
				new_data.unlocked = false
				active = false
...
function BlackMarketGui:populate_melee_weapons_new(data)
	...
	for i, melee_weapon_data in ipairs(data.on_create_data) do
		...
		if managers.dlc:is_installing() and not tweak_data.blackmarket.melee_weapons[new_data.name].installed then
			new_data.lock_texture = "guis/textures/pd2/skilltree/padlock"
			new_data.install_lock = "bm_menu_install_locked"
			new_data.unlocked = false
			active = false
...
function BlackMarketGui:populate_mods(data)
	...
	for index, mod_t in ipairs(data.on_create_data) do
		...
		if managers.dlc:is_installing() and not tweak_data.weapon.factory.parts[new_data.name].installed then
			new_data.lock_texture = "guis/textures/pd2/skilltree/padlock"
			new_data.install_lock = "bm_menu_install_locked"
			active = false
		end
		...
		local preview_forbidden = forbid
	...
	for type, mods in pairs(data.on_create_data) do
		...
		elseif data.category == "textures" then
			...
			if managers.dlc:is_installing() then
				active = tweak_data.blackmarket.textures[new_data.name].installed

				if tweak_data.blackmarket.textures[new_data.name].installed then
					active = false
			...
			active = not managers.dlc:is_installing() or tweak_data.blackmarket.materials[new_data.name].installed
		end
