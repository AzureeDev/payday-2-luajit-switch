...
function MenuSceneManager:update(t, dt)
	if SystemInfo:platform() == Idstring("NX64") then
		if self.zooming_in then
			self:change_fov("out", 20 * dt)
		elseif self.zooming_out then
			self:change_fov("in", 20 * dt)
		end
	end
...
function MenuSceneManager:mouse_pressed(o, button, x, y)
	...
	if button == Idstring("0") or button == Idstring("pan_left") or button == Idstring("pan_right") or button == Idstring("pan_up") or button == Idstring("pan_down") then
...
function MenuSceneManager:mouse_moved(o, x, y)
	...
	if self._character_grabbed and self._character_grabbed_current_x then
