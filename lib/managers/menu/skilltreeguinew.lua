...
function auto_scroll_text(text, owner)
...
function NewSkillTreeGui:_setup()
	...
	if SystemInfo:platform() == Idstring("NX64") then
		info_panel:set_world_top(tree_panel:world_y() - 70)
	else
	...
	local desc_panel_h = info_panel:h() - (skillpoints_panel:bottom() + PADDING)

	if SystemInfo:platform() == Idstring("NX64") then
		desc_panel_h = desc_panel_h * 0.77
	end
	...
	legends_panel:set_visible(SystemInfo:platform() ~= Idstring("NX64"))
	...
	legend_panel_reset_skills:set_visible(SystemInfo:platform() ~= Idstring("NX64"))

	if SystemInfo:platform() == Idstring("NX64") then
		local legend_padding = 3
		local button_width = description_panel:w()
		local button_height = tweak_data.menu.pd2_medium_font_size * 1.5
		local button_right = self._panel:w()
		local align_button_text = "center"
		local text_x = 0

		if SystemInfo:platform() == Idstring("NX64") then
			align_button_text = "left"
			text_x = medium_font_size / 2
		end

		local reset_skills_text = self._panel:panel({
			name = "reset_skills_legend",
			layer = 2,
			w = button_width,
			h = button_height
		})
		local reset_text = managers.localization:to_upper_text("nx64_skill_tree_reset_button")

		reset_skills_text:text({
			name = "reset_skills_text",
			vertical = "center",
			layer = 2,
			text = reset_text,
			font = small_font,
			...
			color = tweak_data.screen_colors.text,
			x = text_x,
			align = align_button_text
		})
		reset_skills_text:bitmap({
			texture = "guis/textures/switch/large_unselected_hud",
			name = "reset_skills_button",
			layer = 1,
			align = "center",
			texture_rect = {
				46,
		...
		reset_skills_text:set_world_right(button_right)
		reset_skills_text:set_world_y(description_panel:world_bottom() + 12)

		local switch_skillset = self._panel:panel({
			name = "switch_skillset",
			layer = 2,
			w = button_width,
			h = button_height
		})
		reset_text = managers.localization:to_upper_text("menu_st_switch_skillset")

		switch_skillset:text({
			name = "switch_skills_text",
			vertical = "center",
			layer = 2,
			text = reset_text,
			font = small_font,
		...
		switch_skillset:set_world_right(button_right)
		switch_skillset:set_world_y(reset_skills_text:world_bottom() + legend_padding)

		local invest = self._panel:panel({
			name = "invest_legend",
			layer = 2,
			w = button_width,
			h = button_height
		})
		reset_text = managers.localization:to_upper_text("menu_controller_invest")

		invest:text({
			name = "invest_legend_text",
			vertical = "center",
			layer = 2,
			text = reset_text,
			font = small_font,
		...
		invest:set_world_right(button_right)
		invest:set_world_y(switch_skillset:world_bottom() + legend_padding)

		local refund = self._panel:panel({
			name = "refund_legend",
			layer = 2,
			w = button_width,
			h = button_height
		})
		reset_text = managers.localization:to_upper_text("menu_controller_refund")

		refund:text({
			name = "refund_legend_text",
			vertical = "center",
			layer = 2,
			text = reset_text,
			font = small_font,
		...
		refund:set_world_right(button_right)
		refund:set_world_y(invest:world_bottom() + legend_padding)
	end
...
function NewSkillTreeGui:_update_description(item)
	...
	if h > desc_panel:h() - text:top() then
		...
		if SystemInfo:platform() == Idstring("NX64") then
			...
			local owning_panel = self.description_panel

			text:animate(callback(nil, _G, "auto_scroll_text"), owning_panel)
		end
	else
		text:set_font_size(small_font_size)
	end
...
function NewSkillTreeGui:_update_legends(item)
	...
	elseif SystemInfo:platform() == Idstring("NX64") then
		local gray_color = Color(255, 128, 128, 128) / 255
		local can_reset = self:has_spent_skill_points()

		self._panel:child("reset_skills_legend"):child("reset_skills_button"):set_color(can_reset and Color.white or gray_color)
		self._panel:child("reset_skills_legend"):child("reset_skills_text"):set_color(can_reset and Color.white or gray_color)
		self._panel:child("refund_legend"):child("refund_legend_button"):set_color(can_refund and Color.white or gray_color)
		self._panel:child("refund_legend"):child("refund_legend_text"):set_color(can_refund and Color.white or gray_color)
		self._panel:child("invest_legend"):child("invest_legend_button"):set_color(can_invest and Color.white or gray_color)
		self._panel:child("invest_legend"):child("invest_legend_text"):set_color(can_invest and Color.white or gray_color)
	else
		if can_refund then
			table.insert(legends, {
...
function NewSkillTreeGui:move_up()
	...
	if self._selected_item then
		return self:set_selected_item(self._selected_item:get_link("up"))
	end
end
...
function NewSkillTreeGui:move_down()
	...
	if self._selected_item then
		return self:set_selected_item(self._selected_item:get_link("down"))
	end
end
...
function NewSkillTreeGui:move_left()
	...
	if self._selected_item then
		return self:set_selected_item(self._selected_item:get_link("left"))
	end
end
...
function NewSkillTreeGui:move_right()
	...
	if self._selected_item then
		return self:set_selected_item(self._selected_item:get_link("right"))
	end
end
...
function NewSkillTreeGui:mouse_moved(o, x, y)
	...
	if SystemInfo:platform() == Idstring("NX64") then
		local legends = {
			self._panel:child("reset_skills_legend"):child("reset_skills_button"),
			self._panel:child("switch_skillset"):child("switch_skills_button"),
			self._panel:child("invest_legend"):child("invest_legend_button"),
			self._panel:child("refund_legend"):child("refund_legend_button")
		}
		local disabled_colour = Color(255, 128, 128, 128) / 255

		for _, legend in ipairs(legends) do
			if legend:inside(x, y) and legend:visible() and legend:color() ~= disabled_colour then
				inside = true
				pointer = "link"
	...
	if self._skillset_panel:inside(x, y) and SystemInfo:platform() ~= Idstring("NX64") then
...
function highlight_animation_skilltree_gui_new(panel, time_to_animate, item)
	...
	item.button_anim_finished = true

	panel:set_visible(false)
end

function NewSkillTreeGui:set_press_anim(panel)
	if SystemInfo:platform() == Idstring("NX64") then
		...
		self.anim_panel:child("anim_box"):animate(callback(nil, _G, "highlight_animation_skilltree_gui_new"), anim_time, self)
	end
end

function NewSkillTreeGui:mouse_pressed(button, x, y)
	local anim_owning_panel = nil
	...
	if button == Idstring("0") and self._skillset_panel:inside(x, y) and SystemInfo:platform() ~= Idstring("NX64") then
	...
	if self._panel:child("switch_skillset"):child("switch_skills_button"):inside(x, y) then
		self:set_press_anim(self._panel:child("switch_skillset"))

		self.button_anim_callback = "switch_skills"

		managers.menu:on_press_rumble()

		return
	elseif self._panel:child("reset_skills_legend"):child("reset_skills_button"):inside(x, y) then
		self:set_press_anim(self._panel:child("reset_skills_legend"))

		if SystemInfo:platform() == Idstring("NX64") then
			self.button_anim_callback = "respec_dialogue"
		elseif self:has_tree_spent_points(self._selected_page:trees_idx()) then
		...
		return
	elseif SystemInfo:platform() ~= Idstring("NX64") and self._panel:child("reset_all_text_panel"):child("reset_all_skills_button"):inside(x, y) and self:has_spent_skill_points() then
		self:respec_all()
		managers.menu:on_press_rumble()
	...
	if SystemInfo:platform() == Idstring("NX64") and self._selected_item and button == Idstring("0") then
		if self._panel:child("refund_legend"):child("refund_legend_button"):inside(x, y) then
			self:set_press_anim(self._panel:child("refund_legend"))
			self:refund_point(self._selected_item)
			managers.menu:on_press_rumble()

			return
		elseif self._panel:child("invest_legend"):child("invest_legend_button"):inside(x, y) then
			self:set_press_anim(self._panel:child("invest_legend"))
		...
		if self._selected_tab and self._selected_tab:inside(x, y) then
...
function NewSkillTreeGui:special_btn_pressed(button)
	...
	elseif button == Idstring("menu_respec_tree") then
		if SystemInfo:platform() == Idstring("NX64") then
			self:respec_dialogue()
		elseif self:has_tree_spent_points(self._selected_page:trees_idx()) then
			self:respec_page(self._tree_items[self._active_page])
		end

		return
	elseif button == Idstring("menu_respec_tree_all") and SystemInfo:platform() ~= Idstring("NX64") and self:has_spent_skill_points() then
		self:respec_all()
...
function NewSkillTreeGui:respec_dialogue()
	local can_reset_this_tree = self:has_tree_spent_points(self._selected_page:trees_idx())
	local can_reset_all = self:has_spent_skill_points()

	if not can_reset_this_tree and not can_reset_all then
		return
	end

	local dialog_data = {
		title = managers.localization:text("nx64_reset_skills_title"),
		text = managers.localization:text("nx64_reset_skills_msg"),
		nx_button_minw = 166,
		nx_forced_button_h = 76,
		forced_h = 180
	}
	local reset_tree_button = {
		text = managers.localization:text("nx64_reset_skills_single_button"),
		callback_func = can_reset_this_tree and callback(self, self, "respec_page", self._tree_items[self._active_page])
	}
	local reset_all_button = {
		text = managers.localization:text("nx64_reset_skills_all_button"),
		callback_func = can_reset_all and callback(self, self, "respec_all")
	}
	local cancel_button = {
...
function NewSkillTreeGui:update(t, dt)
	if self.button_anim_finished then
		if self.button_anim_callback then
			if self.button_anim_callback == "switch_skills" then
				managers.menu:open_node("skill_switch", {})
			elseif self.button_anim_callback == "respec_dialogue" then
				self:respec_dialogue()
			end
		end

		self.button_anim_callback = nil
		self.button_anim_finished = false
	end
...
function NewSkillTreeTabItem:init(page_tab_panel, page, tab_x, index, gui, page_item)
	...
	local page_tab_bg = nil

	if SystemInfo:platform() == Idstring("NX64") then
		page_tab_bg = page_panel:bitmap({
			texture = "guis/textures/switch/large_unselected_hud",
			name = "PageTabBG",
			texture_rect = {
				46,
...
function NewSkillTreeTabItem:refresh()
	...
	if SystemInfo:platform() == Idstring("NX64") then
		local background = self._page_panel:child("PageTabBG")
		...
		self._page_panel:child("PageText"):set_color(self._active and Color.white or self._selected and tweak_data.screen_colors.button_stage_2 or tweak_data.screen_colors.button_stage_3)
	else
