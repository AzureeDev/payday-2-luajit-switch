...
function SpecializationBoxGui:_create_text_box(ws, title, text, content_data, config)
	...
	local nxLayout = SystemInfo:platform() == Idstring("NX64")
	...
	if nxLayout then
		small_text.font_size = small_text.font_size * 1.28
		medium_text.font_size = medium_text.font_size * 1.28
	end
	...
	if nxLayout then
		progress_text:set_position(3, 9)
		progress_bg:set_position(progress_bg:left(), 43)
		progress_bg:set_w(self._scroll_panel:w() - progress_bg:left() - 0)
		progress_bg:set_h(44)
		progress_bg:set_color(Color(0, 0.69, 1))
		progress_bg:set_alpha(0.2)
	else
	...
	if nxLayout then
		progress_bar:set_color(Color(0, 0.69, 1))
		progress_bar:set_blend_mode("normal")
		progress_end:set_visible(false)
	else
	...
	conversion_rate_text:set_position(progress_bg:left(), progress_bg:bottom() + (nxLayout and 4 or 2))
	...
	exp_panel:set_top(conversion_rate_text:bottom() + 20 + (nxLayout and 14 or 0))
	exp_panel:set_h(self._scroll_panel:h() - exp_panel:top() - 10 - (nxLayout and 121 or 0))
	...
	exp_title_text:set_position(5, 5 + (nxLayout and 5 or 0))
	...
	points_gained_title_text:set_position(5, 5 + (nxLayout and 5 or 0))
