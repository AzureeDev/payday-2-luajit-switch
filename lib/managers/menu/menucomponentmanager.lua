...
require("lib/managers/NotificationPopUpHud")
...
function MenuComponentManager:init()
	...
	managers.notifications = NotificationPopUpHud:new()
end
...
function MenuComponentManager:set_active_components(components, node)
	...
	for _, component in ipairs(components) do
		local obj = self._active_components[component]
	...
	local gui = managers.menu:active_menu().renderer:active_node_gui()

	if gui ~= nil and gui.reactivate ~= nil then
		gui:reactivate()
	end
end
...
function MenuComponentManager:update(t, dt)
	...
	if SystemInfo:platform() == Idstring("NX64") and self._player_inventory_gui then
		self._player_inventory_gui:update(t, dt)
	end
...
function MenuComponentManager:create_ingame_contract_gui()
	...
	self._ingame_contract_gui:set_layer(tweak_data.gui.PAUSE_COMPONENT_LAYER)
end
...
function MenuComponentManager:create_ingame_waiting_gui()
	...
	self._ingame_waiting_gui:set_layer(tweak_data.gui.PAUSE_COMPONENT_LAYER)
		...
		if not self._is_game_installing and managers.menu:active_menu() then
			...
			if logic then
				...
				if node then
					...
					local crimenet_local = node:item("crimenet_local")

					if crimenet_local then
						crimenet_local:set_enabled(true)
