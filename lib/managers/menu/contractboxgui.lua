...
function ContractBoxGui:create_contract_box()
	...
	if SystemInfo:platform() == Idstring("NX64") then
		self._contract_panel:set_leftbottom(0, self._panel:h() - 60)
	else
	...
	if self._contract_text_header then
		...
		if wfs_text and not managers.menu:is_pc_controller() then
			if SystemInfo:platform() == Idstring("NX64") then
				wfs_text:set_leftbottom(20, self._contract_text_header:top())
			else
	...
	if wfs then
		...
		if SystemInfo:platform() == Idstring("NX64") then
			wfs:set_world_leftbottom(self._contract_panel:world_left() + 5, self._contract_panel:world_bottom())
		else
