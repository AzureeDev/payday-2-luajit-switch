...
function MultiProfileItemGui:init(ws, panel)
	self._ws = ws
	local profile_panel = self._panel or panel:panel({
		w = 280,
		h = 36
	})
...
function MultiProfileItemGui:update()
	...
	if SystemInfo:platform() == Idstring("NX64") then
		self._name_text:set_color(Color.white)
	end
	...
	if not arrow_left then
		if SystemInfo:platform() == Idstring("NX64") then
			...
			if SystemInfo:platform() == Idstring("NX64") then
				arrow_left:set_size(40, 40)
			end

			self._panel:set_h(arrow_left:h())
			self._name_text:set_center_y(arrow_left:center_y())
		else
			arrow_left = self._panel:bitmap({
	...
	if not arrow_right then
		if SystemInfo:platform() == Idstring("NX64") then
			arrow_right = self._panel:bitmap({
...
function MultiProfileItemGui:scale_arrows(scale_x, scale_y)
	local arrow_right = self._panel:child("arrow_right")
	local arrow_left = self._panel:child("arrow_left")

	arrow_right:set_size(arrow_right:w() * scale_x, arrow_right:h() * scale_y)
	arrow_left:set_size(arrow_left:w() * scale_x, arrow_left:h() * scale_y)
	arrow_left:set_left(0)
	arrow_right:set_right(self._panel:w())
	self._panel:set_h(arrow_left:h())
	self._name_text:set_center_y(arrow_left:center_y())
	self._box_panel:set_size(arrow_right:right() - arrow_left:left(), arrow_right:h())
end
...
function MultiProfileItemGui:mouse_moved(x, y)
	...
	if self._name_text:inside(x, y) and SystemInfo:platform() ~= Idstring("NX64") then
...
function MultiProfileItemGui:mouse_pressed(button, x, y)
	if button == Idstring("0") then
		...
		if self._name_selection and SystemInfo:platform() ~= Idstring("NX64") then
			self:trigger()
		end
	end
end
