...
function MissionBriefingTabItem:init(panel, text, i)
	...
	if SystemInfo:platform() == Idstring("NX64") then
		...
		self._tab_deselect_rect = self._main_panel:bitmap({
			texture = "guis/textures/switch/large_unselected_hud",
			layer = 0,
			visible = true,
			name = "tab_deselect_rect_" .. tostring(self._index),
			texture_rect = {
				46,
				31,
				420,
	...
	if self._tab_deselect_rect then
		self._tab_deselect_rect:set_shape(self._tab_text:shape())
	end

	self:update_tab_backgrounds_nx()
...
function MissionBriefingTabItem:update_tab_backgrounds_nx()
	if SystemInfo:platform() == Idstring("NX64") then
		local box_size_modifier_w = 0
		local box_size_modifier_h = 0

		if self._tab_deselect_rect then
			self._tab_deselect_rect:set_w(self._tab_deselect_rect:w() + box_size_modifier_w)
			self._tab_deselect_rect:set_h(self._tab_deselect_rect:h() + box_size_modifier_h)
		end

		self._tab_select_rect:set_w(self._tab_select_rect:w() + box_size_modifier_w)
		self._tab_select_rect:set_h(self._tab_select_rect:h() + box_size_modifier_h)
	end
end
...
function MissionBriefingTabItem:destroy()
	...
	if self._tab_deselect_rect then
		self._main_panel:remove(self._tab_deselect_rect)
	end

	self._panel = nil
	self._tab_text = nil
	self._tab_select_rect = nil
	self._tab_deselect_rect = nil
end
...
function AssetsItem:chk_preplanning_textures_done()
	...
	if alive(preplanning_panel) then
		...
		text:set_visible(managers.menu:is_pc_controller() or managers.menu:is_switch_controller())
...
function AssetsItem:move(x, y)
	...
	if new_selected >= 1 and new_selected <= #self._assets_list then
		local is_new = new_selected ~= asset_selected
...
function MissionBriefingGui:init(saferect_ws, fullrect_ws, node)
	...
	self._ready_button = self._panel:text({
		...
		layer = 2,
		text = ready_text,
		font_size = SystemInfo:platform() == Idstring("NX64") and 30 or tweak_data.menu.pd2_large_font_size,
		font = tweak_data.menu.pd2_large_font,
		color = SystemInfo:platform() == Idstring("NX64") and Color(1, 1, 1) or tweak_data.screen_colors.button_stage_3
	})
	...
	if SystemInfo:platform() == Idstring("NX64") then
		self._ready_tick_box = self._panel:bitmap({
			texture = "guis/textures/switch/tick_box_unselected_hud",
			name = "ready_tickbox",
			layer = 2,
			texture_rect = {
				35,
		...
		self._ready_tick_box:set_visible(true)
		self._ready_tick_box:set_size(40, 40)
	else
		self._ready_tick_box = self._panel:bitmap({
	...
	if SystemInfo:platform() == Idstring("NX64") then
		self._ready_button:set_rightbottom(self._panel:w() - 30, self._panel:h() - 5)

		self._ready_button_box = self._panel:bitmap({
			texture = "guis/textures/switch/large_unselected_hud",
			name = "ready_box",
			layer = 1,
			visible = true,
		...
		self._ready_button_box:set_size(self._ready_button:w() + 10, 40)
		self._ready_button_box:set_center_y(self._ready_button:center_y())
		self._ready_button_box:set_center_x(self._ready_button:center_x())
		self._ready_tick_box:set_right(self._ready_button_box:left() - 5)
		self._ready_tick_box:set_center_y(self._ready_button_box:center_y())
	else
...
function MissionBriefingGui:mouse_pressed(button, x, y)
	...
	if SystemInfo:platform() == Idstring("NX64") and button == Idstring("press") and (self._ready_button:inside(x, y) or self._ready_tick_box:inside(x, y)) then
	...
	if SystemInfo:platform() ~= Idstring("NX64") and (self._ready_button:inside(x, y) or self._ready_tick_box:inside(x, y)) then
		self:on_ready_pressed()
		managers.menu:on_press_rumble()
	end
...
function MissionBriefingGui:move_up()
	...
	if self._items[self._selected_item] then
		return self._items[self._selected_item]:move_up()
	end
end
...
function MissionBriefingGui:move_down()
	...
	if self._items[self._selected_item] then
		return self._items[self._selected_item]:move_down()
	end
end
...
function MissionBriefingGui:move_left()
	...
	if self._items[self._selected_item] then
		return self._items[self._selected_item]:move_left()
	end
end
...
function MissionBriefingGui:move_right()
	...
	if self._items[self._selected_item] then
		return self._items[self._selected_item]:move_right()
	end
end
