...
function IngameWaitingGui:init(ws)
	...
	self._button_panel = self._content_panel:panel({
		...
		h = text_font_size + bp_adjust
	})
	self._peer_btns_panel = self._button_panel:panel({
		w = 0
	})
	self._peer_btns = {
		self:add_button(self._peer_btns_panel, "hud_waiting_accept", "drop_in_accept", "spawn", 30, true),
		self:add_button(self._peer_btns_panel, "hud_waiting_return", "drop_in_return", "return_back", 30, true),
		self:add_button(self._peer_btns_panel, "hud_waiting_kick", "drop_in_kick", "kick", nil, true)
	}
	self._left_btns_panel = self._button_panel:panel({
		w = 0
	})
	self._left_btns = {
		self:add_button(self._left_btns_panel, "hud_waiting_ok", "drop_in_accept", "left_ok", nil, true)
	}

	self._content_panel:set_h(self._button_panel:bottom() + PADDING + bp_adjust)
	...
	if SystemInfo:platform() == Idstring("NX64") then
		self._content_panel:set_world_center_y(360)
	elseif managers.menu_component._ingame_contract_gui then
...
function IngameWaitingGui:close()
	self:_update_hidden_menu(nil)

	if SystemInfo:platform() == Idstring("NX64") then
		self:dummy_set_highlight(false)
	end
...
function IngameWaitingGui:calc_next(current)
	...
	for id = current, CriminalsManager.MAX_NR_CRIMINALS do
		...
		if peer and managers.wait:is_waiting(id) then
			if SystemInfo:platform() == Idstring("NX64") then
				self._saved_names[id] = peer:xuid()
			else
...
function IngameWaitingGui:add_button(panel, text, binding, func_name, padding, full_button)
	local params = {
		MY_BTN = managers.localization:btn_macro(binding, true, true) or ""
	}
	text = managers.localization:text(text, params)
	local btn, btn_panel = IngameWaitingButton:new(panel, text, binding, callback(self, self, func_name), full_button)
...
function IngameWaitingGui:temp_hide_non_dummy_items(node_gui, hidden)
	local items = node_gui.node:items()

	for _, item in pairs(items) do
		if item._type ~= "dummy" then
			item:set_temp_hidden(hidden)
		end
	end
end

function IngameWaitingGui:_update_hidden_menu(we_are_active)
	if we_are_active ~= self._hiding_ext_menu and SystemInfo:platform() == Idstring("NX64") then
		local active_node_gui = managers.menu:active_menu().renderer:active_node_gui()

		if active_node_gui ~= nil then
			if active_node_gui == self._check_active_node_gui and self._dummy_item ~= nil then
				self._hiding_ext_menu = we_are_active

				self:temp_hide_non_dummy_items(active_node_gui, we_are_active)
				active_node_gui:refresh_gui(active_node_gui.node)

				self._dummy_item.item.no_select = not self._content_panel:visible()

				active_node_gui.node:select_item(nil)
				active_node_gui:highlight_item(active_node_gui.node:item(), false)
			else
				self:temp_hide_non_dummy_items(active_node_gui, we_are_active)
			end
		end
...
function IngameWaitingGui:try_get_dummy()
	...
	if active_node_gui then
		...
		if SystemInfo:platform() == Idstring("NX64") then
			self._check_active_node_gui = active_node_gui
		end
...
function IngameWaitingGui:set_enabled(enabled)
	...
	if self._dummy_item then
		...
		self:_update_hidden_menu(self._content_panel:visible())
	end
end
...
function IngameWaitingGui:set_panel_for(peer_id)
	...
	if SystemInfo:platform() == Idstring("NX64") then
		self._saved_names[peer_id] = self._peer:xuid()
	else
	...
	if SystemInfo:platform() == Idstring("NX64") then
		self._name_text:set_text(self._peer:xuid() .. "  " .. (self._peer:rank() > 0 and managers.experience:rank_string(self._peer:rank()) .. "-" or "") .. (self._peer:level() or "") .. "")
	else
...
function IngameWaitingGui:dummy_set_highlight(highlight)
	...
	if SystemInfo:platform() == Idstring("NX64") then
		...
		if active_menu then
			active_menu.input:set_force_input(highlight)
		end
	end

	if not alive(self._background) then
		return
	end
...
function IngameWaitingButton:init(parent_panel, text, binding, callback, full_button)
	self._panel = parent_panel:panel()
	self._callback = callback
	self.binding = binding and Idstring(binding)
	local init_color = font_color_rest

	if SystemInfo:platform() == Idstring("NX64") then
		init_color = font_color_highlighted
	end
	...
	if SystemInfo:platform() == Idstring("NX64") then
		...
		self._panel:grow(pad_x * 2, pad_y * 2)
		self._text:move(pad_x, pad_y)

		local unselected = self._panel:bitmap({
		...
		local selected = self._panel:bitmap({
		...
		if full_button then
			MenuNodeGui.add_corner_deco(self, self._panel, 3, self._panel)
			self.corner_deco:resize(self._panel)
		end
	end
...
function IngameWaitingButton:set_highlighted(highlighted)
	...
	if SystemInfo:platform() == Idstring("NX64") then
		self._unselected:set_visible(not highlighted)
		self._selected:set_visible(highlighted)

		if self.corner_deco then
			self.corner_deco:set_highlight(highlighted)
		end

		self._text:set_color(highlighted and Color.white or font_color_highlighted)
	else
