...
function MenuItemMultiChoice:setup_gui(node, row_item)
	...
	if row_item.add_unique_panel then
		row_item.gui_panel = node.item_panel:parent():parent():panel({
			name = row_item.name,
			w = node.item_panel:w()
		})
	else
	...
	if SystemInfo:platform() == Idstring("NX64") and (row_item.crimenet_contract_gui or row_item.is_large_menu_button and not row_item.offshore_payday) then
		row_item.font_size = row_item.font_size * 0.4
	end
	...
	if SystemInfo:platform() == Idstring("NX64") then
		...
		local font_size = row_item.font_size
	...
	if SystemInfo:platform() == Idstring("NX64") and row_item.wants_switch_arrows then
		row_item.arrow_left = row_item.gui_panel:bitmap({
			texture = "guis/textures/switch/direction_arrow_l_selected_hud",
			y = 0,
			x = 0,
			texture_rect = {
				3,
		...
		row_item.arrow_right = row_item.gui_panel:bitmap({
			texture = "guis/textures/switch/direction_arrow_r_selected_hud",
			y = 0,
			x = 0,
			texture_rect = {
				3,
...
function MenuItemMultiChoice:reload(row_item, node)
	...
	local choice_text_colour = not self._enabled and row_item.disabled_color or self:selected_option():parameters().color or node.row_item_hightlight_color

	if choice_text_colour then
		row_item.choice_text:set_color(choice_text_colour)
	end
	...
	if SystemInfo:platform() == Idstring("NX64") then
		self:layout_nx(row_item, node, safe_rect)

		if row_item.colored_background then
			row_item.background:set_world_shape(row_item.gui_panel:world_shape())
		end
	end
...
function MenuItemMultiChoice:layout_nx(row_item, node, safe_rect)
	if row_item.is_large_menu_button then
		row_item.gui_panel:set_right(row_item.gui_panel:parent():w())

		if not row_item.has_scroll_bar then
			row_item.gui_panel:set_w(tweak_data.menu.main_menu_width)
		end
		...
		elseif row_item.jukebox_gui then
			local contact_w = 575

			row_item.gui_panel:set_height(tweak_data.menu.crimenet_contract_buttons_height)
			row_item.choice_text:set_font_size(tweak_data.menu.options_font_size)
			row_item.gui_panel:set_w(contact_w / (row_item.items_on_row or 1))
			row_item.gui_text:set_w(contact_w / (row_item.items_on_row or 1))
		elseif row_item.offshore_payday then
			local offshore_w = 400

			row_item.gui_panel:set_height(row_item.font_size * 1.5)
			row_item.gui_text:set_height(row_item.gui_panel:h())
			row_item.gui_panel:set_w(offshore_w / (row_item.items_on_row or 1))
			row_item.gui_text:set_w(offshore_w / (row_item.items_on_row or 1))
			row_item.gui_panel:set_right(node:_right_align() + 265)
			row_item.gui_text:set_right(row_item.gui_panel:right())
		elseif row_item.switch_reticle then
			row_item.gui_panel:set_w(row_item.gui_panel:parent():w() - 10)
			row_item.gui_panel:set_left(5)
			row_item.gui_panel:set_height(tweak_data.menu.options_button_height)
			row_item.choice_text:set_font_size(tweak_data.menu.options_multichoice_choice_font_size)
			row_item.gui_text:set_font_size(tweak_data.menu.options_multichoice_text_font_size)
			row_item.gui_text:set_w(row_item.gui_panel:w())
		elseif not row_item.is_contract_broker then
			row_item.gui_panel:set_height(tweak_data.menu.options_button_height)
			row_item.choice_text:set_font_size(tweak_data.menu.options_multichoice_choice_font_size)
			row_item.gui_text:set_font_size(tweak_data.menu.options_multichoice_text_font_size)

			if row_item.has_scroll_bar then
				row_item.gui_panel:set_right(row_item.gui_panel:parent():w() / (row_item.items_on_row or 1) - (tweak_data.menu.main_menu_padding + tweak_data.menu.scroll_bar_width))
			else
				row_item.gui_panel:set_right(row_item.gui_panel:parent():w() / (row_item.items_on_row or 1))
			end
		...
		row_item.gui_text:set_color(row_item.color)
		row_item.choice_text:set_color(Color(1, 1, 1))

		local arrow_scale = 0.75
		local arrow_offset_scale = 0.1

		row_item.gui_text:set_y(0)

		if row_item.crimenet_contract_gui or row_item.jukebox_gui then
			row_item.gui_text:set_y(3)

			arrow_scale = 0.75
			arrow_offset_scale = 0.1
		elseif row_item.offshore_payday then
			arrow_scale = 1
			arrow_offset_scale = 0

			row_item.choice_text:set_center_y(row_item.choice_panel:h() / 2)
		end
		...
		row_item.gui_text:set_center_x(row_item.gui_panel:w() / 2)
		row_item.gui_text:set_align("center")

		local padding = tweak_data.menu.checkbox_padding

		row_item.arrow_left:set_h(row_item.gui_panel:h() - padding * 2)
		row_item.arrow_left:set_w(row_item.gui_panel:h() - padding * 2)
		row_item.arrow_right:set_h(row_item.gui_panel:h() - padding * 2)
		row_item.arrow_right:set_w(row_item.gui_panel:h() - padding * 2)
		row_item.arrow_left:set_bottom(row_item.gui_panel:h() - padding)
		row_item.arrow_left:set_x(padding)
		row_item.arrow_right:set_right(row_item.gui_panel:w() - padding)
		row_item.arrow_right:set_bottom(row_item.gui_panel:h() - padding)

		local x, y, text_width, text_height = row_item.choice_text:text_rect()

		row_item.choice_text:set_size(text_width, text_height)
		row_item.choice_panel:set_size(text_width, text_height)
		row_item.choice_panel:set_w(row_item.arrow_right:x() - (row_item.arrow_left:x() + row_item.arrow_left:w()) - row_item.font_size)
		row_item.choice_panel:set_bottom(row_item.arrow_left:bottom() + 2)
		row_item.choice_panel:set_align("center")
		row_item.choice_text:set_align("center")
		row_item.choice_panel:set_center_x(row_item.gui_panel:w() / 2)
		row_item.choice_text:set_center_x(row_item.choice_panel:w() / 2)

		if row_item.offshore_payday then
			row_item.choice_panel:set_center_y(row_item.gui_panel:h() / 2)
			row_item.choice_text:set_top(0)
		end
	...
	elseif row_item.is_contract_broker then
		row_item.gui_panel:set_right(safe_rect.width / 2 + 220)
		row_item.gui_panel:set_top(105)
		row_item.gui_text:set_font_size(24)
		row_item.gui_text:set_w(row_item.arrow_left:x() - (row_item.gui_text:x() + row_item.gui_text:font_size()))
		row_item.gui_panel:set_height(row_item.gui_text:font_size() * 2)
		row_item.gui_text:set_center_y(row_item.gui_panel:h() / 2)
		row_item.arrow_left:set_h(row_item.gui_panel:h() * 0.8)
		row_item.arrow_left:set_w(row_item.gui_panel:h() * 0.8)
		row_item.arrow_right:set_h(row_item.gui_panel:h() * 0.8)
		row_item.arrow_right:set_w(row_item.gui_panel:h() * 0.8)
		row_item.arrow_right:set_right(row_item.gui_panel:w() - row_item.gui_text:font_size() / 2)
		row_item.arrow_left:set_center_y(row_item.gui_panel:h() / 2)
		row_item.arrow_right:set_center_y(row_item.gui_panel:h() / 2)
		row_item.choice_text:set_font_size(24)

		local x, y, text_width, text_height = row_item.choice_text:text_rect()

		row_item.choice_panel:set_size(text_width, text_height)
		row_item.choice_text:set_size(text_width, text_height)
		row_item.choice_panel:set_w(row_item.arrow_right:x() - (row_item.arrow_left:x() + row_item.arrow_left:w()))
		row_item.choice_panel:set_align("center")
		row_item.choice_text:set_align("center")
		row_item.choice_panel:set_center_x(row_item.arrow_left:x() + row_item.arrow_left:w() + (row_item.arrow_right:x() - (row_item.arrow_left:x() + row_item.arrow_left:w())) / 2)
		row_item.choice_panel:set_center_y(row_item.gui_panel:h() / 2)
		row_item.choice_text:set_center_y(row_item.choice_panel:h() / 2)
	end
end
...
function MenuItemMultiChoice:highlight_row_item(node, row_item, mouse_over)
	...
	if SystemInfo:platform() ~= Idstring("NX64") or not row_item.wants_switch_arrows then
