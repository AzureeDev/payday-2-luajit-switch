		...
		if pass_all_tests then
			...
			for i = start_difficulty, num_difficulties do
				...
				if job_jc <= pstars + span and job_jc >= pstars - span then
					slot23.marker_dot_color = job_data.marker_dot_color or nil
					slot23.color_lerp = job_data.color_lerp or nil
		...
		if pass_all_tests then
			...
			for i = start_difficulty, num_difficulties do
				...
				if is_not_level_locked then
					...
					slot29.marker_dot_color = job_data.marker_dot_color or nil
					slot29.color_lerp = job_data.color_lerp or nil
...
function CrimeNetManager:update(t, dt)
	...
	if self._ready_for_features then
		self:launch_features()

		self._ready_for_features = false
	end
...
function CrimeNetManager:start(skip_servers)
	self._ready_for_features = skip_servers
...
function CrimeNetManager:deactivate()
	...
	if SystemInfo:platform() == Idstring("NX64") then
		managers.network.matchmake:map_active(false)
	end
end
...
function CrimeNetManager:activate()
	...
	if SystemInfo:platform() == Idstring("NX64") then
		managers.network.matchmake:map_active(true)
	end
end

function CrimeNetManager:is_active()
	return self._active
end
...
function CrimeNetManager:_find_online_games(friends_only)
	...
	elseif is_nx64 then
		self:_find_online_games_nx64(friends_only)
	else
...
function CrimeNetManager:_find_online_games_nx64(friends_only)
	local function f(info_list)
		managers.network.matchmake:search_lobby_done()
		...
		if info_list then
			...
			for i, room in ipairs(info_list) do
				local owner_name_str = room.owner_name
				local room_id_str = tostring(room.room_id)
				local friend_str = room.friend_id and tostring(room.friend_id)

				if managers.network.matchmake:is_server_ok(friends_only, room.owner_id, room.numbers) then
					local host_name = owner_name_str
					local level_id, name_id, level_name, difficulty_id, difficulty, job_id, state_string_id, state_name, state, num_plrs = self:_server_properties(room.numbers)
					local is_friend = friend_names[tostring(room.owner_id)] and true or false

					if name_id then
						found_rooms[room_id_str] = true

						if not self._active_server_jobs[room_id_str] and table.size(self._active_jobs) + table.size(self._active_server_jobs) < tweak_data.gui.crime_net.job_vars.total_active_jobs then
							self._active_server_jobs[room_id_str] = {
								alive_time = 0,
								added = false,
								room_id = room.room_id,
			...
			for id, _ in pairs(self._active_server_jobs) do
				if not found_rooms[id] then
					print("[CNM] -- DEAD REMOVE: ", id)
	...
	if not NX64Online:server_available() then
		return
	end
...
function CrimeNetManager:enable_launch_features()
	self._ready_for_features = true
end

function CrimeNetManager:launch_features()
...
function CrimeNetGui:init(ws, fullscreeen_ws, node)
	...
	if SystemInfo:platform() == Idstring("NX64") then
		if Global.game_settings.single_player then
			text_id = "menu_crimenet_offline"
		elseif Global.game_settings.local_game then
			text_id = "NX64_menu_crimenet_local"
		else
			text_id = "cn_menu_num_players_offline"
		end
	...
	local num_players_text = self._panel:text({
		...
		font_size = tweak_data.menu.pd2_small_font_size * legend_scale,
	...
	host_icon:set_w(host_icon:w() * legend_scale)
	host_icon:set_h(host_icon:h() * legend_scale)
	...
	join_icon:set_w(join_icon:w() * legend_scale)
	join_icon:set_h(join_icon:h() * legend_scale)
	...
	friends_icon:set_w(friends_icon:w() * legend_scale)
	friends_icon:set_h(friends_icon:h() * legend_scale)
	...
	if managers.crimenet:no_servers() or is_xb1 then
		...
		join_text:hide()
		friends_icon:hide()
		friends_icon:set_bottom(host_text:bottom())
		friends_text:hide()
	...
	risk_icon:set_w(risk_icon:w() * legend_scale)
	risk_icon:set_h(risk_icon:h() * legend_scale)
	...
	ghost_icon:set_w(ghost_icon:w() * legend_scale)
	ghost_icon:set_h(ghost_icon:h() * legend_scale)
	...
	kick_none_icon:set_w(kick_none_icon:w() * legend_scale)
	kick_none_icon:set_h(kick_none_icon:h() * legend_scale)
	...
	kick_vote_icon:set_w(kick_vote_icon:w() * legend_scale)
	kick_vote_icon:set_h(kick_vote_icon:h() * legend_scale)
	...
	if MenuCallbackHandler:bang_active() then
		...
		job_plan_loud_icon:set_w(job_plan_loud_icon:w() * legend_scale)
		job_plan_loud_icon:set_h(job_plan_loud_icon:h() * legend_scale)
		...
		job_plan_stealth_icon:set_w(job_plan_stealth_icon:w() * legend_scale)
		job_plan_stealth_icon:set_h(job_plan_stealth_icon:h() * legend_scale)
	...
	if not no_servers and not is_xb1 then
		...
		elseif SystemInfo:platform() == Idstring("NX64") then
			local invites_button = self._panel:text({
				...
				blend_mode = "add",
				layer = 40,
				text = managers.localization:to_upper_text("nx64_menu_cn_view_invites"),
				font_size = tweak_data.menu.pd2_medium_font_size,
				font = tweak_data.menu.pd2_medium_font,
				color = tweak_data.screen_colors.text
			})
			...
			self._ib1 = invites_button
			local online = false

			invites_button:set_visible(online)
			...
			self._ib2 = blur_object

			blur_object:set_visible(online)
		end
...
function CrimeNetGui:update(t, dt)
	...
	if SystemInfo:platform() == Idstring("NX64") and (self.panning_left or self.panning_right or self.panning_up or self.panning_down) then
		local mx = 0
		local my = 0

		if self.panning_left then
			mx = -10
		end

		if self.panning_right then
			mx = 10
		end

		if self.panning_up then
			my = -10
		end

		if self.panning_down then
			my = 10
		end
	...
	if SystemInfo:platform() == Idstring("NX64") then
		if self._is_zooming_in then
			self:_set_zoom("in", self.zoom_x, self.zoom_y)
		elseif self._is_zooming_out then
			self:_set_zoom("out", self.zoom_x, self.zoom_y)
		end

		self._online_check = (self._online_check or 0) - 1

		if self._online_check < 0 then
			self._online_check = 60
			local online = managers.network.matchmake:is_inet_mode()

			if online ~= self._online_state then
				self._online_state = online

				if alive(self._ib1) then
					self._ib1:set_visible(online)
				end

				if alive(self._ib2) then
					self._ib2:set_visible(online)
				end
			end
		end
	end
...
function CrimeNetGui:mouse_pressed(o, button, x, y)
	...
	if self:mouse_button_click(button) then
		...
		if not SystemInfo:platform() == Idstring("NX64") and self._panel:inside(x, y) then
			self._released_map = nil
			self._grabbed_map = {
				x = x,
				y = y,
	...
	elseif not SystemInfo:platform() == Idstring("NX64") then
	...
	elseif button == Idstring("pinch") then
		self.zoom_x = x
		self.zoom_y = y
		self._is_zooming_out = true
	elseif button == Idstring("spread") then
		self.zoom_x = x
		self.zoom_y = y
		self._is_zooming_in = true
	else
		local left, right, up, down = nil

		if button == Idstring("pan_left") then
			self.panning_left = true
		end

		if button == Idstring("pan_right") then
			self.panning_right = true
		end

		if button == Idstring("pan_up") then
			self.panning_up = true
		end

		if button == Idstring("pan_down") then
			self.panning_down = true
		end
	end
...
function CrimeNetGui:mouse_released(o, button, x, y)
	...
	self._is_zooming_out = false
	self._is_zooming_in = false
...
function CrimeNetGui:_set_zoom(zoom, x, y)
	...
	if zoom == "in" then
		local new_zoom = math.clamp(self._zoom * zoom_factor, self.MIN_ZOOM, self.MAX_ZOOM)
	...
	if is_xb1 or is_ps4 then
		...
		if self._showtext then
			if not alive(self._myname) then
				self._myname = self._panel:text({
					text = "Goris",
					y = 13,
					alpha = 0.5,
					x = 84,
		...
		elseif alive(self._myname) then
			self._myname:hide()
...
function CrimeNetGui:mouse_moved(o, x, y)
	...
	if managers.menu:is_pc_controller() or SystemInfo:platform() == Idstring("NX64") then
		if self._panel:child("back_button"):inside(x, y) then
	...
	if self._grabbed_map and SystemInfo:platform() ~= Idstring("NX64") then
