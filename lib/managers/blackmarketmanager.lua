...
function BlackMarketManager:verify_installed_items()
	...
	for i = 1, CriminalsManager.MAX_NR_CRIMINALS do
		local character = self._global._preferred_characters[i]
		local new_name = CriminalsManager.convert_old_to_new_character_workname(character)
		local char_tweak = tweak_data.blackmarket.characters.locked[new_name] or tweak_data.blackmarket.characters[new_name]
		...
		if not char_tweak.installed then
			print("[BlackMarketManager:verify_installed_items] - Preferred Character is not installed!", character)
	...
	self:_update_preferred_character(true)

	local equipped_mask = self:equipped_mask()
	local valid_mask = tweak_data.blackmarket.masks[equipped_mask.mask_id] and tweak_data.blackmarket.masks[equipped_mask.mask_id].installed

	if not valid_mask then
		print("[BlackMarketManager:verify_installed_items] - Mask is not installed!", equipped_mask.mask_id)
	end

	if valid_mask then
		local mask_mod_lookup = {
			pattern = "textures",
			color = "colors",
			material = "materials"
		}

		for mod, mod_data in pairs(equipped_mask.blueprint) do
			local mod_tweak = tweak_data.blackmarket[mask_mod_lookup[mod]]

			if not mod_tweak or not mod_tweak[mod_data.id] or not mod_tweak[mod_data.id].installed then
				valid_mask = false

				print("[BlackMarketManager:verify_installed_items] - Mask mod is not installed!", equipped_mask.mask_id, mod, mod_data.id)
	...
	local weapon_list = {
		{
			type = "primaries",
			equipped = self:equipped_primary()
		},
		{
			type = "secondaries",
			equipped = self:equipped_secondary()
		}
	}

	for _, weapon in pairs(weapon_list) do
		local equipped_weapon = weapon.equipped
		local valid_weapon = equipped_weapon and tweak_data.weapon[equipped_weapon.weapon_id] and tweak_data.weapon[equipped_weapon.weapon_id].installed

		if not valid_weapon then
			print("[BlackMarketManager:verify_installed_items] - Weapon is not installed!", equipped_weapon.weapon_id)
		end

		if valid_weapon then
			for _, mod in pairs(equipped_weapon.blueprint) do
				if not tweak_data.weapon.factory.parts[mod] or not tweak_data.weapon.factory.parts[mod].installed then
					valid_weapon = false

					print("[BlackMarketManager:verify_installed_items] - Weapon Mod is not installed!", equipped_weapon.weapon_id, mod)
		...
		if not valid_weapon then
			for _, data in pairs(Global.blackmarket_manager.crafted_items[weapon.type]) do
				data.equipped = false
			end

			self:aquire_default_weapons()
			self:equip_weapon(weapon.type, self:equipped_weapon_slot(weapon.type))
		end
	end

	local equipped_melee = self:equipped_melee_weapon()
	local valid_melee = tweak_data.blackmarket.melee_weapons[equipped_melee] and tweak_data.blackmarket.melee_weapons[equipped_melee].installed

	if not valid_melee then
		print("[BlackMarketManager:verify_installed_items] - Melee Weapon is not installed!", equipped_melee)
	end

	if not valid_melee then
		for _, data in pairs(Global.blackmarket_manager.melee_weapons) do
			data.equipped = false
		end

		self:aquire_default_weapons()
		self:equip_melee_weapon(self:equipped_melee_weapon())
	end

	self:equip_grenade(self._defaults.grenade)
end
...
function BlackMarketManager:_cleanup_blackmarket()
	...
	local function chk_global_value_func(global_value, data, real_global_value)
		if data then
			data.global_value = real_global_value or "normal"
		end
						...
						elseif item_tweak_data.inaccessible then
							...
							if not table.contains(global_values, global_value) then
								table.insert(changed_items, {
									id = item,
									category = category,
									global_value = item_tweak_data.infamous and "infamous" or item_tweak_data.dlc or item_tweak_data.global_value or "normal"
								})
								add_invalid_item_func(global_value, category, item)
							else
