...
NX64PlatformManager = NX64PlatformManager or class(GenericPlatformManager)
PlatformManager.PLATFORM_CLASS_MAP[_G.Idstring("NX64"):key()] = NX64PlatformManager

function NX64PlatformManager:init()
	GenericPlatformManager.init(self)

	self._current_nx64_presence = ""
	self._nx64_set_presence_time = 0
end

function NX64PlatformManager:destroy_context()
	GenericPlatformManager.destroy_context(self)
end

function NX64PlatformManager:set_presence(name)
	GenericPlatformManager.set_presence(self, name)
end

function NX64PlatformManager:set_rich_presence(name, callback)
	print("NX64PlatformManager:set_rich_presence", name)
	GenericPlatformManager.set_rich_presence(self, name)
	NX64Online:set_presence_info(managers.localization:text("nx64_presence_" .. name))
end

function NX64PlatformManager:set_playing(is_playing)
	if not Global.game_settings.is_playing ~= not is_playing then
		if not is_playing then
			-- Nothing
		end

		NX64PlatformManager.super.set_playing(self, is_playing)
	end
end
