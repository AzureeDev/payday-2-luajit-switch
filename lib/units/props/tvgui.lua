...
function TvGui:start()
	if not self._playing and self._video_panel then
...
function TvGui:pause()
	if self._playing and self._video_panel then
...
function TvGui:load(data)
	if data.TvGui.playing and self._video_panel then
		self._video_panel:play()
