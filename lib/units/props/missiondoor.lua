...
function MissionDoor:_c4_sequence_done()
	...
	self:c4_explosioin_feedback()
end

function MissionDoor:c4_explosioin_feedback()
	local player = managers.player:player_unit()
	local feedback = managers.feedback:create("mission_triggered")

	if player then
		feedback:set_unit(player)
		feedback:set_enabled("camera_shake", false)
		feedback:set_enabled("rumble", true)
		feedback:set_enabled("above_camera_effect", false)

		local strength_l, strength_r = managers.rumble:_calc_stereo_multiplier(self._unit:position(), 1000)
		local params = {
			"rumble",
			"multiplier_data_l",
			strength_l,
