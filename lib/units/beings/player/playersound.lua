...
function PlayerSound:play(sound_name, source_name, sync)
	...
	if SystemInfo:platform() == Idstring("NX64") and type(sound_name) == "string" then
		local rumble_found, rumble_name = nil
		rumble_found, rumble_name = managers.rumble:audio_presets_contain(sound_name)

		if rumble_found then
			managers.rumble:play(rumble_name, nil, 0.8, nil, 3)
		end
	end
