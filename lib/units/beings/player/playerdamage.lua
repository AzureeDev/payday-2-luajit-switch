...
function PlayerDamage:_on_damage_event()
	...
	if self:get_real_armor() <= 0 and self._has_damage_speed then
...
function PlayerDamage:_on_revive_event()
	...
	if self._heartbeat_rumble then
		managers.rumble:stop(self._heartbeat_rumble)
	end
end
...
function PlayerDamage:damage_melee(attack_data)
	...
	managers.rumble:play("damage_melee", nil, 0.8, nil, 0)
...
function PlayerDamage:damage_bullet(attack_data)
	...
	local direction_to_attacker = attacker_pos - player_pos
	direction_to_attacker = direction_to_attacker / direction_to_attacker:length()
	local hit_pos = player_pos + direction_to_attacker
	local strength_l, strength_r = managers.rumble:_calc_stereo_multiplier(hit_pos, 1000)
	local params = {
		multiplier_data_l = strength_l,
		multiplier_data_r = strength_r
	}

	managers.rumble:play("damage_bullet", nil, 0.8, params, 0)
	self:_hit_direction(attack_data.attacker_unit:position())
...
function PlayerDamage:damage_killzone(attack_data)
	...
	if attack_data.type and (attack_data.type == "gas" or attack_data.type == "fire") then
		self.killzone_rumble = managers.rumble:play("bad_gas", nil, 0.8, nil, 0)
	end

	self:_call_listeners(damage_info)
end

function PlayerDamage:stop_killzone_rumble()
	managers.rumble:stop(self.killzone_rumble)
end
...
function PlayerDamage:update_downed(t, dt)
	if self._downed_timer and self._downed_paused_counter == 0 then
		...
		managers.environment_controller:set_downed_value(self._downed_progression)
		SoundDevice:set_rtpc("downed_state_progression", self._downed_progression)
		managers.rumble:set_multiplier(self._heartbeat_rumble, self._downed_progression * 0.01)
...
function PlayerDamage:on_downed()
	...
	if not managers.rumble:is_playing(self._heartbeat_rumble) and self._revives > 0 then
		self._heartbeat_rumble = managers.rumble:play("heartbeat", nil, 0.8, nil, 0)
	end
end
