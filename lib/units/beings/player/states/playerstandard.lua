...
function PlayerStandard:_do_action_melee(t, input, skip_damage)
	...
	if self._state_data.melee_charge_rumble then
		managers.rumble:stop(self._state_data.melee_charge_rumble)

		self._state_data.melee_charge_rumble = nil
	end
	...
	if instant_hit then
		...
		managers.rumble:play("melee_pop", nil, 0.8, nil, 0)
	end
end
...
function PlayerStandard:_do_melee_damage(t, bayonet_melee, melee_hit_ray)
	...
	if col_ray and alive(col_ray.unit) then
		...
		local hit_glass = false

		for _, material in ipairs(hit_unit:get_objects_by_type(Idstring("material"))) do
			if material:name() == Idstring("mat_glass") then
				hit_glass = true
			end
		end

		managers.rumble:play(hit_glass and "smash_glass" or "melee_hit")
...
function PlayerStandard:_update_melee_timers(t, input)
	...
	if self._state_data.melee_attack_allowed_t and self._state_data.melee_attack_allowed_t <= t then
		...
		if not self._state_data.melee_charge_rumble then
			self._state_data.melee_charge_rumble = managers.rumble:play("melee_charge", nil, 0.8, nil, 0)
		end
	end
...
function PlayerStandard:_get_intimidation_action(prime_target, char_table, amount, primary_only, detect_only, secondary)
	...
	if prime_target then
		if prime_target.unit_type == unit_type_teammate then
			...
			if rally_skill_data and mvector3.distance_sq(self._pos, record.m_pos) < rally_skill_data.range_sq and self._unit:movement():current_state_name() ~= "arrested" then
...
function PlayerStandard:_check_action_primary_attack(t, input)
	...
	if action_wanted then
		local action_forbidden = self:_is_reloading() or self:_changing_weapon() or self:_is_meleeing() or self._use_item_expire_t or self:_interacting() or self:_is_throwing_projectile() or self:_is_deploying_bipod() or self._menu_closed_fire_cooldown > 0
				...
				elseif self._running and not self._equipped_unit:base():run_and_shoot_allowed() then
					...
					if fired then
						...
						if SystemInfo:platform() == Idstring("NX64") then
							if not weap_base:is_category("saw", "flamethrower") then
								local delay = 4
								local rumble_name = "weapon_fire"

								if weap_tweak_data and weap_tweak_data.rumbles and weap_tweak_data.rumbles.fire then
									rumble_name = weap_tweak_data.rumbles.fire
								else
									for i = 1, table.getn(self.rumbles) do
										if weap_base:is_category(self.rumbles[i].name) then
											rumble_name = rumble_name .. "_" .. self.rumbles[i].name
											delay = self.rumbles[i].delay
								...
								if weap_base:got_silencer() then
									rumble_name = rumble_name .. "_s"
								end

								if fire_mode == "auto" then
									rumble_name = rumble_name .. "_auto"
								end

								if weap_base.akimbo then
									rumble_name = rumble_name .. "_r"
								end

								managers.rumble:play(rumble_name, nil, 0.8, nil, delay)
							end
						else
