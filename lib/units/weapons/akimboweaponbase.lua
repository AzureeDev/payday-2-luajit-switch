...
function AkimboWeaponBase:_fire_second(params)
	if alive(self._second_gun) and self._setup and alive(self._setup.user_unit) then
		...
		if fired then
			...
			managers.hud:set_ammo_amount(self:selection_index(), self:ammo_info())
			self._second_gun:base():tweak_data_anim_play("fire")
			managers.rumble:play(self:_get_rumble_name(), nil, 0.8, nil, 3)
		end
...
function AkimboWeaponBase:_get_rumble_name()
	local rumble_name = "weapon_fire"

	self:weapon_tweak_data()

	if self:weapon_tweak_data() and self:weapon_tweak_data().rumbles and self:weapon_tweak_data().rumbles.fire then
		rumble_name = self:weapon_tweak_data().rumbles.fire
	elseif self:is_category("pistol") then
		rumble_name = rumble_name .. "_pistol"
	elseif self:is_category("smg") then
		rumble_name = rumble_name .. "_smg"
	end
