...
function SawWeaponBase:on_unequip(user_unit)
	SawWeaponBase.super.on_unequip(self, user_unit)
	managers.rumble:stop(self.idle_rumble)
	managers.rumble:stop(self.sawing_rumble)
end
...
function SawWeaponBase:stop_shooting(...)
	self:_stop_sawing_effect(true, true)
...
function SawWeaponBase:_start_sawing_effect(is_player)
	if not self._active_effect then
		...
		if is_player == true then
			...
			if not managers.rumble:is_playing(self.sawing_rumble) then
				self.sawing_rumble = managers.rumble:play("weapon_fire_saw", nil, 0.8, nil, 0)
			end
		end
	end
end

function SawWeaponBase:_stop_sawing_effect(is_player, shooting_stopped)
	...
	if is_player == true then
		...
		if not managers.rumble:is_playing(self.idle_rumble) and not shooting_stopped then
			self.idle_rumble = managers.rumble:play("weapon_idle_saw", nil, 0.8, nil, 0)
		end
	end
end
...
function SawWeaponBase:fire(from_pos, direction, dmg_mul, shoot_player, spread_mul, autohit_mul, suppr_mul, target_unit)
	...
	if hit_something then
		self:_start_sawing_effect(true)
		...
		self:_stop_sawing_effect(true, false)
	end
