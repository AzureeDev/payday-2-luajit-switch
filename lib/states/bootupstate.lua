...
function BootupState:init(game_state_machine, setup)
	...
	if SystemInfo:platform() == Idstring("NX64") then
		NX64Online:signal_is_bootup_started()
	end
...
function BootupState:check_confirm_pressed()
	...
	for index, controller in ipairs(self._controller_list) do
		if controller:get_input_pressed("confirm") or is_NX64 and #Input:mouse():pressed_list() > 0 then
...
function BootupState:is_skipped()
	...
	for _, controller in ipairs(self._controller_list) do
		if controller:get_any_input_pressed() or is_NX64 and #Input:mouse():pressed_list() > 0 then
			return true
		end
	end
...
function BootupState:play_next(is_skipped)
	...
	if self._play_data then
		...
		if SystemInfo:platform() == Idstring("NX64") then
			NX64Online:signal_is_bootup_finished()
		end
