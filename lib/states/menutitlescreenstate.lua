...
function MenuTitlescreenState:setup()
	...
	local text_id = (is_nx64 or is_ps3 or is_x360 or is_ps4 or is_xb1) and "menu_press_start" or "menu_visit_forum3"
	...
	if is_nx64 then
		text:set_center_y(self._workspace:panel():h() / 2 + 80)
		text:set_center_x(self._workspace:panel():w() / 2)
	end
...
function MenuTitlescreenState:get_start_pressed_controller_index()
	for index, controller in ipairs(self._controller_list) do
		...
		elseif is_nx64 then
			if controller:get_input_pressed("use_item") then
				managers.rumble:play("start_l", nil, 0.8, nil, 0, true)
			end

			if controller:get_input_pressed("interact") then
				managers.rumble:play("start_r", nil, 0.8, nil, 0, true)
			end

			if controller:get_input_bool("use_item") and controller:get_input_bool("interact") then
				if controller:is_joycon() then
					managers.controller.nx_controller_type = "joycon"
				else
					managers.controller.nx_controller_type = "switch_pro"
				end
...
function MenuTitlescreenState:play_attract_video()
	...
	local res = Vector3(1280, 720, RenderSettings.resolution.z)
