...
function ViewportManager:end_frame(t, dt)
	...
	if SystemInfo:platform() == Idstring("NX64") and NX64Online:is_resolution_changed() then
		self:resolution_changed()
	end
...
function ViewportManager:get_safe_rect()
	local a = is_win32 and 0.032 or (is_ps4 or is_xb1 or is_nx64) and 0.05 or 0.075
	local b = 1 - a * 2
