...
function ControllerWrapper:update(t, dt)
	...
	if SystemInfo:platform() == Idstring("NX64") and managers and managers.controller then
		if self:is_joycon() then
...
function ControllerWrapper:is_joycon(controller_id)
	if SystemInfo:platform() == Idstring("NX64") then
		if controller_id then
			return self._controller_map[controller_id]:is_joycon()
		else
			for _, controller in pairs(self._controller_map) do
				if not controller:is_joycon() then
					return false
				end
			end
