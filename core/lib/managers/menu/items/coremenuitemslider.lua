...
function ItemSlider:increase()
	if SystemInfo:platform() == Idstring("NX64") then
		self:set_value(self._value + self._step * 0.25)
		managers.menu:update_slider_rumble(self:percentage() * 0.01, false)
	else
...
function ItemSlider:_layout(node, row_item)
	...
	if SystemInfo:platform() == Idstring("NX64") and row_item.is_large_menu_button then
		...
		row_item.gui_panel:set_height(_G.tweak_data.menu.slider_button_height)
		...
		row_item.gui_text:set_width(row_item.gui_panel:w())
		row_item.gui_text:set_height(h + _G.tweak_data.menu.checkbox_padding * 2)
		row_item.gui_text:set_y(_G.tweak_data.menu.checkbox_padding)
	...
	if SystemInfo:platform() == Idstring("NX64") and row_item.is_large_menu_button then
		row_item.gui_slider_bg:set_bottom(row_item.gui_panel:h() - sh * 0.75 - _G.tweak_data.menu.slider_button_nx_bar_lift)
		row_item.gui_slider_bg:set_w(row_item.gui_panel:w() * 0.9)
		row_item.gui_slider_bg:set_x(row_item.gui_text:x())
	...
	if not row_item.is_large_menu_button then
