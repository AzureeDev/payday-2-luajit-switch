...
function MenuNode:select_item(item_name)
	if SystemInfo:platform() == Idstring("NX64") then
		if item_name then
			local found = self:item(item_name)
		...
		if not def or not def:visible() or def.no_select then
			for i, item in ipairs(self:items()) do
				if item:visible() and item._type ~= "divider" and not item.no_select then
